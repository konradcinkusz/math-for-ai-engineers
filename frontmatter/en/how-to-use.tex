...
Each program is a stream of small numbered chunks called \emph{frames}. Most
frames end by asking you for something \dash{} a number, a word, a line of
...
at Program~1 and read the main parts in order \dash{} they are not
independent, and where one program leans on an earlier one it names it on the
spot.
...
Kulik, Schwalb and Kulik's 1982 meta-analysis of the format in secondary
...
\medskip
\textbf{Every research claim above is somebody else's, so it is cited rather
than asserted.} The sources are in Appendix~\ref{app:E},
section~\ref{sec:E-sources}, which also records the one claim made here
\dash{} that small steps and immediate reinforcement turned out not to be
essential \dash{} for which no source was surveyed.
\end{warning}
