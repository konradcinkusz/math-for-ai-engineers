...
every frame ends by asking you for something --- a number, a word, a line of
...
paper is enough. Write your answer down --- on paper, not in your head ---
...
\begin{enumerate}[leftmargin=1.6em, itemsep=3pt]
  ...
  \item \emph{now one for you to do} --- you solve it, and check in the next
        frame;
...
\textbf{Quiz} & Foundation programs only. Take it \emph{before} the program:
  it tells you whether you need the program at all, and if you only need part
  of it, which part. Each question names the frames that teach it. \\
\textbf{Frames} & The program itself. \\
...
program $\rightarrow$ \textbf{Can you?} $\rightarrow$ \textbf{retake the Quiz}
$\rightarrow$ Test exercises $\rightarrow$ Further problems
\end{tcolorbox}
\end{center}

\noindent
Most people skip the retake, and the retake is the most informative thing in
the sequence. The Quiz is a diagnostic on the way in and a test on the way out;
the difference between the two scores is the only honest measure of what a
program did for you, and it is frequently smaller than the feeling of having
understood it.
...
at Program~1 and read the main parts in order --- they are not independent, and
...
\begin{note}
Stroud's method was validated to an \textbf{80/80} standard: at least 80 per
cent of students scoring at least 80 per cent, in trials with control groups.
The finding that matters is not that the mean rose but that the \emph{spread
narrowed} --- the method lifts the bottom of the distribution.

\textbf{This book has not been tested on a single reader.} It borrows the
method, and it is not entitled to borrow the evidence. Every build of this
book prints that as outstanding work, and it will keep printing it until
somebody runs the trial.
\end{note}
