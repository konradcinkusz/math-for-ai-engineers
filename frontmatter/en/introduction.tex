...
\textbf{It assumes no mathematics beyond school arithmetic.} Not algebra, not
trigonometry, not calculus. Part~I begins with arithmetic and means it, and if
you have a degree in a numerate subject the Quizzes exist precisely so you can
skip what you already have.

\textbf{It does assume you already work with these systems.} That is a
different floor and it is worth stating, because the two are easy to run
together. Program~F1 has you compute what a seven-billion-parameter model
weighs, and it does not stop to say what a parameter is; a later program reads
an attention step as a shape. Every payoff in this book is a payoff \emph{in
your job}, so the vocabulary of that job \dash{} model, token, parameter,
training run, evaluation \dash{} is assumed throughout and the mathematics is
not.

If you have never trained or served a model, the mathematics here will still
be correct and the reasons for caring about it will not land. There is no
shame in that and this is not the book for it.
...
What you get instead is the other half of that skill, and it has a program to
itself: how to read a theorem somebody else has proved, and say exactly what it
assumes, what it claims, what would refute it and what it is silent about. That
is the half this job actually needs, and the boxes above are it being practised
before it was named.
...
and more than half of them demand a response \dash{} $\val{appf.elicit.pct}$
per cent, a figure Appendix~F prints on every build. The answer is at the top
of the next frame, so you are asked to commit before you check. The effect is
that retrieval practice is not something you have to organise for yourself at
the end of a chapter \dash{} it is built into the way the pages turn.
...
Nine parts, \val{appf.programs} programs.

\textbf{Part~I \dash{} Foundation} (F1--F13) assumes nothing and is triaged by
Quiz. It is bent towards the payoff rather than towards a school syllabus:
logarithms get a whole program because Program~P2 computes in log space, and
the chain rule gets one because backpropagation is the chain rule and nothing
else.

\textbf{Part~II \dash{} Number, precision and cost} (P1--P3) comes before linear
...
represent $\num{0.1}$ has spent four hundred pages teaching a fiction.

\textbf{Part~III \dash{} Linear algebra} (P4--P11) is the largest part, because it
is the subject this audience uses daily and understands least.

\textbf{Part~IV \dash{} Discrete structures and argument} (P12--P14) covers
...
\textbf{Part~V \dash{} Calculus and automatic differentiation} (P15--P18) takes the
chain rule all the way to what \code{loss.backward()} does.

\textbf{Part~VI \dash{} Optimisation} (P19--P22) is absent from Stroud's first
volume entirely.

\textbf{Part~VII \dash{} Probability and statistics} (P23--P28) goes past the
normal distribution, where Stroud stops, to inference and Bayes \dash{} because
...
\textbf{Part~VIII \dash{} Information theory} (P29--P31) is where the loss function
comes from.

\textbf{Part~IX \dash{} Assembling it} (P32--P34) spends the whole book on one
...
example \dash{} it is something the reader will carry.
\end{note}
...
Where Polish and English mathematical usage genuinely differ \dash{} $\mathrm{tg}$ against
$\mathrm{tan}$, the decimal comma, the angle brackets Polish tradition uses for a
closed interval, $D^2(X)$ against $\Var(X)$ \dash{} the decision is recorded in
