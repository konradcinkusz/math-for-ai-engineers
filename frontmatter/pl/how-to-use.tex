...
\emph{ramkami}. Większość ramek kończy się pytaniem \dash{} o liczbę, słowo
...
\textbf{Quiz} & Rozwiąż go \emph{przed} programem: mówi, czy w ogóle
  potrzebujesz tego programu, a jeśli tylko fragmentu, to którego. Każde
  pytanie wskazuje ramki, które go uczą. To test diagnostyczny, nigdy ocena. \\
...
program $\rightarrow$ \textbf{Czy potrafisz?} $\rightarrow$ \textbf{Zadania
sprawdzające} $\rightarrow$ Dalsze wyzwania
\end{tcolorbox}
\end{center}

\noindent
Większość ludzi pomija Zadania sprawdzające, a to właśnie one niosą
najwięcej informacji. Quiz jest testem diagnostycznym na wejściu i niczym
więcej: rozwiązany po raz drugi to te same pytania zadane czytelnikowi, który
pamięta już odpowiedzi, więc różnica między dwoma wynikami Quizu mierzy twoją
pamięć. Zadania sprawdzające punktują to, czego uczył program, a ich wynik
bywa wyraźnie niższy niż poczucie zrozumienia.
...
Następnie zacznij od Programu~1 i czytaj główne części po kolei \dash{} nie
są niezależne, a tam, gdzie jeden program opiera się na wcześniejszym, wskazuje
go na miejscu.
...
Metaanaliza Kulik, Schwalba i Kulika z 1982 roku, obejmująca ten format w
...
\medskip
\textbf{Każde twierdzenie badawcze powyżej jest cudze, więc jest cytowane, a
nie zakładane.} Źródła są w Dodatku~\ref{app:E}, w
sekcji~\ref{sec:E-sources}, która odnotowuje też jedno postawione tu
twierdzenie \dash{} że małe kroki i natychmiastowe wzmocnienie okazały się
nieistotne \dash{} dla którego nie zbadano żadnego źródła.
\end{warning}
