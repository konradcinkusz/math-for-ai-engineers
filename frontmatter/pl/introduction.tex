...
\textbf{Nie zakłada matematyki wykraczającej poza szkolną arytmetykę.} Ani
algebry, ani trygonometrii, ani rachunku różniczkowego. Część~I zaczyna się od
arytmetyki i to nie jest figura retoryczna, a jeśli masz wykształcenie ścisłe,
Quizy istnieją właśnie po to, żebyś mógł pominąć to, co już umiesz.

\textbf{Zakłada natomiast, że już pracujesz z takimi systemami.} To inna
podłoga i warto to powiedzieć wprost, bo obie łatwo pomylić. Program~F1 każe ci
policzyć, ile waży model o siedmiu miliardach parametrów, i nie zatrzymuje się,
żeby wyjaśnić, czym jest parametr; dalszy program czyta krok uwagi jako
kształt. Każda korzyść w tej książce jest korzyścią \emph{w twojej pracy}, więc
słownictwo tej pracy \dash{} model, token, parametr, trening, ewaluacja \dash{}
jest zakładane wszędzie, a matematyka nie.

Jeśli nigdy nie trenowałeś ani nie serwowałeś modelu, matematyka tutaj nadal
będzie poprawna, a powody, żeby się nią przejmować, nie trafią. Nie ma w tym
nic wstydliwego i nie jest to książka do tego.
...
Dostajesz za to drugą połowę tej umiejętności i ma ona własny program: jak
czytać twierdzenie udowodnione przez kogoś innego i powiedzieć dokładnie, co
ono zakłada, co orzeka, co by je obaliło i o czym milczy. To ta połowa, której
ta praca naprawdę potrzebuje, a ramki powyżej są nią ćwiczoną, zanim została
nazwana.
...
Maszyneria jest prosta. Książka to ciąg ponumerowanych \emph{ramek} i ponad
połowa z nich żąda odpowiedzi \dash{} $\val{appf.elicit.pct}$ procent, a liczbę
tę Dodatek~F podaje przy każdym budowaniu. Odpowiedź znajduje się na początku
następnej ramki, więc masz się zadeklarować, zanim sprawdzisz. Efekt jest taki,
że przypominanie z pamięci nie jest czymś, co musisz sobie sam zorganizować na
końcu rozdziału \dash{} jest wbudowane w sposób, w jaki przewraca się strony.
...
Dziewięć części, \val{appf.programs} programów.
...
programowi: logarytmy dostają cały program, bo Program~P2 liczy w skali
logarytmicznej, a reguła łańcuchowa dostaje swój, bo propagacja wsteczna
\emph{jest} regułą łańcuchową i niczym więcej.
...
\textbf{Część~III \dash{} Algebra liniowa} (P4--P11) jest największa, bo to
dziedzina, z której ten czytelnik korzysta codziennie i którą rozumie
najsłabiej.

\textbf{Część~IV \dash{} Struktury dyskretne i argumentacja} (P12--P14) obejmuje
...
(P15--P18) prowadzi regułę łańcuchową aż do tego, co robi \code{loss.backward()}.

\textbf{Część~VI \dash{} Optymalizacja} (P19--P22) w pierwszym tomie Strouda nie
występuje w ogóle.

\textbf{Część~VII \dash{} Prawdopodobieństwo i statystyka} (P23--P28) wychodzi poza
...
\textbf{Część~VIII \dash{} Teoria informacji} (P29--P31) to miejsce, z którego
bierze się funkcja straty.

\textbf{Część~IX \dash{} Złożenie} (P32--P34) poświęca całą książkę jednej
