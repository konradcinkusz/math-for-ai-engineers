...
retoryczna. Jeśli masz wykształcenie ścisłe, nie szkodzi --- Quizy istnieją
właśnie po to, żebyś mógł pominąć to, co już umiesz --- ale podłoga jest
...
końcu rozdziału --- jest jedynym sposobem, w jaki da się przewracać strony.
...
\textbf{Część~I --- Podstawy} (F1--F13) nie zakłada niczego i jest sortowana
...
\textbf{Część~II --- Liczba, precyzja i koszt} (P1--P3) wyprzedza algebrę
...
przyzna, że maszyna nie potrafi przedstawić liczby $0{,}1$, przez czterysta
stron uczyła fikcji.

\textbf{Część~III --- Algebra liniowa} (P4--P10) jest największa, bo to
dziedzina, z której ten czytelnik korzysta codziennie i którą rozumie
najsłabiej.

\textbf{Część~IV --- Struktury dyskretne i argumentacja} (P11--P13) obejmuje
...
\textbf{Część~V --- Rachunek różniczkowy i różniczkowanie automatyczne}
(P14--P17) prowadzi regułę łańcuchową aż do tego, co robi \code{loss.backward()}.

\textbf{Część~VI --- Optymalizacja} (P18--P21) w pierwszym tomie Strouda nie
występuje w ogóle.

\textbf{Część~VII --- Prawdopodobieństwo i statystyka} (P22--P27) wychodzi poza
rozkład normalny, na którym Stroud się zatrzymuje, ku wnioskowaniu i metodzie
bayesowskiej --- bo prawdziwym statystycznym zadaniem tego czytelnika jest
rozstrzygnięcie, czy zmierzona różnica jest prawdziwa, a tego nie obejmuje
żaden podręcznik matematyki inżynierskiej.

\textbf{Część~VIII --- Teoria informacji} (P28--P30) to miejsce, z którego
bierze się funkcja straty.

\textbf{Część~IX --- Złożenie} (P31--P33) poświęca całą książkę jednej
...
matematycznej nie jest zepsutym przykładem --- jest czymś, co czytelnik ze
sobą zabierze.
\end{note}
...
liczby, pozostaje pusta. Puste tabele nie są przeoczeniem --- są mechanizmem.
\end{note}
...
Tam, gdzie polski i angielski uzus matematyczny naprawdę się różnią ---
$\tg$ wobec $\tan$, przecinek dziesiętny, nawiasy ostre polskiej tradycji dla
przedziału domkniętego, $D^2(X)$ wobec $\Var(X)$ --- decyzja jest zapisana w
