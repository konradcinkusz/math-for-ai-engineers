...
% The frame number as a filled badge. NOT in the margin: a margin number has
% to alternate sides under `twoside` and would be the first thing to overflow
% on a 17 cm page.
\newcommand{\framebadge}{%
  \begingroup
  \setlength{\fboxsep}{3pt}%
  \colorbox{mblue}{\textcolor{white}{\bfseries\footnotesize\theframeno}}%
  \endgroup
}

% The boundary between two frames: a rule with the new frame's number sitting
% on its left end, like a tab on a card.
%
% The badge used to be the first thing in the text flow instead. That reads
% correctly only when a frame opens with a sentence -- and about a third of
% them open with \ans, whose box starts a new vertical block, so the badge was
% left stranded alone on the line above it with the box beneath. Putting the
% number on the rule makes the boundary one object, and it no longer matters
% what the frame opens with.
%
% \smash inside a zero-width \makebox: the badge must not contribute height,
% or it would push the rule down and reintroduce the gap it was meant to close.
% Order matters: the rule goes down FIRST, inside an \rlap so it contributes no
% width, and the badge is painted over it second. Drawing the badge first and
% the rule second -- the obvious way round -- puts a hairline straight through
% the digit, and at this size that reads as a struck-out number rather than as
% a badge sitting on a line.
%
% The shift is measured rather than guessed: centre the badge's own box on the
% rule, so the alignment survives a change of point size. It has to, because
% the A4 edition sets 12pt and the trade edition 11pt from the same source.
\newcommand{\framerule}{%
  \par\penalty-250
  ...
  \sbox0{\framebadge}%
  \smash{\raisebox{\dimexpr0.3pt-0.5\ht0+0.5\dp0\relax}{\usebox0}}%
