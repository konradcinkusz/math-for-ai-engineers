...
\item Obserwujesz od $\val{p33.watch}$ kroków i strata nie zeszła poniżej
  wartości początkowej. Czy trening utknął?
  \teachesat{15--16}
...
\item Panel wygładza krzywą straty przy $\beta = \val{p33.beta.dash}$. Jaką
  ...
  \teachesat{30--31}
  \answerto{$\val{p33.smooth.pct}$ procent. Wariancja po wygładzeniu to
  $\frac{1-\beta}{1+\beta}$ surowej, więc o obrazie decyduje ustawienie tak
  samo jak trening.}
...
\begin{fr}
Obserwujesz od $\val{p33.watch}$ kroków i strata nie zeszła poniżej wartości,
którą miała, gdy zacząłeś obserwować.
...
\sumitem{5--6}{Najlepszy rozmiar kroku na misie Programu~\ref{prog:P17} to
  $\val{p33.eta.best}$, a zabójczy $\val{p33.eta.bound}$: pasmo szerokie na
  $\val{p33.eta.margin}$ procent.}
\sumitem{7--8}{Średnia z partii straty o rozrzucie na przykład
  $\val{p33.sigma.token}$ po $\val{p33.batch}$ przykładach ma rozrzut
  $\val{p33.sigma.step}$.}
\sumitem{9--11}{Z sześciu elementów $\val{p33.loud}$ zgłaszają coś, a
...
\sumitem{15--17}{Zdrowy trening, którego strata spada o $\val{p33.improve}$ na
  krok, nie pokazuje poprawy przez $\val{p33.watch}$ kroków w
  $\val{p33.p.flat}$ procentach przypadków.}
\sumitem{18--21}{Płaski odcinek musi trwać około $\val{p33.plateau.k}$ kroków,
  zanim zdrowy trening produkowałby go rzadziej niż w $\val{p33.alpha}$
  przypadków.}
\sumitem{22--23}{Trening liczący $\val{p33.steps}$ kroków zawiera około
  $\val{p33.spike.expected}$ odchyleń o cztery rozrzuty niezależnie od tego,
  co jeszcze jest o nim prawdą.}
\sumitem{24--27}{Po korekcie o długość treningu skok musi przekroczyć
  $\val{p33.spike.z}$ rozrzutu, żeby zaskakiwał \dash{} arytmetyka
  Programu~\ref{prog:P27} z krokami w miejsce modeli.}
\sumitem{28--33}{Średnia wykładnicza pokazuje
...
\sumitem{34--37}{Rozbieżność to jedyny kształt niepotrzebujący statystyki:
  powyżej $\eta = \val{p33.eta.bound}$ czynnik większy niż jeden jest
  stosowany co krok.}
\sumitem{40--43}{Czyste prawo potęgowe dopasowane do prawdy postaci
...
\sumitem{48--50}{Jego własna raportowana niepewność jest zbudowana z reszt,
...
\item Zespół akumuluje cztery mikropartie o $1000$, $10$, $500$ i $490$
  ...
  tym z dużej partii?
  \answerto{Nie. Średnia ważona tokenami daje $\num{2.475}$; zakumulowana
  uśrednia te cztery i daje $4$. Błąd jest cichy, jest w gradiencie, a nie w
  raporcie, i jest pułapką Programu~\ref{prog:P21} z jedną mikropartią
  więcej.}
