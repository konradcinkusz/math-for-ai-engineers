...
\outcome{20--28}{powiedzieć, co niedoskonały sędzia robi ze zmierzoną różnicą
  i o czym mówi publikowana liczba jego zgodności}
...
\outcome{48--52}{wymienić trzy rzeczy, które książka może zrobić z liczbą,
  której nie potrafi ci podać, i powiedzieć, która z nich zostawia dług}
\end{outcomes}
...
\item Dwa modele dzieli $\val{p27.gap.pts}$ punktu na jednym zbiorze
  ...
  w zbiorze zmienia się nieznacznie. Co może się stać z uporządkowaniem?
  \teachesat{11--14}
...
\begin{fr}
Program~\ref{prog:P27} zostawił ci zbiór ewaluacyjny: $\val{p27.items}$
pozycji, najlepszy model na $\val{p27.near.hi}$ procentach, następny na
...
Zauważ, ile kosztuje nazwanie tego próbą. Nikt nie wylosował tych dwustu
pozycji z niczego; ktoś je napisał, w jakimś celu, pewnego popołudnia.
Traktowanie zbioru jako próby jest więc decyzją modelową, a nie obserwacją
\dash{} i jest to decyzja, która uprawnia każdy wzór, jaki ten program zaraz
pożyczy.
...
Tablica wyników tamtego programu to inna tabela niż zbiór powyżej:
$\val{p27.models}$ modeli, każdy oceniony na $\val{p27.lb.n}$ pozycjach.
Która z dwóch części wnosi tam więcej i o ile?
\dotline
\nextframe
\end{fr}

\begin{fr}
\ans{Niedopasowanie: kwadrat jego obciążenia jest $\val{p34.mis.ratio}$
  raza większy od wariancji.}
...
To są dwie zapisane liczby z Programu~\ref{prog:P27}, co do cyfry. To ten sam
zbiór ewaluacyjny, z jedną rzeczą o nim zapisaną, która była tam od początku.
...
$\val{p27.items}$ musi zmienić rodzaj, zanim oba modele zamienią się
miejscami?
\dotline
\nextframe
\end{fr}
...
drugiego i uporządkowanie, które raportuje zbiór, jest odwrotne.
...
Postaw to obok dwóch liczb, o które to porównanie naprawdę się spiera.
Omawiana różnica to $\val{p27.gap.pts}$ punktu. Program~\ref{prog:P27}
policzył, że ten zbiór ewaluacyjny nie odróżni obu modeli, dopóki różnica nie
sięgnie $\val{p27.net.needed.pts}$ punktu.
...
% ============================================================
\section{Ile kosztuje precyzja}
\label{sec:P34-interval}
\index{ewaluacja!koszt precyzji}

\begin{fr}
Program~\ref{prog:P27} zapisał dwie liczby o jednej pozycji tablicy wyników,
a ta sekcja je wycenia.

Pozycja na $\val{p27.lb.p.pct}$ procentach przy $\val{p27.lb.n}$ pozycjach ma
błąd standardowy $\val{p27.lb.se}$ punktu. Tamten program pokazał też coś
większego, co siedzi na tym z wierzchu: posortuj $\val{p27.models}$ modeli o
\emph{równej} prawdziwej skuteczności po ich zaobserwowanych wynikach, a ten
na szczycie siedzi $\val{p27.lb.margin}$ punktu powyżej własnej prawdy
\dash{} $\val{p27.emax}$ takich błędów standardowych. To jest \textbf{efekt
selekcji}, a nie błąd pomiaru, i jest zbudowany z tego samego błędu
standardowego, więc kurczy się z liczbą pozycji z tego samego powodu.

Załóż, że tyle luzu to za dużo, żeby działać, i chcesz połowy. Ile pozycji?
\dotline
\nextframe
\end{fr}
...
Precyzja idzie jak $1/\sqrt{n}$, więc połowienie marginesu to podniesienie
...
Pociągnij o krok dalej. Ile pozycji, żeby zbić to przekroczenie poniżej
jednego punktu?
\dotline
\nextframe
\end{fr}
...
Margines wynosi tu $\val{p34.lb.margin}$, gdzie Program~\ref{prog:P27}
drukuje $\val{p27.lb.margin}$, i ta dodatkowa cyfra jest sednem, a nie
pedanterią: ta linia \emph{podnosi go do kwadratu}, a kwadrat jest miejscem,
w którym zaokrąglenie widać. Przy $\val{p27.lb.margin}$ ten sam zapis daje
$\num{9610}$. Obie liczby są więc dobre do trzech cyfr i ani o jedną dalej,
i dlatego liczba pozycji jest zapisana tak, jak jest.

Blisko dziesięciu tysięcy pozycji, żeby przybić jedną liczbę z dokładnością do
punktu.

Program~\ref{prog:P27} policzył z tej samej maszynerii liczby na inne
pytanie: ile pozycji potrzeba, żeby \emph{odróżnić dwa modele} o punkt.
Zapisał jedną dla ewaluacji biegnących na różnych promptach i jedną dla
biegnących na tych samych. Jak się te dwie mają do tej tutaj?
\dotline
\nextframe
\end{fr}

\begin{fr}
\ans{$\val{p27.n.rho00}$ na różnych promptach i $\val{p27.n.rho90}$ na tych
  samych.}

Puść oba modele na \emph{różnych} promptach, a oddzielenie ich o punkt
kosztuje odrobinę więcej niż przybicie własnej liczby jednego modelu do punktu
\dash{} $\val{p27.n.rho00}$ wobec $\val{p34.one.pt.n}$, czynnik
$\val{p34.detect.ratio}$. Puść je na \emph{tych samych} promptach, a kosztuje
$\val{p27.n.rho90}$, mniej więcej ósmą część: czynnik
$\val{p34.paired.ratio}$ w drugą stronę.

Bierze się to z porównania sparowanego z Programu~\ref{prog:P27}. Sparowane
ewaluacje dzielą wszystko to, co czyni pozycję łatwą albo trudną, więc
wspólna część skraca się w \emph{różnicy}, a liczba pozycji spada dokładnie o
$1-\rho$. Nic w żadnym z modeli się nie zmieniło i niczego nie dokupiono.
Ewaluację uruchomiono inaczej.

\begin{note}
To jest uczciwy odczyt tych liczb: są \textbf{deklaracją budżetu}. Mówią,
...
dziesięciu tysięcy pozycji na model \dash{} dlatego niemal każde publikowane
porównanie jest raczej niedomocowane niż błędne, i dlatego najtańszą rzeczą na
tej stronie jest decyzja o tym, jak ewaluacja jest uruchamiana, a nie o tym,
ile jej kupić.
\end{note}
\end{fr}
...
Efekt kurczy się o $a - b$, dokładnie, na mocy tożsamości, w której nie ma
$p$; arytmetyka z Sekcji~\ref{sec:P34-interval} mówi zaś, że liczba skaluje
się jak kwadrat rozdzielczości, którą kupuje. Liczba pozycji dzieli się więc
przez $(a-b)^{2}$ \dash{} o połowę więcej pozycji z okładem, kupione niczym
innym niż ocenianiem.

Połowa szumowa nie jest całkiem nietknięta i warto być dokładnym co do tego,
która połowa jest. Oceniona pozycja zostaje \emph{uznana} za dobrą z
prawdopodobieństwem $p\,a + (1-p)\,b$, a nie $p$, czyli z raportowaną
skutecznością, którą ta sekcja się otworzyła, więc jej wariancja też się
rusza \dash{} tutaj w dół, o $\val{p34.judge.var.pct}$ procent. Działa to
przeciwko czynnikowi $(a-b)^{2}$, a nie z nim, i jest od niego dużo mniejsze.
Połowa efektu jest tożsamością; połowa szumowa to poprawka rzędu procenta.

To nie jest mały dodatek do budżetu. Nie ma go też w niczyim raporcie.
\end{fr}
...
Przy skuteczności $\num{0.715}$, której ten program używa, i tych dwóch
stopach daje to $\val{p34.judge.agree.pct}$ procent, czyli przyzwoitą
kartę.
...
\begin{fr}
\begin{ansblock}
Nie. Zgodność wiąże kombinację $p\,a - (1-p)\,b$ i nic więcej, więc
$\num{0.715}\,a - \num{0.285}\,b = \num{0.6365}$ jest wszystkim, co karta
mówi. Przebiegnij $a$ i $b$ po parach, które to spełniają, a tłumienie leży
w przedziale
...
B musi być o więcej niż trzecią bardziej zwięzły od A, żeby spłacić swoją
...
\begin{fr}
I cena, i długość należą do modelu, dlatego właśnie podanie jednej z nich
niczego nie załatwia: żadnej nie da się wydzielić z porównania i żadna nie
jest własnością samego zadania.

Program~\ref{prog:P29} spotkał tożsamość tego samego kształtu w innej
walucie, a sekcja~\ref{sec:P34-measures} stawia obie obok siebie.
...
Program~\ref{prog:P29} zmierzył dokładnie to: ten sam model na tym samym
tekście raportuje $\val{p29.bpc.a}$ i $\val{p29.bpc.b}$ bita na znak przy
$\val{p29.tpc.a}$ i $\val{p29.tpc.b}$ tokena na znak, czyli czynnik
$\val{p29.bpc.ratio}$, w którym model nie gra żadnej roli. To kontrast z
rachunkiem: tam oba czynniki należały do modelu i żaden się nie skracał.
\end{fr}
...
Przeskaluj liczbę z pięćdziesięciu pozycji jak $1/N$: $\val{p31.bias.n50}
\times 50$ podzielone przez jedną dwudziestą $\val{p34.probe.i}$ daje tę
liczbę. Cztery razy tyle pozycji, żeby zbić artefakt do jednej dwudziestej
tezy, na pomiarze, który rutynowo raportuje się z kilkudziesięciu.

Dwie rzeczy o samej liczbie. Prawo $1/N$ jest prawem, do którego estymator
zmierza, a nie takim, któremu jest posłuszny \dash{} własna para z
Programu~\ref{prog:P31}, $\val{p31.bias.n50}$ przy $50$ i
$\val{p31.bias.n100}$ przy $100$, spada odrobinę szybciej niż $1/N$, więc
skalowanie od mniejszej liczby jest jego stroną ostrożną. A w przeciwieństwie
do błędu losowego to obciążenie nie okrąża prawdy: idzie w jedną stronę, więc
nigdy nie uśrednia się po warstwach jednego artykułu.
\end{fr}
...
Zanim przeczytasz dalej: załóż, że książka techniczna ma liczbę, o którą
prosi jej własna tabela, a której nie potrafi ci podać. Wymień rzeczy, które
może z tym zrobić, i powiedz, która z nich zostawia cię z długiem.
\dotline
\nextframe
\end{fr}
...
\begin{enumerate}
\item \textbf{Zostawić pustą, bo eksperymentu nie przeprowadzono.} Dług
  \dash{} a książka jest ci winna to, co by go rozstrzygnęło, i ile to
  kosztuje.
\item \textbf{Zostawić pustą, bo liczba jest własnością twojej maszyny.}
  Nie dług. Tabela wypełniona przez kogoś innego byłaby czynnie myląca, a to,
  co tam należy, to metoda.
\item \textbf{Wydrukować ją i oznaczyć jako orientacyjną}, bo zmierzono ją,
  a rusza się z wyborem, którego nikt nie ustandaryzował.
\end{enumerate}
\end{ansblock}
...
\begin{summarybox}
\sumitem{2--3}{\result{Zbiór ewaluacyjny jest próbą, a wynik estymatorem, więc oba
  błędy z Programu~\ref{prog:P26} stosują się do niego: losowanie pozycji i
  dystans między benchmarkiem a twoim zadaniem}.}
\sumitem{4--6}{\result{Tylko pierwszy maleje z liczbą pozycji. Powyżej około
...
\sumitem{11--12}{\result{Przeważenie jednego ustalonego zbioru ewaluacyjnego o
...
\sumitem{13--14}{\result{Wynik jednego modelu przebiega $\val{p34.mix.swing}$ punktów
...
\sumitem{17--18}{\result{Precyzja idzie jak $1/\sqrt{n}$, więc połowienie marginesu
  to $\val{p34.halve.times}$ raza tyle pozycji, a zbicie czoła tabeli
  $\val{p27.models}$ równie dobrych modeli do punktu od jego własnej prawdy
  około $\val{p34.one.pt.n}$ z nich}.}
\sumitem{18--19}{\result{Oddzielenie dwóch modeli o punkt kosztuje po
  $\val{p27.n.rho00}$ pozycji na różnych promptach i $\val{p27.n.rho90}$ na
  tych samych, więc decyzja projektowa jest warta więcej niż budżet}:
  Program~\ref{prog:P27} podaje ten czynnik jako $1-\rho$, na liczbach z tej
  strony.}
\sumitem{21--22}{\result{Niedoskonały sędzia raportuje $(a-b)$ raza prawdziwą różnicę
  między dowolnymi dwoma modelami, dokładnie, a stała nie zależy od
  skuteczności żadnego z nich}.}
\sumitem{24--26}{\result{Dzieli to użyteczną liczbę pozycji przez $(a-b)^{2}$
  \dash{} szum rusza się o mniej więcej procent, a efekt o całość \dash{}
  a zgodność sędziego nie wyznacza $a-b$}: przy
  $\val{p34.judge.agree.pct}$ procentach biegnie ono od
  $\val{p34.judge.atten.lo}$ do $\val{p34.judge.atten.hi}$.}
\sumitem{27--28}{\result{Zgodność wiąże tłumienie tylko przy skuteczności połowy, a
  poprawka z Programu~\ref{prog:P28} do podanego przez sędziego
  prawdopodobieństwa zostawia tłumienie nietknięte}.}
\sumitem{30--31}{\result{Koszt na zadanie to cena za token razy tokeny na zadanie,
  więc model $\val{p34.bill.price.times}$ raza droższy za token może być o
  $\val{p34.bill.saving.pct}$ procent tańszy w utrzymaniu}.}
\sumitem{33--34}{\result{Bufor jest na token w locie, więc zwięźlejszy model obsługuje
  też $\val{p34.bill.conc.times}$ raza tyle rozmów na jednej karcie}.}
\sumitem{37--39}{\result{Bity na znak mierzą tokenizator, gdy tokenizatory się
  różnią, a wtykowa informacja wzajemna jest dodatnia na danych niezależnych} \dash{} $\val{p31.bias.n50}$ nata przy $50$ pozycjach.}
\sumitem{41--42}{\result{Wynik sondy to luźne ograniczenie dolne, więc słaba sonda nie
...
\sumitem{48--49}{Książka z liczbą, której nie potrafi podać, ma trzy ruchy i
  tylko jeden jest długiem: pusto, bo nie przeprowadzono; pusto, bo
  nieprzenoszalna; albo wydrukowana i oznaczona jako orientacyjna.}
...
\item Tabela w książce jest pusta, a książka mówi, że liczba zależy od twojego
  sprzętu. Czy to dług?
  \answerto{Nie. To drugi z trzech ruchów: liczby, która jest własnością
