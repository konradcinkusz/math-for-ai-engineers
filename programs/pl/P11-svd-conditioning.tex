...
\item Aproksymację rzędu $1$ odjęto od $A$. Co zostaje, w terminach wartości
  ...
  \answerto{Mierzone jako suma kwadratów: $\sigma_2^{2}$, jeśli usunięty
  kawałek był największy \dash{} każdy kawałek rzędu jeden wnosi kwadrat własnej
  wartości osobliwej i nic poza tym nie wnosi. Jej pierwiastek, $\sigma_2$, to
  ta sama odpowiedź w normie Frobeniusa.}
...
osobliwych. \textbf{$\lvert\det A\rvert$ jest iloczynem rozciągnięć} \dash{}
czyli to, co Program~\ref{prog:P09} nazywał objętością, powiedziane w języku
tego programu. Moduł nie jest tu ozdobą: oba obroty wnoszą znak i nic poza tym,
więc znak $\det A$ mówi, czy odwzorowanie odwraca przestrzeń, i nie niesie
żadnej informacji o rozmiarze.
\end{fr}
...
Twierdzenie z Programu~\ref{prog:P10} stosuje się tylko do macierzy
symetrycznych, a ten program jest o macierzach dowolnego kształtu. Powiedz,
dlaczego nie jest to luka.
\dotline
\nextframe
\end{fr}

\begin{fr}
\ans{Bo $A\T A$ jest symetryczna, jakiekolwiek było $A$. Wartości osobliwe
istnieją dla każdej macierzy, bo są pożyczone z macierzy symetrycznej, którą
zawsze da się zbudować.}
Zbudowane $A$ ma wartości osobliwe $\val{p11.sig.hi}$ i $\val{p11.sig.lo}$.
Jest kwadratowe, więc ma też własne wartości własne.

Warto rozdzielić obie konstrukcje, zanim odpowiesz. Wartości osobliwe wzięły
się z dwóch \emph{różnych} obrotów, po jednym z każdej strony, czego wymaga
macierz dowolnego kształtu; wartość własna pyta o kierunek, który macierz
zostawia w spokoju, a to pytanie dotyczy tylko jednej strony. Nic dotąd nie
mówi, że obie odpowiedzi muszą być powiązane.
...
$\val{p11.sig.hi}$ to różnica poniżej $5\%$, a wykres jednego opisany jako
drugie wyglądałby zupełnie rozsądnie.
\end{trapbox}
...
Mimo to implikują tę samą listę wartości osobliwych, a powód mieści się w
jednej linijce: jeśli $R\T R v = \lambda v$ przy $\lambda \ne 0$, to
$RR\T(Rv) = \lambda(Rv)$, więc każda niezerowa wartość własna jednej jest
niezerową wartością własną drugiej. \textbf{Większa macierz Grama niesie
dodatkowe zera}, po jednym na każdy wymiar, do którego macierz nie sięga.

Ich ślady zgadzają się na $\val{p11.rect.energy}$, co skrypt sprawdza. To jest
wspólne widmo pokazujące się jako suma, a nie powód tej zgodności \dash{} same
równe ślady niczego by nie dowiodły, bo $\operatorname{diag}(1, 3)$ i
$\operatorname{diag}(2, 2)$ mają ten sam ślad i różne wartości osobliwe.
\end{fr}
...
Konstrukcja ma nazwę i pułapkę w środku. $u_i v_i\T$ to kolumna razy wiersz,
czyli \textbf{iloczyn zewnętrzny} \dash{} to, czego według
Programu~\ref{prog:P06} aktualizacja niskiego rzędu nigdy nie powinna tworzyć.
Przenieś transpozycję na drugi wektor, a $a\T b$ jest iloczynem skalarnym,
czyli \emph{liczbą}. Jeden znak w innym miejscu, a po jednej stronie macierz,
po drugiej skalar. Tutaj dostępny jest zresztą tylko pierwszy: $u_i$ ma tyle
składowych, ile $A$ ma wierszy, a $v_i$ tyle, ile kolumn, więc dla prostokątnej
$A$ tych dwóch wektorów w ogóle nie da się sparować na odwrót.

Kawałki nie oddziałują: wektory $u$ są prostopadłe i wektory $v$ też.

I to właśnie czyni następne pytanie w ogóle odpowiadalnym. Gdy kawałki
zachodzą na siebie, nie da się powiedzieć, co wnosi którykolwiek z nich z
osobna \dash{} całość nie jest sumą części, a pytanie o udział jest pytaniem o
coś, czego nie ma. Kawałki prostopadłe to dokładnie ten przypadek, w którym
udział istnieje.

Przeczytaj więc $A$ jako jeden długi wektor i weź jego kwadrat długości, tak
jak zrobiłby to Program~\ref{prog:P05}. Na co rozkłada się ta suma?
\dotline
\nextframe
\end{fr}
...
Na zbudowanej macierzy z sekcji 1 obcięcie do rzędu $1$ zostawia
$\val{p11.ey.err2}$ mierzone w ten sposób \dash{} czyli $\sigma_2^{2}$, i
skrypt sprawdza to na ułamkach.

\textbf{Dwie miary, oddalone o jeden kwadrat, i przy tej zmianie jednostki
warto się zatrzymać.} Wielkość, którą dwie poprzednie ramki dodawały, jest sumą
kwadratów, więc obcięcie zostawia $\sigma_2^{2} = \val{p11.ey.err2}$. Jej
pierwiastek, $\val{p11.ey.err}$, to samo $\sigma_2$ \dash{} długość macierzy
błędu czytanej jako jeden długi wektor, która ma nazwę, \textbf{norma
Frobeniusa}, i to w tej mierze podane jest poniżej twierdzenie
Eckarta--Younga.

Błąd nie jest \emph{mniej więcej} $\sigma_2$ ani przez tę wielkość
ograniczony. W tej normie \emph{jest} $\sigma_2$.
...
Zauważ, że same liczby maleją wzdłuż tej listy stukrotnie, a ich kwadraty
dziesięciotysiąckrotnie, więc dwa sposoby ich sumowania dają zupełnie różne
odpowiedzi. Weź sumę kwadratów \dash{} te $\sigma_i^{2}$ sprzed czterech ramek,
bo to ona się rozkłada.

Jaki jej ułamek nadal masz?
\dotline
\nextframe
\end{fr}
...
Wejście przychodzi już obarczone błędem \dash{} z pomiaru, z czujnika albo z
ostatniego zaokrąglenia, które opisał Program~\ref{prog:P01}. Macierz
...
Tamten przypadek miał wskaźnik uwarunkowania rzędu miliona i kosztował sześć z
szesnastu cyfr. Uogólnij to: ile cyfr kosztuje wskaźnik $\kappa$?
\dotline
\nextframe
\end{fr}

\begin{fr}
\ans{\textbf{Mniej więcej $\logb{10}\kappa$ z nich.} Podwójna precyzja zaczyna
z jakimiś szesnastoma, więc wskaźnik $10^{16}$ nie zostawia nic, a wskaźnik
...
Przed tabelą: przewidz z grubsza, ile cyfr dziesiętnych dzieli obie
odpowiedzi.
\dotline
\nextframe
\end{fr}
...
\begin{fr}
\ans{Podniesienie $\kappa$ do kwadratu podwaja $\logb{10}\kappa$, więc równania
normalne powinny oddać mniej więcej drugie tyle cyfr, ile samo zadanie już
kosztuje.}
...
wiarygodne dla każdego, kto nie znał odpowiedzi wcześniej. Są błędne
...
Dwie tezy, obie z liczbami. Powiedz, która z nich rozstrzyga tę radę i
dlaczego druga rozstrzygnąć nie może.
\dotline
\nextframe
\end{fr}

\begin{fr}
\ans{Ta druga. Czynnik $\val{p09.cost.ratio}$ w szybkości to kompromis
...
obróć przez $U\T$, podziel przez każdą \emph{niezerową} wartość osobliwą, a
zerowe wyślij do zera, obróć przez $V$. Zapisane jako macierz nazywa się to
\textbf{pseudoodwrotnością}, $A^{+}$.

To zastrzeżenie jest całym powodem, dla którego ten obiekt istnieje. Zerowa
wartość osobliwa to kierunek, który $A$ spłaszcza, a spłaszczenia nie da się
odwrócić żadną regułą; odmowa odpowiedzi w tym kierunku jest tym, co pozwala
zdefiniować $A^{+}$ dla macierzy, która nie ma odwrotności.
...
Warto powiedzieć wprost, po co ona jest. Pseudoodwrotność nie jest sposobem na
odwrócenie macierzy, która odwrotności nie ma \dash{} nic nim nie jest \dash{}
jest nazwą odpowiedzi, którą Program~\ref{prog:P08} już wyprowadził, zapisaną
tak, by wystarczał jej jeden symbol. Nazwanie czegoś nie czyni tego możliwym;
czyni to cytowalnym.
...
% ============================================================
\begin{summarybox}
\sumitem{1--2}{\result{\textbf{Równania wektora własnego nie da się w ogóle zapisać dla
...
\sumitem{3--4}{\result{Naprawa żąda \textbf{dwóch} zestawów prostopadłych kierunków
  ...
  warunków}.}
\sumitem{5--6}{\result{Sprawdzone dokładnie na macierzy zbudowanej z \emph{dwóch}
...
\sumitem{7--8}{\result{$A\T A = V\Sigma^{2}V\T$, więc \textbf{wartości osobliwe są
...
\sumitem{9--10}{\result{\textbf{Wartości osobliwe to nie wartości własne}, a przykład
  dobrano tak, by pomyłka była zrozumiała: $\val{p11.lam.hi}$ i
  $\val{p11.lam.lo}$ wobec $\val{p11.sig.hi}$ i $\val{p11.sig.lo}$, różnica
  poniżej $5\%$, której żaden wykres nie pokaże. Różnią się też rodzajem \dash{}
  wartości osobliwe istnieją dla każdego kształtu, są zawsze rzeczywiste i
  nieujemne i nigdy nie brakuje im kierunków}.}
\sumitem{11}{\result{Macierz prostokątna ma \textbf{dwie} macierze Grama różnych
  rozmiarów, których ślady zgadzają się na $\val{p11.rect.energy}$ \dash{} co
  jest wspólnym niezerowym widmem pokazującym się jako suma, a nie powodem tej
  zgodności. Większa niesie dodatkowe zera, po jednym na każdy wymiar, do
  którego macierz nie sięga}.}
\sumitem{12--13}{\result{$A$ jest \textbf{sumą kawałków rzędu jeden}, po jednym na
...
\sumitem{14--15}{\result{Obcięcie do rzędu $r$ zostawia zatem \textbf{dokładnie}
  wyrzucone kawałki: $\val{p11.ey.err2}$ dla przykładowej macierzy, czyli
  $\sigma_2^{2}$, mierzone jako suma kwadratów zdefiniowana ramkę wcześniej.
  Jej pierwiastek, $\val{p11.ey.err}$, to $\sigma_2$ \dash{} norma Frobeniusa
  błędu i miara, w której podane jest twierdzenie Eckarta--Younga. Nie mniej
  więcej $\sigma_2$, nie ograniczone przez nią}.}
\sumitem{16--17}{\result{\textbf{Eckart--Young}: ze wszystkich macierzy rzędu $r$
  ...
  wyniesie.}
\sumitem{18--19}{\result{Dwie z sześciu wartości osobliwych niosące $\val{p11.spec.pct}\%$
...
\sumitem{20--21}{\result{Błąd przychodzi z wejściem i jest rozciągany jak wszystko inne,
...
\sumitem{22--23}{\result{\textbf{Tracisz mniej więcej $\logb{10}\kappa$ cyfr
...
\sumitem{24}{\result{Dla macierzy symetrycznej dodatnio określonej $\kappa$ jest
...
\sumitem{25--26}{\result{$A\T A$ ma \emph{kwadraty} wartości osobliwych, więc
...
\sumitem{27--29}{\result{Zmierzone na dopasowaniu stopnia $\val{p11.ls.deg}$ przez
  ...
  niż awaria}.}
\sumitem{30}{\result{Rada \enquote{nie twórz równań normalnych} to więc dwie tezy z
...
\sumitem{31--32}{\result{\textbf{Pseudoodwrotność} $A^{+}$ to obróć, podziel przez każdą
  \emph{niezerową} wartość osobliwą, a zerowe wyślij do zera, obróć \dash{}
  zgodna z $A^{-1}$ tam, gdzie ta istnieje, i dająca odpowiedź najmniejszych
  kwadratów tam, gdzie nie. Dzielenie przez
...
\sumitem{33}{\result{\textbf{Jeden rozkład odpowiada na pytania sześciu programów}: rząd
...
\item Obcięcie $A$ do rzędu $2$ zostawia błąd $1$. Co to mówi o $\sigma_3$ i co
  ...
  \answerto{W normie Frobeniusa błąd wynosi
  $\sqrt{\sigma_3^{2} + \sigma_4^{2} + \cdots}$, więc $\sigma_3 \le 1$, z
  równością dokładnie wtedy, gdy $\sigma_4$ i dalsze znikają \dash{} czyli gdy
  $A$ ma trzy wartości osobliwe. A na mocy Eckarta--Younga żadna inna macierz
  rzędu $2$ nie jest bliżej, więc $1$ nie jest błędem tej metody, tylko
...
\item Dlaczego macierz z zerową wartością osobliwą nie ma odwrotności, w języku
  tego programu?
  \answerto{Bo $\Sigma$ spłaszcza ten kierunek: odwrócenie odwzorowania tam
  oznaczałoby dzielenie przez zero, i dlatego odwrotność nie istnieje.
  Odwzorowanie nie jest różnowartościowe, co jest jądrem z
  Programu~\ref{prog:P08} i zerowym wyznacznikiem z Programu~\ref{prog:P09}.
  Zauważ, że $A^{+}$ \emph{nie} dzieli przez zero \dash{} wysyła ten kierunek
  do zera i odmawia odpowiedzi, i tak przeżywa tam, gdzie $A^{-1}$ nie może.}
\end{testexercises}
...
\item Pokaż, że $\kappa(A) \ge 1$ dla każdej macierzy, i powiedz, które macierze
  ...
  $\Sigma$ jest wielokrotnością identyczności, a $A$ macierzą ortogonalną
  przeskalowaną stałą \dash{} obrotem albo odbiciem, a dla prostokątnego $A$
  dowolną przeskalowaną izometrią \dash{} co rozciąga każdy kierunek jednakowo i
  nie może zniekształcić błędu względnego wcale.}
