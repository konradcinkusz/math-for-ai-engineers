...
\mermaidfig{p12-pairs-not-items}{Dlaczego liczą się pary i co dzieje się z ich
liczbą, gdy pokój rośnie.}{jeden element wobec reszty czy dowolne dwa}
\end{fr}
...
% ============================================================
\begin{summarybox}
\sumitem{1--2}{\result{\textbf{Dwa pytania rozstrzygają, która reguła obowiązuje}: czy
  ...
  ustalone}.}
\sumitem{3--4}{\result{Dwie z czterech liczb, dla $\val{p12.four.k}$ z
...
\sumitem{4--5}{\result{Reguła iloczynu potrzebuje \textbf{ustalonej liczby możliwości
  ...
  iloczynem}.}
\sumitem{6}{\result{$\binom{n}{k} = \val{p12.four.unordnorep}$ to wariacje podzielone
  ...
  $2! = 2$}.}
\sumitem{7--8}{\result{Czwarta komórka, bez kolejności i z powtórzeniami, to
...
\sumitem{9--10}{\result{$\val{p12.four.n}!$ to $\val{p12.fact.n}$ ustawień, a
...
\sumitem{17--20}{\result{\textbf{Rekurencja Pascala
  ...
  elementu}.}
\sumitem{21--23}{\result{\textbf{Włączenia i wyłączenia: znaki zmieniają się na
...
\sumitem{24--26}{\result{\textbf{Szufladki: $m$ elementów w $n$ szufladach wkłada co
...
\sumitem{27--28}{\result{\textbf{Rachunek urodzinowy liczy pary, a nie elementy.}
...
\sumitem{29--31}{\result{Dwa wzory, każdy dobry tam, gdzie drugi zawodzi: iloczyn
...
\sumitem{32--35}{\result{\textbf{Bezpieczna pojemność skrótu to mniej więcej
  ...
  $\val{p12.bits128}$ bitów}.}
\sumitem{36--38}{\result{$\val{p12.beam.vocab}^{\val{p12.beam.len}}$ to
...
\sumitem{39--43}{\result{\textbf{Wartość Shapleya uśrednia po każdej kolejności, a
...
\item Wiązka o szerokości $b$ przez $L$ kroków przy słowniku $V$ ocenia
  ...
  \answerto{$b = \num{0.01}\,V^{L}/(LV)$, czyli
  $\val{p12.beam.pct.width}$. Sensem zadania jest to, że ta liczba jest nie
  tyle wielka, co nieosiągalna, więc żadna szerokość nie jest argumentem z
  pokrycia.}
