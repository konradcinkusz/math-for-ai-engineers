...
\begin{fr}
Program~\ref{prog:F10} powiedział wprost, że graf to zbiór plus relacja i że
porządne powiedzenie tego należy tutaj. To jest ten program i w
przeciwieństwie do poprzedniego opiera się na Części~III: do sekcji 3 cały
temat zamieni się w mnożenie macierzy.
...
deduplikacja korpusu jest dokładnie zadaniem znalezienia składowych i
zostawienia po jednym dokumencie z każdej.
...
\begin{fr}
Lista wygrywa więc na pamięci, wygrywa na sąsiadach i remisuje na odczycie.
Macierz byłaby dziwną rzeczą do trzymania \dash{} poza jedną własnością, która
nie ma nic wspólnego z odczytywaniem czegokolwiek.
...
\[ X' = \sigma(AXW) \]
Czytaj trzy czynniki od prawej: $W$ miesza własne cechy wierzchołka, $A$
zastępuje wiersz każdego wierzchołka sumą wierszy jego sąsiadów, a $\sigma$
jest nieliniowością, bez której Program~\ref{prog:P06} pokazał, że złożenie by
się zapadło.
...
Bo $k$ warstw stosuje $A$ $k$ razy, a wzór niezerowych wyrazów $A^{k}$ to
dokładnie pary połączone spacerem długości $k$ \dash{} czyli twierdzenie
sprzed dwóch ramek, przybywające jako zdanie o architekturze.
...
krawędzi. Graf skierowany bez cykli jest \textbf{acykliczny}, a skierowany graf
acykliczny to \emph{DAG}.

Zastrzeżenie o powtarzaniu czyni to słowo używalnym i warto się przy nim
zatrzymać. Każdy graf nieskierowany mający choć jedną krawędź ma spacer, który
wychodzi z wierzchołka i wraca \dash{} owo \enquote{tam i z powrotem} sprzed
dwóch ramek \dash{} więc bez tego zastrzeżenia nic nie byłoby acykliczne, a
słowo nie znaczyłoby nic. W grafie skierowanym zastrzeżenie pracuje mniej, bo
przejście krawędzią z powrotem jest możliwe tylko wtedy, gdy taka krawędź
istnieje; \emph{to} jest powód, dla którego acykliczność orzeka się o grafach
skierowanych, a o nieskierowanych prawie nigdy.
...
osobnymi wierzchołkami. Dlatego sieć rekurencyjną da się w ogóle
zróżniczkować i dlatego jej pamięć rośnie z liczbą rozwiniętych kroków.
\end{trapbox}
\end{fr}
...
własnym i wszystko, co tamten program powiedział, stosuje się bez zmian
\dash{} w tym to, że znajdowanie go przez wielokrotne mnożenie jest metodą
potęgową i że tempo osiadania rządzi druga wartość własna.
...
jest dodatnia, o co prosi odpowiednie twierdzenie.
\end{note}
\end{fr}
...
nie kosztuje, a kupuje determinizm. \emph{Gdzie osiada uwaga} stało się
wektorem własnym.
...
% ============================================================
\begin{summarybox}
\sumitem{1--3}{\textbf{Graf to zbiór wierzchołków i zbiór ich par}, i nic
...
\sumitem{4--5}{\textbf{Stopnie sumują się do podwojonej liczby krawędzi}, bo
...
\sumitem{6--8}{Graf pełny o $\val{p13.complete.n}$ wierzchołkach ma
...
\sumitem{9--10}{\textbf{Macierz czy lista to pytanie o gęstość i o to, którą
...
\sumitem{11--13}{Macierz odpowiada na \emph{czy istnieje krawędź} w jednym
...
\sumitem{14--15}{\textbf{$(A^{k})_{ij}$ liczy spacery długości $k$ z $i$ do
  ...
  arytmetykę.}
\sumitem{16--18}{\textbf{Spacer to nie ścieżka}: $(A^{2})_{ii}$ to stopień
  $i$, liczący \enquote{tam i z powrotem}. Zliczanie ścieżek jest problemem
  trudniejszym i potęgi macierzy tego nie robią.}
\sumitem{19--20}{\textbf{Przekazywanie komunikatów jest mnożeniem przez
  macierz sąsiedztwa}, więc $k$ warstw sięga dokładnie $k$ skoków:
  ...
  pojemność.}}
\sumitem{21--23}{\textbf{Graf skierowany ma porządek topologiczny wtedy i
  tylko wtedy, gdy jest acykliczny}, a zwykle ma ich więcej niż jeden:
  $\val{p13.topo.count}$ tutaj spośród $\val{p13.topo.total}$.}
\sumitem{24--25}{\textbf{Każdy porządek topologiczny liczy te same wartości},
  ...
  zrównoleglać.}
\sumitem{26}{\textbf{Cykl czyni obliczenie nieokreślonym, a nie trudnym.}
  Rozerwij go albo rozwiń \dash{} czym jest sieć rekurencyjna i dlaczego jej
  pamięć rośnie z liczbą rozwiniętych kroków.}
\sumitem{27--30}{Krokiem błądzenia losowego jest $P\T p$, a
...
\sumitem{31--34}{\textbf{Wierzchołek bez krawędzi wychodzącej wysusza
