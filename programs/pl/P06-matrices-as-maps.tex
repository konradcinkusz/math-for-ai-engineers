...
jest.}{tabela i dwie reguły, których przestrzega}
...
Więc $AB$ jest macierzą złożenia \dash{} ale którego? Powiedz, czy robi
najpierw $A$, czy najpierw $B$, i dlaczego notacja czyta się tak, jak się
czyta.
\dotline
\nextframe
\end{fr}
...
\begin{fr}
\ans{Najpierw $B$. Czytaj $AB$ jako \emph{najpierw $B$, potem $A$}, od prawej
do lewej, tak jak czyta się $f(g(x))$ \dash{} notacja stawia funkcję na lewo od
tego, co ona zjada.}
...
stronę to inna funkcja.}{składanie dwóch odwzorowań w obu kolejnościach}
...
wymiarach \emph{wokół różnych osi} przemienne nie są \dash{} obróć książkę
najpierw okładką do góry, a potem na bok, następnie odwrotnie, a okładki wskażą
różne strony. Wokół jednej ustalonej osi są w trzech wymiarach przemienne
dokładnie tak jak na płaszczyźnie, i z tego samego powodu: obie kolejności
dodają te same dwa kąty. \textbf{Dwa
...
liczy $XW\T$ z $X$ o kształcie $k \times n$. Indeks górny to
\textbf{transpozycja}: $W\T$ to $W$ z zamienionymi wierszami i kolumnami, więc
$(W\T)_{ij} = W_{ji}$, a macierz $m \times n$ staje się $n \times m$. To to samo
złożenie z odwróconą konwencją, a Program~\ref{prog:P07} jest miejscem, w
którym osie dostają
...
Program~\ref{prog:F06} zapowiedział, że wagi stają się tutaj macierzą i że
argument o zwijaniu zostaje przeprowadzony jeszcze raz w więcej niż jednym
wymiarze. Weź więc dwie warstwy, $W_1 x + b_1$, potem
...
Jedna macierz i jeden wektor obciążeń. Argument przeżywa dokładnie.
...
Warto zauważyć, od czego to sprawdzenie nie zależy. Nic w tym przekształceniu
nie wymaga, żeby obciążenia były jakieś szczególne, żeby obie szerokości były
równe ani żeby któraś macierz była odwracalna: zwijanie jest własnością
\emph{postaci} $Wx + b$, a nie żadnej z występujących w niej liczb, i dlatego
jedna tożsamość obejmuje każdy stos, jaki ktokolwiek zbuduje.

Dwa kroki sprawiły, że to przekształcenie zadziałało. Jeden został nazwany w
sekcji 1. Powiedz, które dwa i skąd wziął się ten drugi.
\dotline
\nextframe
\end{fr}

\begin{fr}
\ans{$W_2$ rozdziela się względem sumy, a $W_2(W_1x)$ jest $(W_2W_1)x$, bo
iloczyn \emph{jest} złożeniem.}
Pierwsze to addytywność, jedna z dwóch własności, które czyniły tę funkcję
liniową. Drugie nie jest tą drugą: w tym przekształceniu nie pojawia się żaden
skalar. To definicja iloczynu z sekcji 2, czyli zdanie, na którym zbudowany
jest cały ten program, wykonujące pracę jeszcze raz.
Warto się przy tym zatrzymać, bo stos takich warstw jest dużym obiektem.

Osiem warstw \emph{afinicznych} o szerokości $\val{p06.stack.width}$ \dash{}
wagi i obciążenia, liczone tak, jak raportuje je framework \dash{} mieści
...
Powiedz więc, co kupuje głębokość w stosie samych warstw liniowych.
\dotline
\nextframe
\end{fr}

\begin{fr}
\ans{Nic. \textbf{Marnotrawstwem jest dokładnie ta głębokość, którą myślałeś,
że kupujesz.}}
...
musi. Para rozdzielona przez $\relu$ nie utrzymuje.
...
Powiedz więc, co wyklucza zagięta prosta i dlaczego jest to mocniejsze niż
nieznalezienie pojedynczego przekształcenia afinicznego, które pasuje.
\dotline
\nextframe
\end{fr}

\begin{fr}
\ans{Wszystkie przekształcenia afiniczne naraz. \textbf{Żadne pojedyncze
przekształcenie afiniczne jej nie odtworzy, jakiekolwiek by ono nie było}
\dash{} zagięcie prostej jest czymś, czego żadne z nich nie potrafi, więc nie
zostaje nic do przeszukania.}
\begin{aibox}
...
Jest po to, że złożenie bez niej jest dowodliwie marnotrawstwem $(k-1)/k$
parametrów \dash{} $\val{p06.stack.wasted.pct}\%$ przy ośmiu warstwach, połowy
przy dwóch \dash{} a wystarczy dowolna funkcja, która łamie liniowość.
Dlatego dziedzina mogła wymienić logistyczną na $\relu$, a
...
trzymaj chudy czynnik w środku i nigdy nie twórz iloczynu zewnętrznego $BC$.
...
% ============================================================
\begin{summarybox}
\sumitem{1--3}{\result{\textbf{Macierz jest funkcją, a nie prostokątem.} Bierze wektor
  ...
  jako macierz}.}
\sumitem{4--6}{\result{\textbf{\enquote{Liniowy} znaczy te dwie własności, a nie
...
\sumitem{7--8}{\result{\textbf{Przekształcenie liniowe jest wyznaczone przez to, co
...
\sumitem{9--11}{\result{\textbf{Iloczyn $AB$ jest złożeniem} \dash{} najpierw $B$,
  ...
  zapomni}.}
\sumitem{12--14}{\result{\textbf{Łączność jest darmowa}, bo składanie funkcji nigdy nie
...
\sumitem{15--17}{\result{\textbf{Kolejność ma znaczenie, a jeden wymiar nie może tego
...
\sumitem{17--19}{\result{Wśród $\val{p06.commute.trials}$ losowych par $3 \times 3$
  ...
  obroty płaszczyzny wokół jednego środka oraz obroty w trzech wymiarach wokół
  \emph{jednej} osi \dash{} a \textbf{obroty w trzech wymiarach wokół różnych
  osi nie są}, i dlatego dwa wymiary są przypadkiem mylącym}.}
\sumitem{20--22}{\result{\textbf{Kształt to zapisana dziedzina i przeciwdziedzina},
  ...
  jest}.}
\sumitem{23--25}{\result{Wektor jest macierzą $n \times 1$, więc $Ax$ nie potrzebuje
...
\sumitem{26--29}{\result{\textbf{Dwie warstwy afiniczne są jedną}: $W_2(W_1x + b_1) +
...
\sumitem{30--32}{\result{\textbf{Przekształcenie afiniczne nie potrafi zagiąć
...
\sumitem{33--34}{\result{\textbf{Łączność jest darmowa, a nawiasowanie nie}: ten sam
...
\item Zwiń $W_2(W_1x + b_1) + b_2$ do jednego przekształcenia afinicznego.
  ...
  obciążeń, przez rozdzielenie $W_2$ względem sumy, a potem odczytanie
  $W_2(W_1x)$ jako $(W_2W_1)x$, czyli iloczynu zdefiniowanego jako złożenie.}
