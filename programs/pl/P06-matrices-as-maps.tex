...
Skrypt potwierdza to z dokładnością $\val{p06.assoc.err}$ na losowych
macierzach, co warto zrobić raz z tego samego powodu, dla którego warto było
...
największa rozbieżność $\val{p06.collapse.err}$ \dash{} co jest zaokrągleniem,
a nie różnicą.
\end{fr}
...
$\val{p06.bend}$ odstępu, wobec $\val{p06.bend.affine}$ przez zwiniętą
macierz.
\end{ansblock}
...
\sumitem{12--14}{\textbf{Łączność jest darmowa}, bo składanie funkcji nigdy nie
  ...
  dokładnością $\val{p06.assoc.err}$, co warto zrobić raz, bo tożsamość
...
\sumitem{26--29}{\textbf{Dwie warstwy afiniczne są jedną}: $W_2(W_1x + b_1) +
  ...
  wejściach z dokładnością $\val{p06.collapse.err}$. $k$ warstw marnuje
...
\sumitem{30--32}{\textbf{Przekształcenie afiniczne nie potrafi zagiąć
  ...
  ($\val{p06.bend.affine}$) przez zwiniętą macierz i zagięte
