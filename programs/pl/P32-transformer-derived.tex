...
\item Zwykły łańcuch $\val{p32.depth}$ warstw ma gradient będący iloczynem
  jednego czynnika na warstwę, każdy najwyżej $\frac{1}{4}$. Co robi z tym
  dodanie połączenia rezydualnego do każdej warstwy? \teachesat{23--24}
...
\label{sec:P32-score}
\index{uwaga!wynik}
\index{uwaga!dzielnik sprawdzony@dzielnik $1/\sqrt{d_k}$, w bloku}
...
\mermaidfig{p32-score-assembled}{Skąd bierze się wynik}{%
  dwa rzutowania, które występują tylko jako iloczyn}
...
\mermaidfig{p32-where-parameters-are}{Gdzie siedzą parametry bloku}{%
  cztery i osiem}
