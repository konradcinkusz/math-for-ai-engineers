...
dostaje $\val{f12.sat.bound}$ \dash{} poniżej każdej podłogi, jaką ten format
ma. Wtedy wczesne warstwy dostają dokładnie zero i dostają je po cichu.
...
\sumitem{3--5}{Parametry uwagi to cztery rzutowania; wynik to jeden iloczyn
...
\sumitem{9--10}{Przeżywa, dokładnie. Wynik jest formą dwuliniową $x\T M y$ z
...
\sumitem{11--12}{Ale wytrenowany model ma jedno losowanie wag, a nie średnią,
...
\sumitem{13--16}{$\softmax$ czyni wyjście kombinacją wypukłą
  (Program~\ref{prog:P19}), więc warstwa czysto uwagowa nie może wyprodukować
  kierunku, którego jej wartości już nie rozpinają.}
\sumitem{18--20}{Uwaga wielogłowicowa jest darmowa: $\val{p32.heads}$ głowic
  szerokości $\val{p32.d.head}$ mieści dokładnie parametry jednej głowicy
  szerokości $\val{p32.d}$. Zmienia się rząd, a nie koszt.}
\sumitem{21--22}{Zwykły łańcuch $\val{p32.depth}$ warstw ma gradient co
  najwyżej $\val{p32.plain.bound}$, czyli poniżej tego, co pomieści
  $\code{float32}$.}
\sumitem{23--25}{$y = x + f(x)$ czyni jakobian $\prod(1 + f'_k)$, którego
...
\sumitem{28}{Obrót czyni wynik zależnym od odległości między dwiema pozycjami
  i od niczego więcej, a nic model nie kosztuje, bo jest ortogonalny
  (Programy~\ref{prog:F08} i \ref{prog:P09}).}
\sumitem{29--32}{Uwaga to $4d^{2}$, a warstwa gęsta to $8d^{2}$: \textbf{połowa,
...
\sumitem{33--35}{Cache to jeden klucz i jedna wartość na pozycję na warstwę,
