...
\begin{fr}
\ans{$x^{2} + 3$}
Tym razem to $x^{2}$ jest stałą mnożącą $y$, więc przeżywa jako ono samo, a
$3y$ różniczkuje się do $3$, więc drugi składnik też przeżywa.

\begin{trapbox}
Popatrz, co się stało z dwiema odpowiedziami.
...
powiedział, że pochodna jest zerem na dnie krzywej. Ile wynosi gradient w
punkcie, w którym $f$ ma minimum, i dlaczego \emph{każda} składowa musi
zniknąć?
\nextframe
\end{fr}
...
Tutaj $\vect{p}$ to punkt, w którym stoisz, zapisany jako wektor, tak że
$\vect{p} + hu$ oznacza krok długości $h$ od niego; od tego miejsca książka
zostawia tę literę dla punktu.
...
\begin{rigourbox}
\textbf{\enquote{Granica jest liniowa względem kierunku} to założenie, a nie
obserwacja.} Jest to znaczenie słowa \emph{różniczkowalna w punkcie} i to ono
wykonuje całą pracę w kroku powyżej. Samo istnienie obu pochodnych cząstkowych
nie wystarcza.

Standardowy kontrprzykład warto zobaczyć raz, bo jest paskudniejszy, niż
wygląda:
\[ f(x, y) = \frac{x\,y^{2}}{x^{2} + y^{4}}, \qquad f(0, 0) = 0 \]
Obie cząstkowe w początku układu są zerami, bo $f$ jest zerem wzdłuż każdej
osi. Iloczyn skalarny przewiduje więc tempo zero w każdym kierunku. Tempo
wzdłuż jednostkowego kierunku $(a, b)$ przy $a \neq 0$ wynosi w istocie
$b^{2}/a$, co skrypt sprawdza w trzech kierunkach \dash{} nieograniczone, gdy
$a$ maleje, i zerowe tylko wzdłuż osi $x$.

Każda funkcja, którą ta książka różniczkuje, jest tam różniczkowalna, więc
tożsamość obowiązuje wszędzie. Czym jest różniczkowalność i jak ją sprawdzić,
to pierwszy kurs analizy.
\end{rigourbox}

\transcript{p15-gradient}

\mermaidfig{p15-any-direction}{Jedna liczba na kierunek i skąd ona
pochodzi.}{tempo wzdłuż wektora jednostkowego}
\end{fr}
...
Jest mniejsza niż własna długość gradientu, $\val{p15.grad.len}$. To nie
przypadek i o tym jest następna sekcja.
...
dokładne, bo $\cos\theta$ nigdy nie przekracza $1$; oraz że próbkowany
najlepszy ustępuje o nie więcej niż rozdzielczość. Drugie zamienia niedobór z
zawstydzenia w wyjaśnienie.

Pierwsza połowa była kiedyś przypisywana nierówności
Programu~\ref{prog:P05}, a ta książka nigdzie takiej nierówności nie podaje.
Ma tożsamość powyżej i cosinus, który nie może przekroczyć jedynki, i tyle
temu argumentowi wystarczy.
\end{note}
\end{fr}
...
długiej niecce.}{dlaczego prostopadła mija środek}
\end{fr}

% ============================================================
\section{Co to robi z marszem}
\label{sec:P15-zigzag}
\index{metoda gradientu prostego!wyprowadzenie}
\index{gradient|seealso{metoda gradientu prostego}}
...
$(\val{p15.zig.start.x}, \val{p15.zig.start.y})$ na niecce, którą zapisuje
kolejna ramka, wartość wynosi $\val{p15.sign.before}$. Jeden krok o rozmiarze
$\val{p15.sign.eta}$ przeciwnie do gradientu sprowadza ją do
$\val{p15.sign.down}$. Jeden krok tego samego rozmiaru \emph{zgodnie} z
...
Zapisana, jest to
\[ f(x, y) = \tfrac{1}{2}\left(\val{p15.lam.hi}\,x^{2}
   + \val{p15.lam.lo}\,y^{2}\right),
   \qquad
   \nabla f = (\val{p15.lam.hi}\,x,\; \val{p15.lam.lo}\,y) \]
i każda liczba w tej sekcji bierze się z tych dwóch wyrażeń, więc czytelnik,
który chce jedną sprawdzić, ma do tego wszystko.

\textbf{Połówka nie jest ozdobą.} Sekcja~\ref{sec:P15-steepest} zapisała
funkcję kwadratową jako $ax^{2} + by^{2}$, której gradient to $(2ax, 2by)$
\dash{} tam współczynnik jest połową krzywizny i obie konwencje różnią się o
czynnik dwa w każdym gradiencie. Z połówką z przodu współczynniki
\emph{są} wartościami własnymi, gradient to każda wartość własna razy jej
własna współrzędna, a krok mnoży tę współrzędną przez $1 - \eta\lambda$, a nie
przez $1 - 2\eta\lambda$. W tej postaci napisana jest reszta tej sekcji i cały
Program~\ref{prog:P17}, i dlatego warto zapłacić tę połówkę.
...
osiami. Sekcja~\ref{sec:P15-steepest} rozstrzygnęła już, czy ujemny gradient
wskazuje tam minimum, więc warto zapytać o to drugie. Zanim cokolwiek
policzysz: jak bardzo jest obok \dash{} o stopień czy dwa, czy o coś, co
byłoby widać?
\nextframe
\end{fr}

\begin{fr}
\ans{$\val{p15.zig.angle}$ stopni od niego.}
...
\textbf{To nie błąd zaokrąglenia, to prawie połowa kąta prostego.} Sterowanie nie jest lekko nietrafione; wskazuje zupełnie gdzie
...
$\val{p15.zig.pathlen}$ drogi, żeby pokonać $\val{p15.zig.moved}$ terenu, więc
$\val{p15.zig.sideways.pct}$ procent marszu idzie w bok \dash{} co jest
ilorazem dwóch liczb powyżej, a skrypt sprawdza, że tak jest.
...
$1$ dla okrągłej niecki co do ostatniego bitu \dash{} przy kontroli puszczonej
dla $\lambda = \val{p15.lam.lo}$, tak że $1 - \eta\lambda$ jest dodatnie i
każdy krok ląduje przed środkiem, a nie za nim. Podnieś krok powyżej
$1/\lambda$, a marsz nadal jest zbieżny i nadal nie schodzi z prostej, ale
ląduje po drugiej stronie w każdym kroku, więc droga przestaje równać się
przemieszczeniu, choć kierunek jest tak samo dokładny.
\end{note}
\end{fr}
...
sumują się. To zdanie potrzebuje naprzemiennego znaku, który ta sekcja
właśnie zmierzyła, i dlatego ten program produkuje problem i zostawia go
stojącym.
\end{rigourbox}
\end{fr}
...
\sumitem{10--11}{\result{Zapisuje się go jako wektor \textbf{po to, by dało się go
  pomnożyć skalarnie}, i stąd bierze się reszta programu}.}
\sumitem{12--13}{\result{W minimum gradient jest wektorem zerowym, a \textbf{odwrotne
  zdanie zawodzi} \dash{} siodło też ma gradient zerowy, a odróżnienie ich
  należy do Programu~\ref{prog:P17}}.}
\sumitem{14--17}{\result{\textbf{Tempo zmiany wzdłuż kierunku jednostkowego $u$ to
...
\sumitem{20--21}{\result{Z cosinusem z Programu~\ref{prog:P05}
...
\sumitem{23--24}{\result{Przy $\cos\theta = 0$ tempo jest zerem, więc \textbf{gradient
...
\sumitem{27--28}{\result{\textbf{Prostopadłość do poziomicy to nie to samo co
  wskazywanie minimum}, chyba że poziomice są okręgami. Jedno jest lokalne, a
  drugie globalne}.}
\sumitem{29--30}{\result{Marsz to marsz Programu~\ref{prog:F11} z pochodną
  wektorową, a minus ma teraz mocniejszy powód: gradient jest najszybszym
  \emph{wzrostem}}.}
\sumitem{30}{\result{\textbf{Znak jest najczęstszym błędem w ręcznie pisanej pętli.}
...
\sumitem{31--32}{\result{Na niecce Programu~\ref{prog:P10}, o wartościach własnych
  ...
  dokładnie}.}
\sumitem{33--34}{\result{Marsz przechodzi w poprzek doliny $\val{p15.zig.crossings}$
  razy w $\val{p15.zig.steps}$ krokach i \textbf{przebywa
  $\val{p15.zig.ratio}$ raza dalej, niż pokonuje}}.}
\sumitem{35--36}{\result{Każda współrzędna mnożona jest przez $1 - \eta\lambda$:
...
\sumitem{37--38}{\result{\textbf{Ani jeden krok argumentu nie skorzystał z tego, że
...
\item W niecce o poziomicach będących okręgami ile razy spadek gradientu
  przechodzi z jednej strony na drugą, zanim dotrze \dash{} i od czego zależy
  twoja odpowiedź?
  \answerto{Ani razu, o ile $\eta < 1/\lambda$, tak że $1 - \eta\lambda$ jest
  dodatnie. Ujemny gradient wskazuje środek dokładnie, więc każdy krok idzie
  wzdłuż tej samej prostej, a droga wynosi $1$ raza własne przemieszczenie
  \dash{} co skrypt orzeka dokładnie, przy kroku tego rozmiaru.
  \textbf{Nie jest to prawdą dla każdego zbieżnego kroku.} Między $1/\lambda$
  a $2/\lambda$ marsz jest zbieżny i nadal nie schodzi z prostej, ale każdy
  krok ląduje za środkiem, więc przechodzi w każdym z nich. Przechodzenie
  doliny \emph{w bok} to co innego, wymaga poziomic nieokrągłych i to cała
  treść sekcji 5.}
\end{testexercises}
...
\item Masz nieckę o wartościach własnych $\lambda$ i $1$ i podwajasz
  ...
  pojedynczy kąt byłby faktem o jednej niecce.

  \textbf{Nie rośnie bez ograniczeń, a sufitem nie jest kąt prosty.} Z tego
  punktu startowego wspina się ku $45$ stopniom, czyli kątowi między
  $(\lambda, 1)$ a $(1, 1)$, gdy $\lambda$ ucieka, i skrypt orzeka też to. Na
  każdej niecce wypukłej prawdą jest, że kąt pozostaje \emph{ostry}, bo
  $\nabla f \cdot \vect{p} = 2f$, co jest dodatnie wszędzie poza minimum.}
\end{furtherproblems}
