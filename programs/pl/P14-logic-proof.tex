...
\sumitem{17--18}{\result{\textbf{Uniwersalna aproksymacja jest dla-każdego-a-potem-
  istnieje}, więc sieć może zależeć od funkcji, tolerancji i ustalonej
  aktywacji. Milczy o
...
\sumitem{28--29}{\result{\textbf{Ograniczenie na $\val{p14.conf}$ procent użyte
  $\val{p14.uses}$ razy gwarantuje wszystkie tylko w $\val{p14.allhold}$
  procentach przypadków}, gwarantuje nie więcej niż rzut monetą po
  $\val{p14.flip}$ użyciach i potrzebowałoby $\val{p14.needed}$ procent na
  każde, żeby sięgnąć $\val{p14.joint}$ procent przy $\val{p14.uses}$}.}
