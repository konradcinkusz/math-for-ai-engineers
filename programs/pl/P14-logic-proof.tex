...
\item Ograniczenie zachodzi \enquote{z prawdopodobieństwem co najmniej
  $\val{p14.conf}$ procent}. Używasz go $\val{p14.uses}$ razy, na niezależnych
  losowaniach. Co ta gwarancja daje dla wszystkich naraz? \teachesat{28--29}
  \answerto{Co najmniej $\val{p14.allhold}$ procent i nic lepszego: zostawia
  miejsce na porażkę gdzieś w $\val{p14.anyfails}$ procentach przypadków.
  Pewność podana raz nie jest pewnością wniosku, który budujesz z dwudziestu
  takich.}
...
\item Zdanie sprawdzono na czterdziestu kolejnych wejściach i za każdym razem
  ...
  $n = \val{p14.euler.first}$, więc czterdzieści potwierdzeń może być
  warte zero.}
\item Twierdzenie o zbieżności zakłada wypukłość funkcji celu. Artykuł cytuje
  ...
  twierdzenie w ogóle nie ma zastosowania.}
\end{quiz}
...
\mermaidfig{p14-parts-of-a-theorem}{Trzy części i ta, którą się gubi.}{%
  założenia, teza, kwantyfikatory}
\end{fr}
...
prawdziwą pracę. Krawędź nieskierowana wskazuje w obie strony, więc każdy z
jej końców musiałby stać po drugim i żaden porządek nie istnieje \dash{} a
jednak, według definicji, którą podaje Program~\ref{prog:P13}, ten graf nie ma
żadnego cyklu, bo marszruta tam i z powrotem powtarza swoją jedyną krawędź.
Acykliczny i bez porządku topologicznego: więc bez tego słowa twierdzenie jest
fałszywe, a cała sekcja 4 Programu~\ref{prog:P13} paruje. Jest to zarazem
słowo, które czytelnik najchętniej pominie, bo stoi przed rzeczownikiem i
czyta się jak dekoracja.
\end{fr}
...
zapisywana $P \Rightarrow Q$ i czytana \emph{$P$ implikuje $Q$}.
...
zdanie, a drugie to inne.}{odwrotne wobec przeciwstawnego}
\end{fr}
...
\begin{note}
Program~\ref{prog:F10} zmierzył negację przepchniętą przez niewłaściwy
spójnik w filtrze, czyli ten sam ruch o jeden spójnik dalej: z
...
\begin{quote}
Dla każdej funkcji ciągłej $f$ na obszarze domkniętym i ograniczonym,
każdej tolerancji $\varepsilon > 0$ i dowolnej ustalonej aktywacji, która nie
jest wielomianem, istnieje sieć z jedną warstwą ukrytą używająca tej
aktywacji, której wyjście jest w odległości mniejszej niż $\varepsilon$ od $f$
wszędzie na tym obszarze.
\end{quote}
...
\begin{fr}
\begin{ansblock}
Dla-każdego, a potem istnieje. Sieć może więc zależeć od $f$, od
$\varepsilon$ i od ustalonej aktywacji \dash{} i zależy.
\end{ansblock}
...
o czym milczy.}{o czym milczy twierdzenie o istnieniu}
\end{fr}
...
stopnia też przybliża dowolną funkcję ciągłą na obszarze domkniętym i
ograniczonym, i nikt
nie wnioskuje stąd, że wielomiany są dobrą klasą modeli. O sprawie
rozstrzyga wszystko to, o czym twierdzenie milczy.
\end{trapbox}

Dwie z hipotez znikają częściej niż kwantyfikator i obie łatwo obalić.
\emph{Domkniętość} pracuje: funkcja ciągła na przedziale z odjętymi końcami
wcale nie musi być ograniczona, a $1/x$ między $0$ a $1$ przyjmuje każdą
wartość powyżej $1$, więc żadna sieć żadnego rozmiaru nie zbliży się do niej
na żadne $\varepsilon$. \emph{Brak wielomianu} pracuje tak samo: przy
aktywacji identycznościowej sieć z jedną warstwą ukrytą jest afiniczna
względem wejścia, najlepszym przybliżeniem afinicznym $x^{2}$ na
$\intcc{0}{1}$ jest $x - \frac{1}{8}$, a jego najgorszy błąd wynosi dokładnie
$\frac{1}{8}$, więc $\varepsilon = \num{0.1}$ jest nieosiągalne niezależnie od
tego, ile jednostek dodasz. Żadna z nich nie jest szczegółem.
\emph{Z czego składa się twierdzenie} policzyło hipotezy jako jedną z trzech
części, więc zdanie cytowane bez nich jest innym zdaniem.
...
\begin{trapbox}
\textbf{Jest pierwsze dla $\val{p14.euler.first}$ kolejnych wartości od
$n = 0$, a złożone w $n = \val{p14.euler.first}$}, gdzie wynosi
...
losowania są niezależne, co ta gwarancja daje dla wszystkich
$\val{p14.uses}$ naraz?
\dotline
\nextframe
\end{fr}
...
\begin{fr}
\ans{Co najmniej $\val{p14.allhold}$ procent}
Przesłanka jest dolną granicą, a nie prawdopodobieństwem, więc iloczyn też
jest dolną granicą i gwarancja nie daje nic lepszego. Czytana w drugą stronę
zostawia miejsce na to, by przynajmniej jedno z $\val{p14.uses}$ twierdzeń
zawodziło nawet w $\val{p14.anyfails}$ procentach przypadków: \textbf{gwarancja
dopuszcza fałszywe zdanie wśród nich częściej niż nie}, a nic w artykule nie
zaznacza które.
...
\begin{tabular}{ll}
\toprule
użyć, zanim gwarancja staje się rzutem monetą & $\val{p14.flip}$ \\
pewność na użycie dla $\val{p14.joint}$ procent przy $\val{p14.uses}$
  & $\val{p14.needed}$ procent \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Ograniczenie na $\val{p14.conf}$ procent gwarantuje nie więcej niż
rzut monetą po $\val{p14.flip}$ użyciach}, a żeby zagwarantować
$\val{p14.uses}$ z nich razem na $\val{p14.joint}$ procent, każde musiałoby
zachodzić na $\val{p14.needed}$ procent \dash{} czyli prawdopodobieństwo
porażki $\val{p14.neededdelta}$ procent zamiast $5$, prawie dwadzieścia razy
ciaśniej.
...
% ============================================================
\begin{summarybox}
\sumitem{1--2}{\result{\textbf{Twierdzenie to założenia, teza i wiążące je
...
\sumitem{3--4}{\result{\enquote{Wtedy i tylko wtedy} to \textbf{dwa} twierdzenia
...
\sumitem{5--6}{\result{\textbf{Najczęstszym nadużyciem jest zatrzymanie tezy i
...
\sumitem{8--9}{\result{\textbf{$P \Rightarrow Q$ jest fałszywe w dokładnie jednym z
  $\val{p14.rows}$ wierszy} \dash{} $P$ prawdziwe i $Q$ fałszywe. Nie mówi nic
  o przyczynie i nic o przypadku, w którym $P$ zawodzi}.}
\sumitem{9--10}{\result{\textbf{Zdanie odwrotne jest innym twierdzeniem}, niezgodnym w
  ...
  przeuczamy}}.}
\sumitem{10--11}{\result{\textbf{A $\lnot P$ nie mówi nic o $Q$.} Wywnioskowanie
  ...
  przepuszcza}.}
\sumitem{11--12}{\result{\textbf{Zdanie przeciwstawne jest tym samym twierdzeniem}, we
  wszystkich $\val{p14.rows}$ wierszach, i dlatego jego dowód dowodzi
  oryginału}.}
\sumitem{13--14}{\result{\textbf{Kontrprzykład to jeden przypadek, w którym założenie
...
\sumitem{15--16}{\result{\textbf{Kolejność kwantyfikatorów zmienia twierdzenie.}
...
\sumitem{17--18}{\result{\textbf{Uniwersalna aproksymacja jest
  dla-każdego-a-potem-istnieje}, więc sieć może zależeć od funkcji, tolerancji
  i ustalonej
  aktywacji. Milczy o
  $\val{p14.silent}$ rzeczach, w tym o tym, jak duża musi być sieć i czy
  uczenie ją znajduje}.}
\sumitem{19}{\result{\enquote{Sieci potrafią przybliżyć dowolną funkcję} traktuje
  twierdzenie o istnieniu jak konstrukcję. Wielomian wysokiego stopnia robi to
  samo i nikt z tego nie argumentuje}.}
\sumitem{20--21}{\result{Trzy kształty do rozpoznania: wprost, nie wprost, indukcyjny.
  \textbf{Zaprzeczeniem implikacji jest $P \wedge \lnot Q$}, dlatego dowód nie
  wprost implikacji zawsze tam się zaczyna}.}
\sumitem{22--23}{\result{\textbf{Sprawdzenie każdego przypadku jest dowodem;
...
\sumitem{24--25}{\result{\textbf{Każdy kształt psuje się w swoim miejscu}: dowód
  ...
  szczegółach}.}
\sumitem{26--27}{\result{\textbf{Twierdzenie o zbieżności zakładające wypukłość nie
...
\sumitem{28--29}{\result{\textbf{Ograniczenie na $\val{p14.conf}$ procent użyte
  $\val{p14.uses}$ razy gwarantuje wszystkie naraz na nie więcej niż
  $\val{p14.allhold}$ procent}, spada do rzutu monetą po $\val{p14.flip}$
  użyciach i potrzebowałoby $\val{p14.needed}$ procent na każde, żeby sięgnąć
  $\val{p14.joint}$ procent przy $\val{p14.uses}$}.}
\sumitem{30--31}{\result{Trzy błędne cytaty to po jednym z każdej części: zgubione
...
\item \enquote{Istnieje krok uczenia działający dla każdej architektury}
  oraz \enquote{dla każdej architektury istnieje działający krok uczenia}.
  Które wolałbyś mieć, a które jest prawdopodobne?
