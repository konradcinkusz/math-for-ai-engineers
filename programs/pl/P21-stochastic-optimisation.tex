...
Rozpiętość $\val{p21.pop.span}$ wokół wielkości $\val{p21.pop.mean}$, a
$\val{p21.pop.wrong}$ ze $\val{p21.pop.subsets}$ wskazują w przeciwną stronę;
kolejne $\val{p21.pop.nodir}$ nie wskazują nigdzie.
\end{fr}
...
partii, którą masz przed sobą, a tutaj partie otaczają odpowiedź z obu stron,
a w przeciwną stronę wskazuje $\val{p21.pop.wrong.pct}$ procent z nich.
...
szumowi w każdej aktualizacji urosnąć o $\sqrt{2}$ \emph{i} podwaja teren
...
\begin{summarybox}
\sumitem{1--2}{\result{Gradient z partii jest \textbf{oszacowaniem} gradientu,
którego
  chciałeś. Szum jest metodą, a nie wadą w niej}.}
\sumitem{3--4}{\result{\emph{Nieobciążoność} znaczy, że średnia po każdej
partii, jaka
...
\sumitem{5--6}{\result{Nie mówi nic o jednym losowaniu: średnie z partii
biegną od
  $\val{p21.pop.lo}$ do $\val{p21.pop.hi}$ wokół $\val{p21.pop.mean}$, a
  $\val{p21.pop.wrong}$ ze $\val{p21.pop.subsets}$ wskazują w przeciwną
  stronę}.}
...
\sumitem{12}{\result{Rozrzut kroku to $\eta$ razy rozrzut oszacowania, więc
$\eta$ i
  $B$ to dwie liczby opisujące jedną wielkość}.}
\sumitem{13--14}{\result{Ogromny krok zdarza się, bo średnia z partii ma
rozrzut. Nic
  nie jest nie tak z modelem, danymi ani optymalizatorem, gdy się zdarza}.}
\sumitem{15--17}{\result{Próg obcinania to decyzja o rozkładzie: na połowie
typowego
...
\sumitem{20--21}{\result{Skalowanie $\eta$ przez $\sqrt{k}$ utrzymuje tę
wariancję
  ...
  rozstrzygnięte}.}
\sumitem{22}{\result{Zostawienie $\eta$ przy zmianie $B$ nie utrzymuje żadnej
z nich
  stałej, i dlatego jest jedynym wyborem, za którym nikt nie argumentuje}.}
\sumitem{23--24}{\result{Wygładzona krzywa straty opóźnia się o swój czas
połowicznego
  ...
  wielkości zmiany}.}
\sumitem{25}{\result{Czytaj surową krzywą, gdy pytaniem jest \emph{kiedy}, a
wygładzoną,
...
\sumitem{36--38}{\result{Po $\val{p21.grad.samples}$ próbkach oszacowanie
funkcji
