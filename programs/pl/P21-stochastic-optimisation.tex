...
\begin{summarybox}
\sumitem{1--2}{Gradient z partii jest \textbf{oszacowaniem} gradientu, którego
  chciałeś. Szum jest metodą, a nie wadą w niej.}
\sumitem{3--4}{\emph{Nieobciążoność} znaczy, że średnia po każdej partii, jaka
...
\sumitem{5--6}{Nie mówi nic o jednym losowaniu: średnie z partii biegną od
...
\sumitem{8--9}{Wariancja średniej z partii spada jak $1/B$, zmierzone z
  dokładnością do $\val{p21.noise.tol}$ procent dla $B$ od
  $\val{p21.noise.b.lo}$ do $\val{p21.noise.b.hi}$.}
\sumitem{10--11}{\textbf{Rozrzut spada jak $1/\sqrt{B}$}, więc
...
\sumitem{12}{Rozrzut kroku to $\eta$ razy rozrzut oszacowania, więc $\eta$ i
  $B$ to dwie liczby opisujące jedną wielkość.}
\sumitem{13--14}{Ogromny krok zdarza się, bo średnia z partii ma rozrzut. Nic
  nie jest nie tak z modelem, danymi ani optymalizatorem, gdy się zdarza.}
\sumitem{15--17}{Próg obcinania to decyzja o rozkładzie: na połowie typowego
...
\sumitem{18--19}{Wariancja aktualizacji to $\eta^{2}\sigma^{2}/B$, co
  rozstrzyga pytanie o skalowanie mechanicznie.}
\sumitem{20--21}{Skalowanie $\eta$ przez $\sqrt{k}$ utrzymuje tę wariancję
  ...
  rozstrzygnięte.}
\sumitem{22}{Zostawienie $\eta$ przy zmianie $B$ nie utrzymuje żadnej z nich
  stałej, i dlatego jest jedynym wyborem, za którym nikt nie argumentuje.}
\sumitem{23--24}{Wygładzona krzywa straty opóźnia się o swój czas połowicznego
  ...
  wielkości zmiany.}
\sumitem{25}{Czytaj surową krzywą, gdy pytaniem jest \emph{kiedy}, a wygładzoną,
  gdy jest nim \emph{czy}.}
\sumitem{26--27}{Akumulacja mikropartii o różnych liczbach tokenów daje
...
\sumitem{36--38}{Po $\val{p21.grad.samples}$ próbkach oszacowanie funkcji
