...
\outcome{23--30}{stosować regułę rozgłaszania dokładnie i rozpoznawać
  kształty, które zestawiają się w sposób, o który nikt nie prosił}
...
\begin{rigourbox}
Pojawiają się tu dwa obiekty, które porządnie definiuje
Program~\ref{prog:P24}, a argument potrzebuje jednego zdania o każdym z nich.
Przy każdej średniej branej po $n$ przykładach,
...
Wszędzie używana jest konwencja $\frac{1}{n}$, bo to ona czyni powyższą
tożsamość dokładną; Program~\ref{prog:P26} mówi, dlaczego czasem chce się
dzielnika $n - 1$, a tutaj nic poza poniższą arytmetyką od tego wyboru nie
zależy.
\end{rigourbox}
...
$1, 2, 3$ w powyższej konwencji $\frac{1}{n}$ \dash{} średnią z $1, 4, 9$ mniej
kwadrat $2$, czyli $\frac{2}{3}$.
...
bufor, którego kolejność pasuje do kształtu \dash{} a to jest osobne działanie
z własną nazwą. Program~\ref{prog:P03} wycenił przebieg przez pamięć; tutaj
dowiadujesz się, że o niego poprosiłeś.

\begin{warning}
\textbf{Zwrot \enquote{w typowym przypadku} niesie tu ciężar, a wyjątek jest
tym, który spotkasz.} Reshape może oddać ten sam bufor tylko wtedy, gdy osie,
które scala, już sąsiadują i są w kolejności bufora. Zrób reshape
\emph{spermutowanej} tablicy przez osie, które nie są, a biblioteka musi
skopiować \dash{} albo, w tej surowej, odmawia i każe najpierw poprosić o
spójność.

To nie jest przypadek brzegowy. To scalanie z powrotem z głów, $(b, h, s, d)
\rightarrow (b, s, h \times d)$, które w drodze wyjściowej wykonuje każda
implementacja uwagi, i jedyny w tej sekcji przebieg przez pamięć, o który nikt
nie prosi.
\end{warning}
...
to, czego żaden z nich nie kosztuje.}{jeden ciąg liczb, dwa sposoby odczytu}
...
\sumitem{30--32}{\result{\code{keepdims} zostawia oś długości $1$ na miejscu, żeby
  ...
  \textbf{żadne z reshape, transpozycji i permute nie przesuwa liczby, dopóki
  osie zostają w kolejności bufora}: każde oddaje ten sam bufor z inną regułą
  czytania, a dane przesuwa dopiero żądanie wyniku spójnego w pamięci \dash{}
  albo reshape przez osie, które permute wyprowadził z kolejności, który musi
  kopiować i jest scalaniem z powrotem z głów}.}
...
\item Powiedz, które z reshape, transpozycji i permute przesuwa liczby w
  ...
  \answerto{Żadne, dopóki osie zostają w kolejności bufora: każde zwraca ten
  sam bufor z inną regułą czytania. Kopiuje żądanie wyniku spójnego w pamięci.
  Kopiuje też reshape, którego scalane osie permute wyprowadził z kolejności
  \dash{} czyli scalanie z powrotem z głów, i jedyna kopia, której nikt nie
  zapisuje.}
