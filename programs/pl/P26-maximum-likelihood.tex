...
\item Weź pochodną logarytmu prawdopodobieństwa po parametrze i uśrednij ją po
  danych, o których sam model mówi, że je zobaczysz. Co dostajesz?
  \teachesat{42--43}
  \answerto{Dokładnie zero. Prawdopodobieństwa sumują się do jedynki bez
  względu na parametr, więc ich pochodna sumuje się do niczego \dash{} i
  właśnie dlatego przyrównanie sumy tej pochodnej do zera jest metodą
  największej wiarogodności.}
\item Potrzebujesz gradientu średniej po losowaniu, a losowania nie da się
  zróżniczkować. Co różniczkujesz zamiast niego? \teachesat{44--45}
  \answerto{Prawdopodobieństwo. Wartość oczekiwana to suma
  $p(x \mid \theta) f(x)$, a $p$ jest różniczkowalne tam, gdzie losowanie nie
  jest, więc gradient wychodzi jako wartość oczekiwana i da się go oszacować
  przez próbkowanie.}
\end{quiz}
...
\begin{fr}
Program~\ref{prog:P21} rozebrał gradient z partii i ustalił o nim jedną
rzecz: uśredniony po każdej partii, jaka mogła zostać wylosowana, jest tym
...
Wszystko w tej sekcji jest dokładne. Weź populację złożoną z
...
Przeczytaj to, zanim pójdziesz dalej: czy $c = 1$ \dash{} wybór
...
\index{estymator!poprawka@poprawka $n-1$}
...
To jest obciążenie, więc ma swój rozmiar. Zgadnij czynnik, zanim pójdziesz
dalej.
\dotline
\nextframe
\end{fr}
...
Dwie rzeczy o tej tabeli warto mieć. Każda próba rozmiaru $n$ została
wyliczona, a nie wylosowana, więc nic nie zostało pominięte i środkowy
wiersz jest dowodem o tej populacji, a nie świadectwem na jej temat. A
\emph{dokładnie} to twierdzenie o dokładnym zerze, więc rachunek idzie na
ułamkach, bez żadnej tolerancji: porównanie z małym progiem zamieniłoby
zdanie o równości na zdanie o tym, jak małą różnicę byłeś gotów przeoczyć.
...
Daje nieobciążoną \emph{wariancję}. Żadna flaga \code{ddof} nie daje
nieobciążonego odchylenia standardowego i żadna nie może, bo poprawka zależy
od rozkładu, a nie od liczebności próby.
...
$\val{p26.sd.short.5}$ procent na populacji z tego programu, i to za mała w tę
stronę, która sprawia, że wyniki wyglądają na powtarzalniejsze, niż są. Sama
liczba zmienia się z rozkładem \dash{} i właśnie dlatego żadna flaga nie może
tego poprawić \dash{} ale kierunek nie.
\end{trapbox}
\end{fr}
...
jest przepisem Programu~\ref{prog:F11} na maksimum. Spróbuj. Dwadzieścia prób
...
próby, była oceną największej wiarogodności}, a żaden wcześniejszy program tej
książki nie musiał tego powiedzieć.
\end{fr}
...
parametrów \dash{} \emph{gęstością}, gdy dane są ciągłe, co jest
rozróżnieniem Programu~\ref{prog:F13} i powodem, dla którego wiarogodność
większa od jedynki wypisana przez bibliotekę nie jest błędem. Nigdy nie jest
to prawdopodobieństwo parametrów. Metoda największej wiarogodności każe więc
maksymalizować to po $\theta$.

Zrób teraz to, co kazała robić \S\ref{sec:P26-mle}, i jeszcze dwie rzeczy.
Weź logarytm, który zamienia to w sumę, i maksymalizuj. Potem, skoro
optymalizatory minimalizują, zmień znak. Potem podziel przez $n$, żeby
sekwencje różnej długości były porównywalne.
...
\begin{fr}
Pierwsza. \textbf{Label smoothing} kładzie $\varepsilon/K$ na każdej z $K$
klas i dodaje pozostałe $1-\varepsilon$ do zaobserwowanej, więc zaobserwowany
token niesie $1 - \varepsilon + \varepsilon/K$, a cel sumuje się do jedynki.
Zwykle wprowadza się je jako sztuczkę regularyzacyjną.

Przeczytaj ten podział uważnie, bo w piśmiennictwie bywa podawany na dwa
sposoby, a tylko jeden z nich sumuje się do jedynki. Nie różnią się o włos:
przy liczbach z tej sekcji rozjeżdżają się na drugim miejscu po przecinku
w celu zaobserwowanego tokena.
...
\index{weight decay!lambda@czym jest jego $\lambda$}
...
Zwróć uwagę, co ten podział rozstrzyga, a czego nie. Pierwszy składnik jest
jedynym, który widzi dane, więc rozkładu a priori nie da się dopasować do
danych, do których się go stosuje; a drugi w ogóle od danych nie zależy, więc
jego rozmiar jest ustalony, zanim przyjdzie choćby jeden przykład. Wybór kary
jest wyborem przekonania, a funkcja celu trzyma te dwie rzeczy osobno,
niezależnie od tego, czy ktoś to powie.

Nałóż teraz na każdą wagę gaussowski rozkład a priori o szerokości $\tau$,
wyśrodkowany na zerze, i powiedz, jaki jest dodany składnik.
\dotline
\nextframe
\end{fr}
...
\sumitem{1--4}{\result{Estymator ma \textbf{dwa} sposoby bycia w błędzie.
  ...
  \emph{wariancja} to odległość ocen od ich własnej średniej.} Program~\ref{prog:P21} dowiódł pierwszego o gradiencie z partii i
...
\sumitem{13--16}{\result{\textbf{Nie poprawia odchylenia standardowego.} Pierwiastek
  ...
  $n = 2$ i wciąż o $\val{p26.sd.short.5}$ procent przy $n = 5$ na populacji z
  tego programu, czyli przy
...
\item Dwa zespoły używają weight decay o sile $\num{0.01}$ i $\num{1.0}$ na
  ...
  Drugie jest o wiele mocniejsze \dash{} stukrotnie większe $\lambda$ to
  rozkład a priori węższy dziesięciokrotnie, więc twierdzi znacznie pewniej,
