...
\index{dywergencja KL|see{dywergencja Kullbacka--Leiblera}}
...
A teraz: ile co najmniej może \emph{kiedykolwiek} kosztować symbol z tego
źródła, gdyby długość słowa kodowego nie musiała być całkowitą liczbą bitów
\dash{} i gdzie w Programie~\ref{prog:P29} ta liczba już stoi zapisana?
\dotline
\nextframe
\end{fr}
...
Program~\ref{prog:P29} ją policzył i wydrukował obok długości kodu, i był
ostrożny co do tego, czym ona jest: to \textbf{podłoga}. To najkrótsza
średnia, jaką kod mógłby osiągnąć, gdyby długość mogła wynosić
$\logb{2}\frac{5}{2}$ bita, a nie może \dash{} więc najlepszy kod, jaki
naprawdę da się zbudować dla tego źródła, daje średnio $\val{p30.src.ce}$, a
nie $\val{p30.src.h}$.

Płacisz więc $\val{p30.src.ce}$ przy podłodze $\val{p30.src.h}$.

O ile jesteś nad podłogą i gdzie widziałeś \emph{tę} liczbę wcześniej?
\dotline
\nextframe
\end{fr}
...
Jest tym, i tę połowę o zaokrągleniu warto potraktować poważnie, zanim
cokolwiek się na niej zbuduje.

Program~\ref{prog:P29} osiąga tu swoją najlepszą długość na dwa sposoby
\dash{} cztery dwucyfrowe słowa albo dokładnie te długości, których używasz
\dash{} a oba dają średnio dokładnie $\val{p30.src.ce}$. Kod, którym wysyłasz
to źródło, \emph{jest} więc jednym z najlepszych dla niego: \textbf{względem
najlepszego dostępnego kodu ten niewłaściwy nie kosztuje cię nic.} Wielkość
$\val{p30.src.kl}$ to nadwyżka nad \emph{podłogą}, do której żaden kod o
całkowitych długościach nie sięga \dash{} i dlatego
Program~\ref{prog:P29} miał prawo nazwać to w całości zaokrągleniem.

I jest jeszcze czymś.

\begin{center}
\emph{Koszt kodu minus podłoga, którą wyznacza samo źródło,\\
...
Kod, którym wysyłasz to źródło \dash{} ten, który Program~\ref{prog:P29}
zbudował dla swojego \emph{pierwszego} alfabetu \dash{} ma długości
...
tyle, że rozkład sumuje się co najwyżej do jedynki. Zwróć uwagę, po czym ta
suma biegnie: dzielenie przez $p_i$ wymaga, by $p_i$ było dodatnie, więc
wyniki, na których $p$ nie ma wagi, wypadają, a po reszcie $q_i$ mogą nie
dobić do jedynki \dash{} dokładnie wtedy, gdy $q$ kładzie masę tam, gdzie $p$
nie ma żadnej. Nierówność to przetrzymuje, a §4 jest o tym przypadku.
\end{fr}
...
ściśle dodatnich rozkładach na trzech wynikach o mianowniku
$\val{p30.tri.den}$, a więc po każdej ich uporządkowanej parze,
dywergencja nigdy nie jest ujemna i wynosi zero tylko tam, gdzie oba się
zgadzają. Wyczerpująco na tej siatce, co czyni
...
Najpierw jedna zmiana jednostki, i to ta kolizja, przed którą ostrzegał
Program~\ref{prog:F03}. Sekcje 1 i 2 liczyły w bitach, bo liczyły cyfry
dwójkowe, które ktoś musi wysłać. Od tego miejsca logarytm jest naturalny, a
liczby są w \textbf{natach}, czyli w tym, co raportuje strata; pomnóż przez
$\logb{2} e = \num{1.443}$, żeby odczytać którąkolwiek z nich z powrotem w
bitach. W matematyce nic się nie zmienia, a każda tożsamość powyżej
przetrzymuje to podstawienie.
...
ustawieniem temperatury, policzony z ośmiu prawdopodobieństw z dwóch
skrajnych kolumn Programu~\ref{prog:F05}.
...
\textbf{zupełnie nic} \dash{} tego wyniku w ogóle nie ma w sumie.
\end{ansblock}
...
\mermaidfig{p30-which-is-first}{Co rozstrzyga pierwszy argument}{%
  który argument waży sumę}
\end{fr}
...
poszukiwań jest dla niego niedostępna, a to, co zostaje, to część
szeroka.
\end{fr}
...
Sprawdzono każdą uporządkowaną trójkę trzech \emph{różnych} rozkładów na tej
siatce: $\val{p30.tri.total}$ z nich.
...
Kolejne $\val{p30.tri.equal}$ trójek spełnia ją z równością, dokładnie, więc
nierówność nie jest tu naruszona tylko tu i ówdzie: jest naruszona w jednej
czwartej przypadków i ledwie utrzymana w sporej części reszty. Oba te
zliczenia rozstrzyga się na liczbach całkowitych, a nie w arytmetyce
zmiennoprzecinkowej, i powód jest z Programu~\ref{prog:P01}: porównanie
dwóch takich sum jako ułamków dziesiętnych rzuca przypadki równości na tę
stronę, na którą wypadnie zaokrąglenie, a zliczenie, które zmienia się wraz z
arytmetyką, jest faktem o maszynie.
...
\mermaidfig{p30-three-promises}{Co obiecuje to słowo i co się utrzymuje}{%
  co obiecuje słowo odległość}
\end{fr}

% ============================================================
\section{Dlaczego jej nie usymetrycznić}
\label{sec:P30-js}
...
Wygląda to na ciekawostkę o masach punktowych. Model z softmaksem nigdy nie
ma nośnika rozłącznego z czymkolwiek \dash{} kładzie trochę masy na każdym
tokenie ze słownika. Kiedy więc w życiu biegu treningowego wielkość
symetryczna i ograniczona i tak się nasyca?
\dotline
\nextframe
\end{fr}
...
treningowy się zaczyna \dash{} i dochodzi tam łagodniejszą drogą niż przez
brakujący wynik.

Na początku model jest niemal jednostajny na słowniku, więc masa, którą kładzie
na tokenie faktycznie oczekiwanym przez cel, wynosi około $\frac{1}{V}$.
Nośniki nakładają się całkiem dobrze; mała jest masa, którą \emph{dzielą}. I
to wystarcza. Przy słowniku $\val{p30.init.vocab}$ tokenów wielkość
symetryczna leży $\val{p30.init.gap}$ poniżej swojego sufitu, gdzie
dywergencja jest wciąż skończona i wciąż maleje, więc ta ograniczona podaje
niemal tę samą liczbę, cokolwiek model zrobi, i niesie \textbf{prawie żadną
informację o tym, w którą stronę się ruszyć}.

Rozłączne nośniki to ostrzejsza wersja tego samego i należą do generatorów o
wyjściach ciągłych, gdzie wyjścia modelu mogą wprost minąć dane. Klasyfikator
nigdy tam nie dociera; nie musi.
...
\sumitem{7--10}{\textbf{\result{Nadwyżka nigdy nie jest ujemna}}, na mocy
  nierówności Jensena z Programu~\ref{prog:P19} dla wklęsłego logarytmu, i wynosi zero
  dokładnie wtedy, gdy oba rozkłady się zgadzają. Sprawdzone po każdej
  uporządkowanej parze z $\val{p30.simplex.n}$ ściśle dodatnich rozkładów na
  siatce.}
...
\sumitem{34--38}{\textbf{\result{To nie jest odległość.}} Nierówność trójkąta, którą
  ...
  $\val{p30.tri.total}$ uporządkowanych trójek różnych rozkładów \dash{}
  $\val{p30.tri.pct}$ procent, czyli w przypadku typowym \dash{} przy czym
  najgorszy objazd jest krótszy o $\val{p30.tri.gap}$ nata, a kolejne
  $\val{p30.tri.equal}$ trójek spełnia ją z równością.}
...
\item Źródło stosuje się do $(\frac12, \frac12)$, a kodujesz je schematem
  ...
  \answerto{Długości kodu to $\logb{2}\frac43 = \val{p30.t1.len}$ oraz
  $\logb{2}4 = 2$, więc entropia krzyżowa wynosi
  $\frac12(\val{p30.t1.len}) + \frac12(2) = \val{p30.t1.ce}$ bita.
  Entropia to $1$ bit. Dywergencja to różnica, $\val{p30.t1.kl}$ bita
  \dash{} tyle kosztuje cię niewłaściwy kod na symbol.}
...
\item Podaj jednozdaniowy powód, dla którego ograniczona, symetryczna
  ...
  skargę z §3 i §6.
  \answerto{Bo się nasyca. Bieg startuje z modelem niemal jednostajnym, więc
  masa, którą dzieli z celem, wynosi około $\frac{1}{V}$ \dash{} nośniki
  się nakładają, ale ledwie \dash{} a wielkość ograniczona leży
  $\val{p30.init.gap}$ poniżej $\ln 2$, cokolwiek model zrobi, więc nie
  mówi prawie nic o tym, w którą stronę się ruszyć.}
