...
\mermaidfig{p31-what-a-margin-drops}{Co odrzucają dwa brzegi}{%
  czego same brzegi nie odtworzą}
\end{fr}
...
\mermaidfig{p31-two-routes-one-number}{Dwie drogi do tej samej wielkości}{%
  różnica i spadek niepewności}
\end{fr}
...
lepsza: jakiekolwiek jest obciążenie, jest rzędu samej wielkości. Nic w
samej liczbie by tego nie zdradziło.
...
pójdziesz dalej. Ile razy to uruchamiasz?
\dotline
\nextframe
\end{fr}
...
Przetasowane uruchomienie to jedno losowanie z rozkładu zerowego.
Powyższe wyliczenie jest jedynym miejscem w tym programie, gdzie widać cały
taki rozkład, a nie jedną jego próbkę; tabela wypisała tylko jego średnią.

Przy $N = 10$ jego rozrzut to $\val{p31.null.sd10}$ nata wobec średniej
$\val{p31.bias.n10}$, a mniej więcej jedno uruchomienie na dwadzieścia
zwraca $\val{p31.null.q95}$ nata lub więcej \dash{} na danych, w których
nic nie ma. Rozrzut przewyższa średnią przy każdym $N$, które obejmuje
tabela, więc uruchomienie zwracające wielokrotność podłogi nie jest
artefaktem najmniejszej próby.

\begin{trapbox}
\textbf{\enquote{Uruchomiłem przetasowaną kontrolę i ją odjąłem.}}

Kontrola jest zmienną losową, a jedno uruchomienie to jedna jej próbka,
więc to, co podajesz, jest prawdziwą nadwyżką powiększoną o to, jak daleko
tamto jedno losowanie padło od obciążenia, które miało zastąpić \dash{} o
wielkość tak dużą jak samo obciążenie. Może wyjść ujemna, a nic w tym
uruchomieniu nie powie, co się stało.
...
\mermaidfig{p31-bias-is-not-signal}{Skąd bierze się estymat wtykowy}{%
  sygnał czy próba}
\end{fr}
