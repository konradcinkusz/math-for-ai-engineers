...
\item Pięć odczytów to $\val{p02.var.offset}$ i cztery kolejne mikrosekundy po
  nim. Ich wariancję, dzieląc przez pięć, liczy się w \code{float32} jako
...
\item Wyniki uwagi przechodzą przez eksponentę wewnątrz softmaksu. Co to może
  zrobić w \code{fp16}? \teachesat{14--15}
  \answerto{Przepełnić się do \code{inf}, przy zwyczajnym wyniku, a nie przy
  jakimś egzotycznym \dash{} a wtedy wszystko dalej staje się \code{nan}.}
...
\item Potrzebujesz $\ln \sigma(x)$ dla dużego ujemnego logitu. Co daje
  policzenie najpierw $\sigma$ i poniżej jakiego z grubsza $x$?
  \teachesat{24--25}
  \answerto{\code{-inf}, bo $\sigma$ wpadła w niedomiar do dokładnego zera
  \dash{} co dzieje się na dolnym urwisku tego formatu, w którym liczysz.
  Policzone jako jedna operacja nie sprawia żadnej trudności.}
...
i dzielą swoich pierwszych $\val{p02.var.shared}$ cyfr, podczas gdy
...
\begin{fr}
Dwie liczby dzielą swoich pierwszych $\val{p02.var.shared}$ cyfr i zostają
...
każdego argumentu wynosi około $\eps$, a $\eps$ to właśnie to, co zmierzył
Program~\ref{prog:P01}: koszt zaokrąglenia jednej liczby do formatu. Nic w tym
nie obejmuje błędu, z którym wartość przychodzi. Jeśli $a$ i $b$ są same
wynikami długich obliczeń, przychodzą z większym błędem, a ten sam współczynnik
mnoży to, z czym przyszły.
...
zdefiniowała. Program~\ref{prog:P11} podaje go porządnie. Tutaj należy wersja
...
naturalnie występują, a czynnik $1/\sqrt{d_k}$, który wyprowadza
Program~\ref{prog:P25}, nie pozwala tej skali rosnąć wraz z rozmiarem głowy
\dash{} z powodów dotyczących gradientu softmaksu, a nie sufitu formatu, choć
zdejmuje ze stołu i tę awarię.
Awaria nie jest rzadkim zdarzeniem na skraju zakresu
...
zwyczajną liczbą niosącą dokładność formatu, a jedną jednostkę powyżej daje
\code{inf}, i nie ma nic pomiędzy. Test, który przechodzi zakres i sprawdza
błąd, przejdzie więc czysto aż do ostatniego bezpiecznego wyniku i nie powie
nic o następnym.
...
\begin{fr}
\ans{\code{bf16} i \code{fp32}, bo dzielą budżet wykładnika, a \emph{górę}
zasięgu eksponenty wyznacza sam ten budżet.}
...
To powtórzenie pointy Programu~\ref{prog:P01} \dash{} o pierwszej kolumnie.
Druga to inna historia, a wiersz, który to mówi, należy do \code{bf16}: dzieli
budżet wykładnika \code{fp32} dokładnie, a jego podłoga leży
$\val{p02.floor.gap}$ wyniku \emph{powyżej} podłogi \code{fp32}.

Każdy wpis w tamtej kolumnie to logarytm najmniejszej liczby
\emph{subnormalnej} formatu, a głębokość liczby subnormalnej kupuje się bitami
mantysy. \code{fp32} ma ich o $\val{p02.floor.bits}$ więcej niż \code{bf16}, co
kupuje o $\val{p02.floor.bits}$ więcej zakresów subnormalnych \dash{}
$\val{p02.floor.bits} \ln 2$, czyli właśnie te $\val{p02.floor.gap}$.

\textbf{Budżet wykładnika ustala sufit; mantysa przedłuża podłogę.}
\end{fr}

\mermaidfig{p02-two-cliffs}{Dwa sposoby, na jakie liczba opuszcza format, i co
ustala odległość między nimi.}{przepełnienie, niedomiar i co je ustala}
...
Oto wiersz $\val{p02.pivot.total}$ wyników:
\[ \val{p02.pivot.s1}, \quad \val{p02.pivot.s2}, \quad
   \val{p02.pivot.s3}, \quad \val{p02.pivot.s4}, \quad \val{p02.pivot.s5} \]

Wyciągaj kolejno każdy z nich, w \code{fp16}. Ile z
$\val{p02.pivot.total}$ możliwości utrzymuje każdy składnik wewnątrz formatu?
\nextframe
\end{fr}
...
I to nie koniec, bo nieskończoność w stracie daje \code{inf}, a nie
\code{nan}. Cel dla pewnej poprawnej predykcji to $y = 1$, więc strata tworzy
$(1 - y)\ln(1 - p) = 0 \times (-\infty)$, a \emph{to} jest \code{nan}. Pewny
\emph{błędny} przykład jedynie rozbiega się do \code{inf}; to ten
\emph{poprawny} zatruwa partię, i to jest ta awaria, która wysyła ludzi na
poszukiwanie błędu w danych.
...
wciąganą do następnego składnika przed jego użyciem.}
...
ruszyło sumę, a $y$ to tyle, o ile miało ją ruszyć. Zatem $c$ trzyma dokładnie
to, co zostało zaokrąglone, \emph{z przeciwnym znakiem} \dash{} i dlatego
pierwsza linijka je odejmuje, a nie dodaje \dash{} a odzyskuje się je
odejmowaniem dwóch prawie równych liczb.
...
Cztery poprawki i naiwna pętla, którą zastępują, na tym samym milionie
wartości:
...
\sumitem{4--6}{\result{\textbf{Katastrofalne skracanie} jest mechanizmem: obie
  wielkości dzielą swoich pierwszych $\val{p02.var.shared}$ cyfr, podczas
...
\sumitem{9--10}{\result{Wzmocnienie wynosi $\frac{|a| + |b|}{|a - b|}$ i jest
  ...
  który należy do Programu~\ref{prog:P11}, bo jest stwierdzeniem o macierzy}.}
...
\sumitem{14--16}{\result{\textbf{Eksponenta opuszcza format przy logarytmie jego
  ...
  \code{bf16} i \code{fp32} dzielą wiersz \emph{u góry}, przy
  $\val{p02.over.fp32}$, bo sufit to sam budżet wykładnika; dołu nie dzielą, bo
  głębokość liczby subnormalnej kupuje się bitami mantysy, a \code{fp32} ma ich
  o $\val{p02.floor.bits}$ więcej}.}
...
\sumitem{24--26}{\result{$\ln\sigma(x)$ policzone w dwóch krokach zwraca
  ...
  \code{-inf} \dash{} a że cel mnoży ten składnik przez zero, strata jest
  \code{nan}-em na predykcji, która była \emph{poprawna}}.}
