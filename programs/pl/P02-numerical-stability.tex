...
\textbf{Współczynnik wzmocnienia to $\frac{|a| + |b|}{|a - b|}$}, a dla
powyższej wariancji jest rzędu $10^{\val{p02.var.shared}}$.
\end{fr}
...
Wiersz \code{bf16} równa się wierszowi \code{fp32} u góry, co jest powtórzeniem
pointy Programu~\ref{prog:P01}: oba dzielą budżet wykładnika, a \textbf{zakres
eksponenty ustala sam ten budżet}. Mantysa nie pojawia się w tej tabeli nigdzie.
\end{fr}
...
\begin{fr}
Dla softmaksu oba kroki składają się w jedną operację, a algebrę warto
zobaczyć, bo ma trzy linijki i usuwa również dzielenie.
\[ \ln \softmax(z)_j = \ln \frac{e^{z_j}}{\sum_k e^{z_k}}
   = z_j - \ln \sum_k e^{z_k} = z_j - \logsumexp(z) \]
Żadnej eksponenty surowego wyniku, żadnego dzielenia, żadnego
prawdopodobieństwa. Log-softmax to odjęcie wielkości z poprzedniej sekcji od
wyniku, a odejmowanie odbywa się między liczbami tego samego rzędu, czyli tak,
...
\begin{fr}
\begin{trapbox}
\textbf{A drugi koniec też zawodzi, i to jest ta połowa, którą się pomija.}
...
Kahana \dash{} trzyma drugą zmienną z tą częścią ostatniego dodawania, która
przepadła, i dodaje ją z powrotem do następnego składnika przed jego użyciem:
\begin{align*}
  y &= x_i - c \\
  t &= s + y \\
  c &= (t - s) - y \\
...
\begin{fr}
Trzecia to to, co robi biblioteka, i nie potrzebuje dodatkowej zmiennej.
\textbf{Dodawaj w zrównoważonym drzewie}, a nie w linii: sparuj wartości, dodaj
pary, sparuj te wyniki i tak dalej. Liczba dodawań się nie zmienia \dash{}
wciąż jest ich mniej więcej milion \dash{} ale \emph{głębokość} owszem: każda
wartość dociera teraz do sumy przez mniej więcej dwadzieścia z nich, a nie przez
milion, więc dryf ma dwadzieścia okazji do nagromadzenia wzdłuż dowolnej ścieżki
zamiast miliona.
...
brakiem odpowiedzi. Druga to fakt, że \textbf{sortowanie wypada o rząd
wielkości gorzej niż pozostałe trzy} \dash{} jest najtańszą poprawką i
najsłabszą, a to właśnie ją najczęściej opisuje się jako wystarczającą.
\end{fr}
...
% ============================================================
\begin{summarybox}
\sumitem{1--3}{\textbf{Algorytm może być poprawny w arytmetyce dokładnej i
  ...
  wariancją.}
\sumitem{4--6}{\textbf{Katastrofalne skracanie} jest mechanizmem: obie
  ...
  skąd się wzięły.}
\sumitem{7--8}{\textbf{Lekarstwem nigdy nie jest większa precyzja} \dash{}
...
\sumitem{9--10}{Wzmocnienie wynosi $\frac{|a| + |b|}{|a - b|}$ i jest
...
\sumitem{11--13}{Na $\val{p02.layers}$ warstwach najgorszy przypadek
  ...
  pomiędzy $\sqrt{n}$ a $n$.}
\sumitem{14--16}{\textbf{Eksponenta opuszcza format przy logarytmie jego
...
\sumitem{17--20}{$\ln\sum_i e^{z_i} = c + \ln\sum_i e^{z_i - c}$ zachodzi dla
  ...
  $\val{p02.pivot.worst}$.}
\sumitem{21}{\textbf{Niedomiar w sumie jest zwykle nieszkodliwy, a
...
\sumitem{22--23}{\textbf{Prawdopodobieństwo jest złą wartością pośrednią na obu
...
\sumitem{24--26}{$\ln\sigma(x)$ policzone w dwóch krokach zwraca
  ...
  \code{nan}.}
\sumitem{27--28}{Przejście w przód kończy się na logitach, a strata wykonuje
  ...
  pomieści.}
\sumitem{29--30}{$\val{p02.sum.n}$ wartości równych $\val{p02.sum.small}$
...
\sumitem{31--35}{Cztery poprawki, i nie są równoważne: od najmniejszych
...
\sumitem{36--37}{$\eps$ w normalizacji warstwowej chroni dzielenie przez
