...
\item Znasz wartość funkcji, nachylenie i krzywiznę w punkcie. Co najlepszego
  możesz powiedzieć o niej w pobliżu? \teachesat{3--4}
...
\item Program~\ref{prog:P15} pokazał, że każdy kierunek jest mnożony przez
  $1 - \eta\lambda$ w każdym kroku. Jaki jest największy użyteczny $\eta$?
  \teachesat{16--17}
  \answerto{$2/\lambda$, bo spacer pozostaje ograniczony dokładnie wtedy, gdy
  $\lvert 1 - \eta\lambda \rvert < 1$. Przy wielu kierunkach wygrywa
...
\item Metoda Newtona używa krzywizny i trafia w minimum funkcji kwadratowej w
  jednym kroku. Dlaczego nie używa się jej do trenowania?
  \teachesat{26--27}
...
przeżywa powyższego zliczania: minimum wymaga zgody każdego znaku, a siodło
jednego odszczepieńca, więc punkt stacjonarny w wysokim wymiarze
...
\begin{fr}
\begin{ansblock}
Tak, i to ledwo \dash{} dlatego jego stroma współrzędna zmienia znak w każdym
kroku zamiast rozbiegać się.
\end{ansblock}
...
Jego konsekwencją jest tempo. Nawet przy najlepszym kompromisowym kroku
$\val{p17.best.eta}$ wolny kierunek kurczy się o czynnik $\val{p17.rate}$ na
...
$H\,\mathbf{s} = -\nabla f$ i zrób krok o $\mathbf{s}$. Ile kroków zajmuje to
...
Program~\ref{prog:F01} zbudował drabinę rzędów wielkości i nie ma na niej
niczego tak dużego. Liczba parametrów jest liczbą, którą da się utrzymać; liczba
\emph{par} parametrów nie jest, a robi to podnoszenie do kwadratu.
...
\begin{fr}
\begin{ansblock}
Bo $\lambda_{\max}$ też się przeskalowuje. Nie ma to znaczenia, bo $\eta$
przeskalowuje się razem z nią, w przeciwną stronę, więc \emph{krok faktycznie
zrobiony} pozostaje bez zmian.
\end{ansblock}
...
% ============================================================
\begin{summarybox}
\sumitem{1--2}{\textbf{Model Taylora drugiego rzędu to wartość, nachylenie i
  połowa krzywizny razy kwadrat}, a $\tfrac{1}{2}$ jest tym, co sprawia, że
  druga pochodna modelu równa się drugiej pochodnej funkcji.}
\sumitem{3--4}{\textbf{\enquote{Najlepszy} znaczy rząd, a nie rozmiar}: błąd
...
\sumitem{5--6}{\textbf{Model jest lokalny.} Czyni opis lokalny lepszym, a nie
  szerszym, i nie mówi nic o tym, gdzie jest minimum.}
\sumitem{7--8}{W punkcie stacjonarnym składnik liniowy znika, więc znak
  krzywizny klasyfikuje \dash{} zupełnie w jednym wymiarze, a w większej
  liczbie nie.}
\sumitem{9--10}{\textbf{Hesjan jest jakobianem gradientu}, więc reguła
  Programu~\ref{prog:P16} czyni go kwadratowym, z jednym wierszem i jedną
  kolumną na wejście.}
\sumitem{11--12}{\textbf{Jest symetryczny}, więc Program~\ref{prog:P10} stosuje
  się bez zmian: rzeczywiste wartości własne, prostopadłe kierunki, każdy z
  własną krzywizną.}
\sumitem{13--14}{Znaki wartości własnych dają misę, odwróconą misę, siodło i
  grzbiet \dash{} klasyfikację z P10, teraz dla dowolnej gładkiej funkcji w
  punkcie stacjonarnym.}
\sumitem{15}{\textbf{Punkt stacjonarny w wysokim wymiarze przytłaczająco
...
\sumitem{16--17}{\textbf{Spacer pozostaje ograniczony dokładnie wtedy, gdy
  $\lvert 1 - \eta\lambda\rvert < 1$, czyli $\eta < 2/\lambda$}, co jest faktem
  o ciągu geometrycznym i o niczym więcej.}
\sumitem{18--19}{\textbf{Najbardziej stromy kierunek ustala krok dla całego
...
\sumitem{20--21}{Spacer z Programu~\ref{prog:P15} przy $\val{p17.p15.eta}$
  ...
  geometryczny.}
\sumitem{22--23}{\textbf{Wskaźnik uwarunkowania $\val{p17.kappa}$ to ta sama
...
\sumitem{24--25}{\textbf{Metoda Newtona ląduje w minimum funkcji kwadratowej w
  $\val{p17.newton.steps}$ kroku}, dając każdemu kierunkowi własny rozmiar
  kroku.}
\sumitem{26--28}{\textbf{A jej hesjan ma $\val{p17.hess.entries}$ wpisów przy
...
\sumitem{29--30}{\textbf{Metoda Newtona znajduje punkty stacjonarne, a nie
  ...
  wymiarze.}
\sumitem{31--33}{\textbf{Przeskalowanie parametru zmienia krzywiznę o
  $\val{p17.rescale.factor}$ i zostawia każde wyjście identyczne.} Więc jeden
  model ma dwie ostrości, a goła liczba opisująca ostrość jest faktem o
  układzie współrzędnych.}
\sumitem{34}{\textbf{Żadna wielkość zbudowana z gołych wartości własnych tego
  ...
  z konstrukcji.}
\sumitem{35--36}{Przeżyłaby miara przekształcająca się tak jak parametry.
  \textbf{Ta książka nie zmierzyła twierdzenia o uogólnianiu i go nie
  rozstrzyga}; dostarcza filtru.}
\sumitem{37--38}{\textbf{Ograniczenie na krok ma ten sam problem i nie cierpi z
  jego powodu}, bo $\eta$ przeskalowuje się razem z $\lambda$, a krok zrobiony
  pozostaje bez zmian. Pytaniem o krzywiznę nigdy nie jest \emph{czy jest
  duża}, tylko \emph{względem czego}.}
\end{summarybox}
...
\item Strata ma w bieżącym punkcie wartości własne hesjanu $50$, $5$ i
  $\num{0.5}$. Podaj największy użyteczny krok, wskaźnik uwarunkowania i to,
  który kierunek jest problemem.
