...
Siatka musi być gęstsza niż pół dziesiątej punktu, więc $1/n$ musi być
poniżej $\num{0.001}$.
...
żeby go znaleźć. Przedział to kwantyle tego rozkładu, zsumowane symbolem
Newtona z Programu~\ref{prog:P12}:
...
$\val{p27.concord}$ pozycjach. Sekcja 1 ustaliła, że B prowadzi o
$\val{p27.net}$ pozycję.
...
Równy podział to \emph{najbardziej prawdopodobny pojedynczy} wynik i mimo to
zdarza się tylko w $\val{p27.p.tie.pct}$ procentach przypadków. Wszystko inne
leży co najmniej $\val{p27.net}$ od równego, więc wynik co najmniej tak
przechylony ma prawdopodobieństwo $\val{p27.p.exact}$ \dash{} tak wygląda
blisko siedem na osiem rzutów trzydziestoma monetami.
...
czytelnik może zabrać na spotkanie, i jest jedenaście razy większa od tej,
którą opublikowano.
...
\begin{fr}
\begin{aibox}
\textbf{Gdzie to widać.} Dwie linijki, które twój harness mógłby drukować i
nie drukuje.
...
\begin{fr}
Liczba $\val{p27.p.exact}$ z poprzedniej sekcji ma nazwę, a ty policzyłeś ją
już przed poznaniem słowa, i to jest właściwa kolejność.
...
strony. \textbf{Jedno na dwadzieścia porównań dwóch identycznych modeli
zaraportuje istotną różnicę} i nie ma wersji tej procedury, w której jest
inaczej.
...
Próg przesuwa mnożnik w przedziale z Programu~\ref{prog:P25} na
$\val{p27.z.bonf}$ zamiast $\num{1.96}$, a wzór z sekcji 3 niesie ten mnożnik
...
jedynej liczbie, o którą trzeba by poprosić.
...
\begin{summarybox}
\sumitem{1--3}{\textbf{Czytaj wynik najpierw wobec jego własnego mianownika.}
  ...
  po przecinku wymaga ponad $\val{p27.decimal.floor}$ pozycji, zanim to
  miejsce się zaokrągli.}
\sumitem{3--5}{\textbf{Opublikowana różnica to $\val{p27.gap.items}$ pozycja.}
...
\sumitem{6}{Porównywanie dwóch osobnych przedziałów po tym, czy się nakładają,
  nie jest testem. Buduj przedział na różnicy, czyli na wielkości, o którą
  pytasz.}
\sumitem{7--9}{\textbf{Bootstrap} losuje z próbki ze zwracaniem i nie
  ...
  przypadkiem.}
\sumitem{10--11}{Jego zastosowaniem jest przypadek bez wzoru \dash{} mediana,
...
\sumitem{12--14}{\textbf{Jednostka losowania to decyzja, której nic nie
...
\sumitem{15--17}{\textbf{Parowanie mnoży potrzebne pozycje przez
...
\sumitem{18--19}{\textbf{Różnicę niosą tylko pozycje niezgodne.} Te
...
\sumitem{19--20}{Przy \enquote{równie dobre} te pozycje są uczciwymi rzutami
  ...
  wariancji. Przerobiona różnica daje $\val{p27.p.exact}$.}
\sumitem{21}{\textbf{Na tej ewaluacji różnica poniżej
  $\val{p27.net.needed.pts}$ punktu nie jest wynikiem}, znalezione przez
  szukanie. Opublikowana różnica to $\val{p27.gap.pts}$ punktu.}
\sumitem{23--25}{\textbf{P-wartość to
  ...
  częstości bazowej.}
\sumitem{26--27}{Mając rozkład a priori da się ją odwrócić, a odpowiedź
...
\sumitem{28--29}{\textbf{Jedno na dwadzieścia porównań identycznych modeli
...
\sumitem{31--33}{\textbf{$\val{p27.models}$ porównań robi z tego
...
\sumitem{34--35}{\textbf{A sortowanie tabeli obciąża jej górny wiersz.}
...
\sumitem{37--39}{To, co zostaje, gdy porównanie nie rozstrzyga rzeczy, na
...
\item Model raportuje $\num{83.3}$ procent na $60$ pozycjach ewaluacyjnych.
  ...
  \answerto{Tak: $\num{83.3}$ procent z $60$ to dokładnie $50$ pozycji. Siatka
  ma tu oczko $100/60 = \num{1.67}$ punktu, więc sąsiadami są $49/60 =
  \num{81.7}$ i $51/60 = \num{85.0}$ procent \dash{} tak gruboziarnisty jest
  benchmark o $60$ pozycjach.}
...
\item Benchmark raportuje $\num{62.48}$ procent. Jaka jest najmniejsza liczba
  ...
  \answerto{Siatka musi być gęstsza niż pół setnej punktu, więc
  $1/n < \num{0.00005}$ i $n > 20\,000$. Prawie żaden benchmark nie ma tylu
  pozycji, więc dwa miejsca po przecinku w publicznym rankingu są ozdobą.}
...
\item Dwa modele porównano na $n$ wspólnych pozycjach i nie zgadzają się na
  ...
  produkowała. $\rho$ trzeba oszacować; $d$ to dwa liczniki w pętli, i
