...
\outcome{7--15}{zbudować przedział ufności dla metryki, która nie ma wzoru,
  nazwać jednostkę, którą trzeba losować, i powiedzieć, ile dana szerokość
  kosztuje pozycji}
\outcome{16--23}{zaplanować i przeprowadzić porównanie dwóch modeli
  ewaluowanych na tych samych promptach}
\outcome{24--31}{powiedzieć, czym p-wartość jest, a czym nie jest}
\outcome{32--39}{powiedzieć, co czterdzieści porównań w jednym rankingu robi z
  tym wszystkim}
\outcome{40--42}{zastosować całość do liczby, którą ktoś opublikował}
\end{outcomes}
...
\item Porównanie zwraca $p = \num{0.03}$. Jakie jest prawdopodobieństwo, że
  oba modele są równie dobre? \teachesat{25--26}
...
\mermaidfig{p27-what-to-resample}{Jednostka losowania to decyzja, której nic
  nie sprawdza}{wiersze, użytkownicy i co widzi biblioteka}
...
więc dokładnie jedna dźwignia. Twoja ewaluacja ma $\val{p27.boot.n}$ pozycji,
a przedział powyżej biegnie od $\val{p27.boot.lo}$ do $\val{p27.boot.hi}$,
czyli ma $\val{p27.w.200}$ punktów szerokości; tablica wyników drukuje jedno
miejsce po przecinku. Mniej więcej ile pozycji trzeba, żeby ten przedział
zszedł poniżej jednego punktu?
\dotline
\nextframe
\end{fr}
...
\begin{fr}
\begin{ansblock}
Około $\val{p27.w.onepoint}$ \dash{} szerokość musi spaść
$\val{p27.w.200}$ razy, a czterokroć więcej pozycji dzieli ją na pół, więc
pozycje idą jak kwadrat tego czynnika: $\val{p27.w.200}$ do kwadratu, razy te
$\val{p27.boot.n}$, które masz.
\end{ansblock}

Zmierzone przez trzy rzędy wielkości przy tej samej dokładności, z
przedziałem policzonym dokładnie zamiast losowanym:

\begin{center}
\begin{tabular}{rrr}
\toprule
pozycji w zbiorze & szerokość przedziału, punkty & przedział, w pozycjach \\
\midrule
...
Drugi wiersz to przedział, który już masz, a reszta to ten sam rachunek przy
innych rozmiarach. Czterokroć więcej pozycji dzieli szerokość na pół na
każdym kroku: Program~\ref{prog:P25} podaje tę regułę, a tutaj jest ona
zmierzona, a nie zacytowana \dash{} i \textbf{dokładność siedzi w trzeciej
kolumnie}, gdzie rozpiętość podwaja się na liczbach całkowitych. Kolumna
punktów to te rozpiętości podzielone przez rozmiar zbioru i zaokrąglone, więc
podzielenie dwóch jej wpisów daje $2$ z dokładnością do dwóch miejsc po
przecinku, a nie $2$ dokładnie.

Jedyny krok, który nie jest połową, to pierwszy, i łamie go najmniejszy
zbiór: $50$ pozycji daje $\val{p27.w.coarse}$ razy szerokość
$\val{p27.boot.n}$ pozycji zamiast dwa razy. Rozpiętość jest całkowitą liczbą
pozycji, a na $50$ pozycjach jedna pozycja warta jest dwa punkty, więc
przedział ma $\val{p27.w.span.50}$ pozycji szerokości, a następna rozpiętość
w dół jest węższa o całe dwa punkty. To siatka z sekcji 1 w najgrubszym
wydaniu i tutaj zaokrągla na zewnątrz: bootstrap podaje odrobinę więcej
niepewności niż wzór.

Tabela sama ogranicza własną odpowiedź \dash{} $\val{p27.w.12800}$ punktu
przy $\num{12800}$ pozycjach i $\val{p27.w.51200}$ przy $\num{51200}$
\dash{} a przejście wypada około $\val{p27.w.onepoint}$. \emph{Około} jest
formą uczciwą, a nie asekuracją: rozpiętość zmienia się po jednej pozycji,
więc w pobliżu przejścia szerokość skacze nad jeden punkt i pod niego w miarę
rośnięcia zbioru, a rozmiar podany co do pozycji byłby własnością siatki, a
nie metody.

To wszystko jest jednym przedziałem na jednym modelu. Pytanie, od którego
program się zaczął, dotyczy różnicy między dwoma modelami, odpowiadająca na
nie liczba należy do sekcji 3, a nie do tej, i okazuje się, że zadanie
pytania o różnicę wprost jest znacznie tańsze niż nakładanie przedziału
osobno na każdy model.
\end{fr}
...
\mermaidfig{p27-winner-is-noise}{Co sortowanie tabeli równych robi z jej
  szczytem}{równe modele, posortowane po błędach}
...
\begin{fr}
Trzecia oś, ta sama suma i żadnej tabeli wyników w środku.

Ewaluacja przebiega i zmiana nie pokazuje wygranej. Uruchamiasz ją znowu na
innym ziarnie. Potem na nowszym wycinku zbioru. Piąty przebieg pokazuje
wygraną przy $\val{p27.alpha}$ i to właśnie ten trafia do raportu.

Nikt nie nazywa tego $\val{p27.peek.m}$ porównaniami i nikt nigdzie nie
zapisuje tej liczby. Policz próby tak, jak tabela wyników liczyła modele: przy
zmianie, która nie robi zupełnie nic, jaka jest szansa, że jedna z
$\val{p27.peek.m}$ prób pokaże wygraną?
\dotline
\nextframe
\end{fr}

\begin{fr}
\ans{$\val{p27.peek.pct}$ procent}

$1 - \num{0.95}^{\val{p27.peek.m}}$, czyli własne wyrażenie tabeli wyników z
próbami w miejscu modeli. Arytmetyce jest wszystko jedno, czy tych $m$ rzeczy
to modele, konfiguracje, czy wtorki.

\textbf{I to jest ta jego wersja, której nikt nie wpisuje do budżetu}, bo
próby są rozdzielone w czasie, a nie ułożone w kolumnie. $\val{p27.models}$
wierszy w tabeli to $\val{p27.models}$, które widać; $\val{p27.peek.m}$ powtórek
przez dwa tygodnie to pięć popołudni, z których każde wydawało się jednym
testem, a te dwa tygodnie nie zostawiają śladu mówiącego inaczej.

Przy $\val{p27.peek.coin}$ próbach wskaźnik przekracza połowę. Procedura, która
donosi o tym, czego szukała, równie często jak moneta, przy każdym kroku
wykonanym poprawnie.

Naprawą jest znowu Bonferroni, a postawienie tego w ten sposób robi z niej
wniosek, a nie drugą regułę do zapamiętania: utrzymanie ogólnego wskaźnika na
$\val{p27.alpha}$ przez $m$ prób wymaga $\val{p27.alpha}/m$ przy każdej z
nich. \textbf{Czyli $m$ musi być ustalone przed pierwszą próbą}, bo progu nie
da się podzielić przez liczbę, której nikt jeszcze nie wybrał. Na tym polega
całe to, że \enquote{uruchom jeszcze raz} nie jest procedurą \dash{} nie na tym, że
powtarzanie jest nieuczciwe, lecz na tym, że zostawia $m$ nieokreślone, dopóki
wynik go nie wybierze. Program~\ref{prog:P25} i jego seria zielonych to ten
sam warunek z drugiej strony: liczba, która uprawnia do twierdzenia, jest
ustalana przed przebiegami, a nie kiedy już są.
\end{fr}
...
\sumitem{14--15}{\result{\textbf{Szerokość ustala liczba pozycji i nic poza nią},
  a czterokroć więcej pozycji dzieli ją na pół \dash{} zmierzone, a nie
  zacytowane. Przedział ma rozpiętość $\val{p27.w.span.200}$ pozycji przy
  $\val{p27.boot.n}$ i $\val{p27.w.span.51200}$ przy $\num{51200}$, a poniżej
  jednego punktu schodzi przy około $\val{p27.w.onepoint}$ pozycjach}.}
\sumitem{16--18}{\result{\textbf{Parowanie mnoży potrzebne pozycje przez
...
\sumitem{19--20}{\result{\textbf{Różnicę niosą tylko pozycje niezgodne.} Te
...
\sumitem{20--21}{\result{Przy \enquote{równie dobre} te pozycje są uczciwymi rzutami
...
\sumitem{24--26}{\result{\textbf{P-wartość to
  ...
  częstości bazowej}.}
\sumitem{27--28}{\result{Mając rozkład a priori da się ją odwrócić, a odpowiedź
...
\sumitem{29--30}{\result{\textbf{Jedno na dwadzieścia porównań identycznych modeli
...
\sumitem{32--34}{\result{\textbf{$\val{p27.models}$ porównań robi z tego
...
\sumitem{35--36}{\result{\textbf{A sortowanie tabeli obciąża jej górny wiersz.}
...
\sumitem{38--39}{\result{\textbf{A ta sama suma liczy próby równie chętnie
  jak modele.} Powtarzanie ewaluacji, aż pokaże wygraną, zamienia
  $\val{p27.alpha}$ w $\val{p27.peek.pct}$ procent przy
  $\val{p27.peek.m}$ próbach, a $\val{p27.peek.coin}$ prób czyni z tego
  monetę. \textbf{Liczba prób musi być ustalona przed pierwszą z nich}, bo
  progu nie da się podzielić przez liczbę, którą wybiera wynik}.}
\sumitem{40--42}{\result{To, co zostaje, gdy porównanie nie rozstrzyga rzeczy, na
...
\item Twój zbiór ewaluacyjny ma $800$ pozycji i chcesz przedział o połowę
  węższy. Ile to kosztuje pozycji i co dzieje się z rozpiętością przedziału
  liczoną w pozycjach?
  \answerto{Czterokroć tyle, czyli $\num{3200}$ \dash{} a rozpiętość się
  \emph{podwaja}, z $\val{p27.w.span.800}$ pozycji na
  $\val{p27.w.span.3200}$. To jeden fakt, a nie dwa: szerokość to rozpiętość
  podzielona przez rozmiar zbioru, więc czterokrotne powiększenie zbioru
  przy podwojeniu rozpiętości daje połowę szerokości. Rozpiętość rosnąca
  wtedy, gdy przedział się zwęża, to połowa, którą czyta się na opak.}
