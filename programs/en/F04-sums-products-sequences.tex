...
Program~\ref{prog:P01} says why floating-point addition is not associative, and
why two accelerators summing the same gradients therefore disagree in the last
bits.
...
exactly when every $b_i$ is equal \dash{} and that is a statement about every
possible $a_i$ at once. For one particular set of numbers they can of course
coincide: two shards scoring the same accuracy agree whatever their sizes.
Equal denominators is the condition you can arrange in advance, which is why it
...
Two factors $\num{0.2}$ apart at the start, and at the far end one is about
$\val{f04.decay.ratio}$ times the other \dash{} and neither of them extreme.
...
One number of state and a couple of multiplications per observation. It is
called an
...
Two things about that expression are worth pausing on. The substitution
stops at $m_0$ because there were only $t$ steps to unroll, so this is an
identity rather than an approximation \dash{} it is exactly what the loop
computed, for whatever $t$ you like. And nothing in getting here used what
the $g$s are. Feed the loop gradients and it is momentum; feed it losses and
it is the smoothed curve on a dashboard; feed it squared gradients and it is
half of Adam. One derivation, which Program~\ref{prog:P20} spends three
times.
...
\sumitem{5--6}{\result{\textbf{$\sum a_i b_i$ is not $(\sum a_i)(\sum b_i)$}, and at
  ...
  a different sign}.}
\sumitem{7--9}{\result{The number of terms is $\text{stop} - \text{start} + 1$, because
...
\sumitem{27--28}{\result{The macro average weights groups equally, the micro average
  weights items equally, and whatever the numerators they agree exactly when
  every denominator is equal. Both are legitimate; the fault is reporting one
  as the other}.}
...
\sumitem{33--34}{\result{$\ln \prod_i x_i = \sum_i \ln x_i$, which is
  ...
  $\val{f04.grow.fifty}$: two factors $\num{0.2}$ apart at the start, and at
  the far end one is about $\val{f04.decay.ratio}$ times the other}.}
