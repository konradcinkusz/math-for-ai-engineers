...
The script confirms it to $\val{p06.assoc.err}$ on random matrices, which is
...
disagreement $\val{p06.collapse.err}$ \dash{} which is rounding, not a
difference.
\end{fr}
...
$\val{p06.bend}$ of the spacing, against $\val{p06.bend.affine}$ through the
collapsed matrix.
\end{ansblock}
...
\sumitem{12--14}{\textbf{Associativity is free}, because composing functions
  never had a bracketing: $A(B(C(x)))$ has one reading. Confirmed to
  $\val{p06.assoc.err}$, which is worth doing once because an identity agreeing
...
\sumitem{26--29}{\textbf{Two affine layers are one}: $W_2(W_1x + b_1) + b_2 =
  ...
  to $\val{p06.collapse.err}$. $k$ layers waste $(k-1)/k$ of their parameters
...
\sumitem{30--32}{\textbf{An affine map cannot bend a straight line}, so three
  collinear inputs come out collinear ($\val{p06.bend.affine}$) through the
  collapsed matrix and bent ($\val{p06.bend}$) through the $\relu$-separated
  pair. That rules out every single affine map at once, and it is why the
