...
is.}{a table, and the two rules it respects}
...
So $AB$ is the matrix of a composition \dash{} but which one? Say whether it
does $A$ first or $B$ first, and why the notation reads the way it does.
\dotline
\nextframe
\end{fr}

\begin{fr}
\ans{$B$ first. Read $AB$ as \emph{do $B$, then $A$}, right to left, the way
$f(g(x))$ is read \dash{} the notation puts the function on the left of the
thing it eats.}
...
different function.}{composing two maps each way round}
...
three dimensions \emph{about different axes} do not \dash{} turn a book
face-up-then-sideways and sideways-then-face-up and the covers point different
ways. About one fixed axis they commute in three dimensions exactly as they do
in the plane, for the same reason: the two orders add the same two
angles. \textbf{Two
...
layer computes $XW\T$ with $X$ of shape $k \times n$. The superscript is the
\textbf{transpose}: $W\T$ is $W$ with its rows and columns exchanged, so
$(W\T)_{ij} = W_{ji}$, and an $m \times n$ matrix becomes $n \times m$. That is
the same composition with the convention flipped, and Program~\ref{prog:P07} is
where the axes get named properly \dash{} along with reshape, transpose and the
...
Program~\ref{prog:F06} said the weights become a matrix here and that the
collapse argument is done again in more than one dimension. So take two layers,
$W_1 x + b_1$ then
...
One matrix and one bias vector. The argument survives exactly.
...
Worth noticing what the check does not depend on. Nothing in the rearrangement
asks the biases to be anything in particular, or the two widths to be equal, or
either matrix to be invertible: the collapse is a property of the \emph{form}
$Wx + b$ rather than of any of the numbers in it, which is why one identity
covers every stack anybody builds.

Two steps made that rearrangement work. One was named in section 1. Say which
two, and where the other one came from.
\dotline
\nextframe
\end{fr}

\begin{fr}
\ans{$W_2$ distributes over the sum, and $W_2(W_1x)$ is $(W_2W_1)x$ because
the product \emph{is} the composition.}
The first is additivity, one of the two properties that made the function
linear. The second is not the other one: no scalar appears anywhere in the
rearrangement. It is section 2's definition of the product, which is the
sentence this whole program is built on doing the work again.
Which is worth pausing on, because a stack of these is a large object.

Eight \emph{affine} layers of width $\val{p06.stack.width}$ \dash{} weights and
biases, counted the way a framework reports them \dash{} hold
...
So say what the depth in a stack of purely linear layers is buying.
\dotline
\nextframe
\end{fr}

\begin{fr}
\ans{Nothing. \textbf{The waste is exactly the depth you thought you were
buying.}}
...
So say what a bent line rules out, and why that is stronger than having failed
to find a single affine map that matches.
\dotline
\nextframe
\end{fr}

\begin{fr}
\ans{Every affine map at once. \textbf{No single affine map reproduces it,
whatever that map might be} \dash{} bending a line is something none of them
can do, so nothing is left to search.}
\begin{aibox}
...
It is there because a composition without it is provably a waste of $(k-1)/k$
of the parameters \dash{} $\val{p06.stack.wasted.pct}\%$ at eight layers, half
at two \dash{} and any function that breaks linearity will do. That is why the
field could swap the logistic for $\relu$
...
the middle and never form the outer product $BC$.
...
% ============================================================
\begin{summarybox}
\sumitem{1--3}{\result{\textbf{A matrix is a function, not a rectangle.} It takes a
  ...
  matrix}.}
\sumitem{4--6}{\result{\textbf{\enquote{Linear} means those two properties, not
  ...
  dimension down}.}
\sumitem{7--8}{\result{\textbf{A linear map is determined by what it does to a basis},
...
\sumitem{9--11}{\result{\textbf{The product $AB$ is the composition} \dash{} do $B$,
  ...
  forgotten}.}
\sumitem{12--14}{\result{\textbf{Associativity is free}, because composing functions
...
\sumitem{15--17}{\result{\textbf{The order matters, and one dimension cannot show it},
...
\sumitem{17--19}{\result{Of $\val{p06.commute.trials}$ random $3 \times 3$ pairs,
  ...
  about one centre, and rotations in three dimensions about \emph{one} axis
  \dash{} and \textbf{rotations in three dimensions about different axes do
  not}, which is why two dimensions is the misleading case}.}
\sumitem{20--22}{\result{\textbf{Shape is the domain and the codomain written down}, so
...
\sumitem{23--25}{\result{A vector is an $n \times 1$ matrix, so $Ax$ needs no second
...
\sumitem{26--29}{\result{\textbf{Two affine layers are one}: $W_2(W_1x + b_1) + b_2 =
...
\sumitem{30--32}{\result{\textbf{An affine map cannot bend a straight line}, so three
...
\sumitem{33--34}{\result{\textbf{Associativity is free and bracketing is not}: the same
...
\item Collapse $W_2(W_1x + b_1) + b_2$ into a single affine map.
  ...
  distributing $W_2$ over the sum and then reading $W_2(W_1x)$ as $(W_2W_1)x$,
  which is the product defined as composition.}
