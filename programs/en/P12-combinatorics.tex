...
% ============================================================
\begin{summarybox}
\sumitem{1--2}{\textbf{Two questions decide which rule applies}: does the
  ...
  both are settled.}
\sumitem{3--4}{Two of the four counts, of $\val{p12.four.k}$ from
...
\sumitem{4--5}{The product rule needs \textbf{the number of options at each
  ...
  product at all.}
\sumitem{6}{$\binom{n}{k} = \val{p12.four.unordnorep}$ is the arrangements
...
\sumitem{7--8}{The fourth cell, unordered with repeats, is
  ...
  more than eight.}
\sumitem{9--10}{$\val{p12.four.n}!$ is $\val{p12.fact.n}$ orderings, and
...
\sumitem{21--23}{\textbf{Inclusion--exclusion: the signs alternate because each
...
\sumitem{24--26}{\textbf{Pigeonhole: $m$ items in $n$ boxes put at least
...
\sumitem{27--28}{\textbf{The birthday count is over pairs, not items.}
...
\sumitem{29--31}{Two formulas, each right where the other fails: the exact
...
\sumitem{32--35}{\textbf{A hash's safe capacity is about the square root of its
...
\sumitem{36--38}{$\val{p12.beam.vocab}^{\val{p12.beam.len}}$ is
  ...
  helps.}
\sumitem{39--43}{\textbf{A Shapley value averages over every ordering, and the
