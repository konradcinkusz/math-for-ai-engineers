...
what happens to that count as the room grows.}{one item against all, or any two}
\end{fr}
...
\item A beam of width $b$ over $L$ steps and a vocabulary of $V$ scores
  ...
  \answerto{$b = \num{0.01}\,V^{L}/(LV)$, which is
  $\val{p12.beam.pct.width}$. The point of the exercise is that the number is
  not merely large but unreachable, so no width is a coverage argument.}
