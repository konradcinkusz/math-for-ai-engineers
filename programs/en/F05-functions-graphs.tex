...
done section 3's work a section early, and you did it without being told a
rule.
...
right it is the line $y = x$ at forty-five degrees. The two halves meet at the
...
This is the fourth sign this reasoning has arrived under. Program~\ref{prog:F01}
...
from the horizontal axis and makes it steeper. The bias $b$ is added outside,
...
Now forbid the bias, so the model is $y = wx$ and nothing else. The
least-squares line of that form has slope $\val{f05.nobias.slope}$, and its
worst miss against the three points is $\val{f05.nobias.worst}$.
...
A function is \textbf{strictly increasing} when it never gives the larger input
the smaller output: whenever $a < b$, it is also true that $f(a) < f(b)$. Not
...
That is also why the loss you train on may be scaled, shifted by a constant, or
have its logarithm taken, and the best model is the same model. It is not why
...
% ============================================================
\begin{summarybox}
\sumitem{1--4}{\textbf{A function is a machine with one output per input.} Two
...
\sumitem{4--5}{The \emph{domain} is the set of inputs the machine accepts, and
...
\sumitem{6--7}{$f$ is the machine and $f(x)$ is one number it produced.
  ...
  than as an error.}
\sumitem{8--10}{The graph is the set of points $(x, f(x))$ drawn, and a value,
  a crossing and a direction are all readable off it without arithmetic.}
\sumitem{10--11}{\textbf{\enquote{Increasing} is a statement about an
...
\sumitem{12--13}{The five shapes the book leans on: line, parabola, V,
  $\relu$'s half-V and $\frac{1}{x}$. $\relu$ is flat for $x \le 0$ and meets
  the other half in a corner.}
\sumitem{14--16}{\textbf{Where the number is written decides which way it
  ...
  picture.}
\sumitem{18--21}{$a f(x)$ stretches away from the horizontal axis by $a$;
...
\sumitem{22--23}{The moves do not commute: $2f(x) + 1$ and
  $2\bigl(f(x)+1\bigr)$ are $7$ and $8$ where $f(x) = 3$. Ordinary precedence
  is the whole rule.}
\sumitem{24--26}{$y = wx + b$ is a stretch and a shift applied to $f(x) = x$.
  $\sigma(0) = \num{0.5}$ exactly, and $\sigma(-x) = 1 - \sigma(x)$, so the
  curve has a half-turn symmetry about $\bigl(0, \frac{1}{2}\bigr)$.}
\sumitem{27--28}{In $\sigma(wx + b)$ the weight sets the steepness: the band
  ...
  weights.}
\sumitem{29--30}{Without a bias every line passes through the origin, and no
...
\sumitem{31--33}{$f(g(x))$ is read from the inside out, and the order is part
...
\sumitem{34--35}{A deep network is a composition and nothing else, which is
...
\sumitem{36--38}{An inverse runs the machine backwards and exists only for a
  one-to-one function; $x^{2}$ has none over all the reals, for frame $2$'s
  reason. Its graph is the original reflected in $y = x$.}
\sumitem{39--40}{The logit $\ln\frac{p}{1-p}$ inverts the logistic \dash{} it
...
\sumitem{41--44}{\textbf{A strictly increasing function keeps the order of a
...
\sumitem{43}{Temperature changes the distribution a great deal and the top of
...
\sumitem{46--47}{$\ln$ is strictly increasing, so a decoder may compare
