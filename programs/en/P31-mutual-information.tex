...
One shuffled run is one draw from the distribution those enumerated tables
came from. The table printed its mean; the same enumeration has the rest of
it, and it is wide.

At $N = 10$ its spread is $\val{p31.null.sd10}$ nats against a mean of
$\val{p31.bias.n10}$. The spread is larger than the quantity it is being used
to measure, and about one run in twenty reports $\val{p31.null.q95}$ nats or
more \dash{} on data with nothing in it. That holds at every $N$ the table
covers, not just the smallest.

\begin{trapbox}
\textbf{\enquote{I ran the shuffled control and subtracted it.}}

The control is a random variable and one run is one sample of it, so the
difference you report can come out negative, or twice the real excess, with
nothing in the run to say which happened.
