...
\outcome{11--15}{write mutual information as a difference of entropies, in
  either of the two directions, and say what each of the two pieces measures}
\outcome{16--26}{say which way a plug-in estimate is wrong on data with no
  dependence, and how many items it takes for that to stop mattering}
\outcome{27--35}{say what post-processing can and cannot do to information,
  and read the same inequality from both of its ends}
\outcome{36--39}{say what would have to be measured before an
...
\item Write mutual information as a difference of entropies. Is there more
  than one way to do it? \teachesat{14--15}
  \answerto{Two ways: $H(Y) - H(Y \mid X)$ and $H(X) - H(X \mid Y)$. They are
  the same number, and the pieces they are built from are not the same
  pieces.}
...
\item Somebody improves their probe and reports a higher score. What does
  that say about the layer? \teachesat{29--30}
  ...
  about the probe.}
\item And if a probe scores badly? \teachesat{31--32}
...
values, and $W$ taking $0$ with probability $\frac{1}{3}$ and $1$ with
...
\mermaidfig{p31-what-a-margin-drops}{What the two margins throw away}{%
  what margins alone cannot rebuild}
\end{fr}
...
Checked rather than asserted: over all $\val{p31.sym.trials}$ non-zero count
patterns of a $2 \times 3$ table with cells in $\{0, 1, 2, 3\}$, each
normalised to a distribution, not one comes out negative.
\end{fr}
...
Checked over the same $\val{p31.sym.trials}$ tables, with the two answers
...
the pair, less the uncertainty in $X$ alone. The first term is
Program~\ref{prog:P29}'s entropy with the \emph{pair} as the outcome, which
needs no new definition \dash{} a pair is a thing that happens, and that
program's sum runs over whatever happens.
...
\begin{fr}
Both of those conditional entropies reach the same mutual information, and
that is two lines of arithmetic rather than a new fact. Split the logarithm
in the definition:
\[ I(X; Y) \;=\; \sum_{x, y} p(x, y) \ln \frac{p(x, y)}{p(x)\,p(y)}
   \;=\; H(X) + H(Y) - H(X, Y) \]
and the conditional entropy above says $H(X, Y) - H(X)$ is $H(Y \mid X)$.
Substitute it, then do the same with the letters the other way round:
...
\mermaidfig{p31-two-routes-one-number}{Two ways to the same quantity}{%
  a gap, and a drop in uncertainty}
\end{fr}
...
$N = 100$ it leaves under two per cent of the bias behind and the bias it is
removing is $\val{p31.bias.n100}$ nats, which nobody needed removed. At
$N = 10$ it leaves $\val{p31.bias.left10}$ per cent of it behind, and that is
the regime the correction was reached for.
...
hundred, then you are far below the $\val{p31.big.per}$ items per cell the
formula needed, so the formula does not apply and the qualitative answer is
worse rather than better: whatever the bias is, it is of the order of the
quantity itself. Nothing in the number itself would say so.

There is a check that costs nothing but the compute, and it is the difference
...
The labels keep the distribution they had and lose every relationship to the
input, so whatever that run reports was produced by your bins, your sample
size and your estimator on data with nothing in it.

It is almost never done, and it is cheap.

Now the question the recipe hides, and it is worth answering before you read
on. How many times do you run it?
\dotline
\nextframe
\end{fr}

\begin{fr}
\ans{More than once, and the enumeration above says why}

A shuffled run is one draw from a null distribution. The enumeration above
is the one place in this program where you can see the whole of such a
distribution rather than one sample of it, and the table printed only its
mean.

At $N = 10$ its spread is $\val{p31.null.sd10}$ nats against a mean of
$\val{p31.bias.n10}$, and about one run in twenty reports
$\val{p31.null.q95}$ nats or more \dash{} on data with nothing in it. The
spread exceeds the mean at every $N$ the table covers, so a run that reports
several times the floor is not an artefact of the smallest sample.

\begin{trapbox}
\textbf{\enquote{I ran the shuffled control and subtracted it.}}

The control is a random variable and one run is one sample of it, so what
you report is the real excess plus however far that single draw fell from
the bias it was standing in for \dash{} a quantity as large as the bias
itself. It can come out negative, and nothing in the run says which
happened.

Shuffle a number of times instead, and read where your observed estimate
sits among the shuffled ones rather than subtracting any single one of them.
That move is Program~\ref{prog:P27}'s under a different name: build the
distribution of a statistic under a null in which the effect has been
removed, then read the observed value's position in it. There the null was
that two models are equally good and every outcome was enumerated; here it
is that the labels have nothing to do with the input, and the runs are the
enumeration.
\end{trapbox}

\mermaidfig{p31-bias-is-not-signal}{Where a plug-in estimate comes from}{%
  signal or sample}
\end{fr}
...
Take a guess before you read on. Can $I(X; Z)$ exceed $I(X; Y)$?
\dotline
\nextframe
\end{fr}
...
Go back to the $\val{p31.dpi.channels}$ enumerated channels \dash{} every
probe in that family. The best of them is a real number, and it is not
$\val{p31.dpi.ixy}$.
...
The family enumerated above is every channel from a three-valued $Y$ to a
two-valued $Z$ on that grid, and it contains all $\val{p31.dpi.det}$ of the
maps that make the choice outright rather than at random.
...
The best choice loses least; no choice loses nothing. That is not an
argument about the eight alone, either: the best of all
$\val{p31.dpi.channels}$ enumerated channels turns out to be one of them, so
randomising the choice does not buy back what merging cost.
...
A network computes each layer from the layers before it and from nothing
else, so $X \to (\text{everything before layer } k+1) \to \text{layer } k+1$
and the inequality applies \dash{} through a residual connection as much
as through a plain chain:
...
\mermaidfig{p31-probe-is-a-floor}{What a probe's score bounds}{%
  a score is a floor}
\end{fr}
...
\item \textbf{The shuffled-label floor}: the same pipeline run on labels with
  no relationship to the input, several times, and where the reported
  estimate sits among those runs. One of them is a draw from a wide
  distribution rather than the floor itself.
...
most disputes and it costs nothing but the compute to run the pipeline
again.
\end{fr}
...
\begin{summarybox}
\sumitem{1--6}{\textbf{\result{A zero correlation settles nothing}}:
...
\sumitem{7--8}{\textbf{\result{So it is never negative and is zero exactly under
  independence}}, both inherited from Program~\ref{prog:P30} with no new
  proof.}
\sumitem{9--10}{\textbf{\result{It is symmetric}}, because swapping the two
  quantities leaves both arguments of the divergence unchanged.}
\sumitem{11--15}{\textbf{\result{The pieces it is built from are not}}: on the same
  pair $H(W \mid Z) = 0$ and $H(Z \mid W) = \val{p31.zw.hzw}$, and both
  routes reach $\val{p31.zw.mi}$.}
\sumitem{16--19}{\textbf{\result{The plug-in estimate is positive on independent
...
\sumitem{20--21}{\textbf{\result{The standard correction fails worst where it is
  needed most}}: $\val{p31.bias.ratio10}$ times off at $N = 10$, converging to
  $\val{p31.bias.ratio100}$ by $N = 100$.}
\sumitem{22--23}{\textbf{\result{Sixteen categories each way needs
  ...
  cell.}
\sumitem{24--26}{\textbf{\result{The check is a shuffled-label run, and one of
  them is a draw rather than a floor}}: its spread $\val{p31.null.sd10}$ nats
  exceeds its own mean $\val{p31.bias.n10}$ at $N = 10$, so shuffle several
  times and read where the observed estimate sits among them.}
\sumitem{27--28}{\result{\textbf{Post-processing cannot create information}: if
  $X \to Y \to Z$ then $I(X; Z) \le I(X; Y)$}, checked against all
  $\val{p31.dpi.channels}$ channels in a family.}
\sumitem{29--34}{\textbf{\result{So a probe's score is a lower bound}}, and
  the best probe in that family still leaves more than a quarter behind.
  A better probe says nothing about the layer, and neither does a worse one.}
\sumitem{36--39}{\textbf{Nothing here was measured on a real model}, and the
...
\item $X$ is uniform on $\{-2, -1, 1, 2\}$ and $Y = \lvert X \rvert$.
  ...
  Program~\ref{prog:P24}'s pair, with $X$ on four values instead of three and
  $Y$ on two as $W$ was \dash{} but equally likely, which is why $H(Y)$ is
  $\ln 2$ where $H(W)$ was $\val{p31.zw.mi}$.}
...
\item You compute a plug-in mutual information of $\num{0.05}$ nats between
  ...
  and $\val{p31.bias.n100}$ nats there, so about a fifth of $\num{0.05}$ is
  what the estimator returns on nothing \dash{} and an expectation is not a
  ceiling, so the tail of that distribution reaches higher still. Shuffle the
  labels and re-run several times, and report where $\num{0.05}$ sits among
  what those runs return.}
...
\item A pipeline bins a continuous activation into $\val{p31.big.k}$ buckets
  ...
  \answerto{Only one axis moves: the label is still
  $\val{p31.big.k}$-way, so the cells go from $\val{p31.big.cells}$ to
  $\val{p31.big.cells.finer}$ and $(a-1)(b-1)$ from $\val{p31.big.ab}$ to
  $\val{p31.big.ab.finer}$. The items needed to hold the bias at one per cent
  of the maximum go from $\val{p31.big.need}$ to $\val{p31.big.need.finer}$
  \dash{} about twice, not four times. Finer bins buy resolution and pay for
  it in items, and the payment is usually not made.}
