...
\label{prog:P13}
\index{graph (vertices and edges)}
\index{graph (vertices and edges)!adjacency matrix}
...
\outcome{27--34}{read a stationary distribution as an eigenvector, and say what
  a vertex with no outgoing edge does to one}
\end{outcomes}
...
\item $\val{p13.big.n}$ vertices and $\val{p13.big.m}$ edges. Roughly how many
...
\item What does the $(i,j)$ entry of $A^{2}$ count? \teachesat{14--15}
  ...
  $\sum_k A_{ik}A_{kj}$ has a term of $1$ for each intermediate vertex joined
  to both, which is Program~\ref{prog:P06}'s composition doing arithmetic.}
\item A directed graph with no cycle has how many topological orders?
  ...
  six-vertex graph in this program, out of $\val{p13.topo.total}$ orderings.
...
\label{sec:P13-what}
\index{graph (vertices and edges)!definition}

\begin{fr}
Program~\ref{prog:F10} gave you the set, and said nothing at all about
graphs. This program needs one thing beside the set \dash{} a set of pairs of
its members \dash{} and everything else follows from those two. Unlike the
last program it does lean on Part~III: by section 3 the whole subject will
have turned into matrix multiplication.
...
and from nothing outside, and finding the components is how a corpus gets
deduplicated: keep one document from each.

That is exactly right when \enquote{is a duplicate of} is transitive, and
near-duplicate matching usually is not. $A$ can match $B$ and $B$ match $C$
while $A$ and $C$ have nothing in common, so a component chains together
documents no pair of which matched \dash{} which is how a deduplication pass
quietly removes a whole topic.
...
\index{graph (vertices and edges)!adjacency list}
...
Read the graph undirected for this section, as the deduplicated corpus above
was: an edge joins its two endpoints rather than running from one of them to
the other, so it sits in the list of each and the matrix comes out symmetric.
The definitions are the general ones, and the sections on DAGs and on random
walks need them that way; what the reading changes is not what an encoding is
but how much of one an edge occupies.
...
\begin{fr}
So the list wins on memory, wins on neighbours, and \emph{loses} the third
question: the matrix answers \emph{is there an edge from $u$ to $v$} in one
step, where the list walks $u$'s neighbours \dash{} at this section's scale,
$\val{p13.big.avgdeg}$ of them on average. That is the matrix's only win, and
paying for it in whole rows is why nobody buys it. The matrix would be an odd
thing to keep at all \dash{} except for one property that has nothing to do
with looking anything up.
...
\label{sec:P13-walks}
\index{graph (vertices and edges)!walk}
...
The distinction is not a nicety. A matrix power counts every sequence of
edges that fits end to end, and there is no arithmetic anywhere in a matrix
product that could skip the sequences which come back to a vertex they have
already used \dash{} a product has no memory of where it has been. So
everything this section proves is about walks by construction, and counting
\emph{paths} is a genuinely harder problem that no power of $A$ answers.

And the walk question is nearly always the one you have. Asking whether a
change here can reach a service $k$ steps away, or how much of the graph a
signal touches after $k$ rounds, is asking about sequences of edges laid end
to end and nothing more: a route that doubles back has wasted a step and
still arrives. The path question \dash{} how many distinct simple routes are
there \dash{} is the one that is rarely asked and expensively answered, so
the cheap machinery this section is about is pointed at the question that
comes up.
...
Before you answer the next one, notice two things about how it is put. The
diagonal is the part of $A^{2}$ nobody looks at, because \enquote{walks from
$i$ back to $i$} sounds like a triviality \dash{} and the theorem covers it
with no exception, so whatever the entry says is forced rather than chosen.
And the word \emph{undirected} is load-bearing: the answer moves if you drop
it, so work out what that word is buying before you commit.
...
$A + I$ replaces each vertex's row by the sum of its neighbours' rows and its
own, and $\sigma$ is the nonlinearity Program~\ref{prog:P06} showed the
composition would collapse without.

The $I$ is the self-loop such a network adds before it aggregates, and it is
not decoration: under $A$ alone a vertex's new row is built out of its
neighbours and not out of itself, so a layer would replace what a vertex knows
...
Because $k$ layers apply $A + I$ $k$ times, and the walk-counting theorem
says what a matrix power's non-zeros are: the pairs joined by a walk of length
\textbf{exactly} $k$. That is the theorem arriving as a statement about
architecture \dash{} and the $I$ is what turns \emph{exactly} into
\emph{within}, because a step spent on a self-loop stands still, so a walk of
any length up to $k$ is padded out to $k$ and counted.
...
\label{sec:P13-dag}
\index{graph (vertices and edges)!directed acyclic}
...
retracing an edge and without passing through any other vertex twice. A
directed graph with none is \textbf{acyclic}, and a directed acyclic graph is
a \emph{DAG}.

Both clauses earn their place, and they exclude different things. Every
undirected graph with even one edge has a walk that leaves a vertex and comes
back \dash{} the \enquote{out and back} the trapbox named \dash{} so without
the clause about edges nothing would be acyclic and the word would mean
nothing. On a directed graph that clause is doing less work, because following
an edge back is only possible if the edge back exists; \emph{that} is the
reason acyclicity is a statement people make about directed graphs and almost
never about undirected ones. The clause about vertices excludes something the
other one does not: two loops joined at a single point, walked as one closed
route. It repeats no edge, so the clause about edges would let it through, and
it is a pair of cycles rather than one.
...
vertices. Unrolling is what gives reverse mode a topological order to work
in, and it is why the memory grows with the number of steps unrolled.
\end{trapbox}
\end{fr}
...
\index{graph (vertices and edges)!random walk}
...
which needs that sum to be positive. A vertex with no outgoing edges has
nothing to divide, and its row stays as it was: all zeros.
...
and the reading that program gives applies unchanged \dash{} repeatedly
applying a matrix multiplies each eigendirection by its own factor raised to
that power, so the one whose factor leaves the vector alone is what a long
walk settles on. An iterative method for finding it, and the rate such a
method would settle at, are in neither that program nor this book.
...
\begin{fr}
That is \textbf{PageRank}\index{graph (vertices and edges)!PageRank}, and the
definition is the whole of it: a page's
...
unique, and with it the matrix is positive, which is what the
Perron--Frobenius theorem asks for.
\end{note}
\end{fr}
...
nothing to check and buys determinism. \emph{Where does a random walk settle}
...
\sumitem{19--20}{\result{\textbf{Message passing is a multiplication by the adjacency
  matrix}, so $k$ layers reach within $k$ hops once the layer adds the
  self-loop: $\val{p13.reach.one}$
