...
\item A plain chain of $\val{p32.depth}$ layers has a gradient that is a
  product of one factor per layer, each at most $\frac{1}{4}$. What does
  adding a residual connection to each layer do to that? \teachesat{23--24}
...
\index{attention!divisor checked@the $1/\sqrt{d_k}$ divisor, in a block}
...
\mermaidfig{p32-score-assembled}{Where a score comes from}{%
  two projections that appear only as a product}
...
same product at a \emph{saturated} layer rather than at its best, and gets a
product under $10^{-\val{f12.sat.exponent}}$ \dash{} below every floor that
format has. Then the early layers receive exactly zero, and receive it
silently.
...
\mermaidfig{p32-where-parameters-are}{Where a block's parameters sit}{%
  four and eight}
...
\sumitem{3--5}{\result{Attention's parameters are four projections; a score is one
...
\sumitem{7--8}{The score's divisor is Program~\ref{prog:P25}'s variance
  ...
  handed it here.}
\sumitem{9--10}{It does, exactly. \result{A score is a bilinear form
...
\sumitem{11--12}{\result{But a trained model has one weight draw and not an average,
...
\sumitem{13--16}{\result{The $\softmax$ makes the output a convex combination
  (Program~\ref{prog:P19}), so a purely attentional layer cannot produce a
  direction its values do not already span}.}
\sumitem{18--20}{\result{Multi-head attention is free: $\val{p32.heads}$ heads of
  width $\val{p32.d.head}$ hold exactly the parameters of one head of width
  $\val{p32.d}$. What changes is rank, not cost}.}
\sumitem{21--22}{\result{A plain chain of $\val{p32.depth}$ layers has a gradient of
  at most $\val{p32.plain.bound}$, and that is its \emph{best} case. The number
  is still representable \dash{} $\code{float32}$'s smallest normal is
  $\val{p01.fp32.minnorm}$ \dash{} so what kills a plain chain is that the
  bound is useless rather than that it underflows. Underflow is the realistic
  case: at a saturated layer the same product falls under
  $10^{-\val{f12.sat.exponent}}$, below every floor the format has}.}
\sumitem{23--25}{\result{$y = x + f(x)$ makes the Jacobian $\prod(1 + f'_k)$, whose
...
\sumitem{28}{\result{A rotation makes a score depend on the distance between two
  positions and nothing else, and costs the model nothing because it is
  orthogonal} (Programs~\ref{prog:F08} and \ref{prog:P09}).}
\sumitem{29--32}{\result{Attention is $4d^{2}$ and the feed-forward block is
  ...
  $\val{p32.f01.params}$.}
\sumitem{33--35}{\result{The cache is one key and one value per position per layer,
