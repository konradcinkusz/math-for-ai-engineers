...
and $3y$ differentiates to $3$, so the second term survives too.

\begin{trapbox}
Look at what has happened to the two answers. $\frac{\partial f}{\partial x}$
...
derivative is zero at the bottom of a curve. What is the gradient at a point
where $f$ has a minimum, and why must \emph{every} component vanish?
\nextframe
\end{fr}
...
Here $\vect{p}$ is the point you are standing at, written as a vector so that
$\vect{p} + hu$ means a step of length $h$ from it; the book keeps that letter
for a point from here on.
...
\begin{rigourbox}
\textbf{\enquote{The limit is linear in the direction} is a hypothesis, not an
observation.} It is what \emph{differentiable at the point} means, and it is
doing all the work in the step above. Having both partial derivatives is not
enough on its own.

The standard counterexample is worth seeing once, because it is nastier than
it looks:
\[ f(x, y) = \frac{x\,y^{2}}{x^{2} + y^{4}}, \qquad f(0, 0) = 0 \]
Both partials at the origin are zero, because $f$ is zero along each axis. So
the dot product predicts a rate of zero in every direction. The rate along a
unit direction $(a, b)$ with $a \neq 0$ is in fact $b^{2}/a$, which the script
checks at three directions \dash{} unbounded as $a$ shrinks, and zero only
along the $x$ axis.

Every function this book differentiates is differentiable where it does so, so
the identity holds throughout. What differentiability is, and how to check it,
is a first course in analysis.
\end{rigourbox}

\transcript{p15-gradient}

\mermaidfig{p15-any-direction}{One number per direction, and where it comes
from.}{the rate along a unit vector}
\end{fr}
...
It is smaller than the gradient's own length of $\val{p15.grad.len}$. That is
not a coincidence and the next section is about why.
...
which is exact because $\cos\theta$ is never above $1$; and that the sampled
best falls short by no more than the resolution. The second turns the
shortfall from an embarrassment into an explanation.

The first half used to be attributed to an inequality of
Program~\ref{prog:P05}'s, and this book states no such inequality anywhere.
What it has is the identity above, and a cosine that cannot exceed one, which
...
bowl and in a long one.}{why the perpendicular misses the centre}
\end{fr}

% ============================================================
\section{What it does to the walk}
\label{sec:P15-zigzag}
\index{gradient descent!derived}
\index{gradient|seealso{gradient descent}}
...
$(\val{p15.zig.start.x}, \val{p15.zig.start.y})$ on the bowl the next frame
writes down, the value is $\val{p15.sign.before}$. One step of size
$\val{p15.sign.eta}$ against the gradient takes it to $\val{p15.sign.down}$.
One step of the same size \emph{with} the gradient takes it to
$\val{p15.sign.up}$.
...
and every number in this section comes from those two expressions, so a reader
who wants to check one has everything needed to.

\textbf{The half is not decoration.} Section~\ref{sec:P15-steepest} wrote a
quadratic as $ax^{2} + by^{2}$, whose gradient is $(2ax, 2by)$ \dash{} so
there the coefficient is half the curvature and the two conventions differ by
a factor of two in every gradient. With the half in front, the coefficients
\emph{are} the eigenvalues, the gradient is each eigenvalue times its own
coordinate, and a step multiplies that coordinate by $1 - \eta\lambda$ rather
than by $1 - 2\eta\lambda$. That is the form the rest of this section and the
whole of Program~\ref{prog:P17} are written in, and it is why it is worth
paying the half.
...
axes. Section~\ref{sec:P15-steepest} has already settled whether the negative
gradient there points at the minimum, so the question worth asking is the
other one. Before computing anything: roughly how far off is it \dash{} a
degree or two, or something you would notice?
\nextframe
\end{fr}

\begin{fr}
\ans{$\val{p15.zig.angle}$ degrees away from it.}
...
\textbf{That is not a rounding error, it is nearly half a right angle.} The steering is not slightly off; it is pointing somewhere else
...
$\val{p15.zig.pathlen}$ of path to cover $\val{p15.zig.moved}$ of ground, so
$\val{p15.zig.sideways.pct}$ per cent of the walking is sideways \dash{} which
is the two figures above divided, and the script checks that it is.
...
is exactly the property this section is about, and it is $1$ for the round bowl
to the last bit \dash{} the control being run at
$\lambda = \val{p15.lam.lo}$, so that $1 - \eta\lambda$ is positive and each
step lands short of the centre rather than past it. Raise the step above
$1/\lambda$ and the walk still converges and still never leaves the line, but
it lands on the far side every step, so the path stops equalling the
displacement while the direction is as exact as ever.
\end{note}
\end{fr}
...
by a fifth. The shallow one is multiplied by a positive number a little under
one, so it creeps towards zero without ever crossing.
...
That sentence needs the alternating sign this section has just measured,
which is why this program produces the problem and leaves it standing.
\end{rigourbox}
\end{fr}
...
\sumitem{8--9}{\result{\textbf{The gradient is the partials written as a vector},
  ...
  $\val{p15.grad.len}$}.}
\sumitem{9}{\result{\textbf{The word does two jobs.} Programs~\ref{prog:F06}
  and~\ref{prog:F11} use
...
\item On a bowl with contours that are circles, how many times does gradient
  descent cross from one side to the other before arriving \dash{} and what
  does your answer depend on?
  \answerto{None, provided $\eta < 1/\lambda$, so that $1 - \eta\lambda$ is
  positive. The negative gradient points exactly at the centre, so every step
  is along the same straight line and the path is $1$ times its own
  displacement \dash{} which the script asserts exactly, at a step of that
  size. \textbf{It is not true for every converging step.} Between $1/\lambda$
  and $2/\lambda$ the walk converges and still never leaves the line, but each
  step lands past the centre, so it crosses on every one of them. Crossing the
  valley \emph{sideways} is the different thing, it needs the contours to be
  non-circular, and that is the whole content of section 5.}
\end{testexercises}
...
\item You have a bowl whose eigenvalues are $\lambda$ and $1$, and you double
  ...
  \textbf{It does not grow without limit, and the ceiling is not a right
  angle.} From this start point it climbs towards $45$ degrees, which is the
  angle between $(\lambda, 1)$ and $(1, 1)$ as $\lambda$ runs away, and the
  script asserts that too. What holds on any convex bowl is that the angle
  stays \emph{acute}, because $\nabla f \cdot \vect{p} = 2f$, which is
  positive anywhere but the minimum.}
\end{furtherproblems}
