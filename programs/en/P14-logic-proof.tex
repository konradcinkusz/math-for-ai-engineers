...
\item A bound holds \enquote{with probability at least $\val{p14.conf}$ per
  cent}. You use it $\val{p14.uses}$ times, on independent draws. What does
  that guarantee give you for all of them together? \teachesat{28--29}
  \answerto{At least $\val{p14.allhold}$ per cent, and nothing better: it
  leaves room for a failure somewhere $\val{p14.anyfails}$ per cent of the
  time. A confidence quoted once is not a confidence in the conclusion you
  build out of twenty of them.}
...
\item A claim is checked on forty consecutive inputs and holds every time.
  ...
  \answerto{That it holds on those forty. $n^{2}+n+41$ is prime for
  $\val{p14.euler.first}$ consecutive values and composite at
  $n = \val{p14.euler.first}$, so forty confirmations can be worth nothing.}
\item A convergence theorem assumes the objective is convex. A paper cites it
  ...
  convex \dash{} so the theorem does not apply at all.}
\end{quiz}
...
dropped.}{hypotheses, conclusion, quantifiers}
\end{fr}
...
work. An undirected edge points both ways, so each of its endpoints would have
to come after the other and no order exists \dash{} and yet, by the definition
Program~\ref{prog:P13} gives, that graph has no cycle at all, because the walk
out and back retraces its one edge. Acyclic and with no topological order: so
without that word the theorem is false and the whole of section 4 of
Program~\ref{prog:P13} evaporates. It is also the word a reader is likeliest
to skip, because it sits before the noun and reads like scenery.
\end{fr}
...
then $Q$, written $P \Rightarrow Q$ and read \emph{$P$ implies $Q$}.
Program~\ref{prog:F10} gave \emph{and}, \emph{or} and \emph{not}; this is the
fourth connective and it is the one people get wrong.
...
same claim and one is a different one.}{converse against contrapositive}
\end{fr}
...
\begin{note}
Program~\ref{prog:F10} measured a negation pushed through the wrong
connective in a filter, which is the same move one connective over:
...
\begin{note}
Only one direction is safe, and it is worth knowing which.
\textbf{There-is-a-$y$-for-all-$x$ always implies
for-all-$x$-there-is-a-$y$}, never the reverse:
...
\begin{quote}
For every continuous function $f$ on a closed bounded region, every
tolerance $\varepsilon > 0$, and any fixed activation that is not a
polynomial, there exists a network with one hidden layer using that
activation whose output is within $\varepsilon$ of $f$ everywhere on that
region.
\end{quote}
...
\begin{fr}
\begin{ansblock}
For-all, then there-exists. So the network is allowed to depend on $f$, on
$\varepsilon$ and on the activation you fixed \dash{} and does.
\end{ansblock}
...
what it is silent about.}{what an existence claim is silent about}
\end{fr}
...
also approximates any continuous function on a closed bounded region, and
...
Two of the hypotheses go missing more often than the quantifier does, and both
are cheap to falsify. \emph{Closed} is doing work: a continuous function on an
interval with its ends left off need not be bounded at all, and $1/x$ between
$0$ and $1$ takes every value above $1$, so no network of any size comes
within any $\varepsilon$ of it. \emph{Not a polynomial} is doing work too:
with the identity activation a one-hidden-layer network is affine in its
input, the best affine approximation to $x^{2}$ on $\intcc{0}{1}$ is
$x - \frac{1}{8}$, and its worst error is exactly $\frac{1}{8}$, so
$\varepsilon = \num{0.1}$ is out of reach however many units are added.
Neither is a detail. \emph{What a theorem is made of} counted the hypotheses
as one of the three parts, so a statement quoted without them is a different
statement.
...
\begin{trapbox}
\textbf{It is prime for $\val{p14.euler.first}$ consecutive values from
$n = 0$, and composite at $n = \val{p14.euler.first}$}, where it is
...
draws are independent, what does that guarantee give you for all
$\val{p14.uses}$ holding together?
\dotline
\nextframe
\end{fr}

\begin{fr}
\ans{At least $\val{p14.allhold}$ per cent}
The premise is a floor rather than a probability, so the product is a floor
too and the guarantee gives nothing better. Read the other way, it leaves
room for at least one of the $\val{p14.uses}$ claims to fail as often as
$\val{p14.anyfails}$ per cent of the time: \textbf{the guarantee permits a
false statement among them more often than not}, and nothing in the paper
marks which.
...
\begin{tabular}{ll}
\toprule
uses before the guarantee is a coin flip & $\val{p14.flip}$ \\
per-use confidence for $\val{p14.joint}$ per cent across $\val{p14.uses}$
  & $\val{p14.needed}$ per cent \\
\bottomrule
\end{tabular}
\end{center}

\textbf{A $\val{p14.conf}$ per cent bound guarantees no better than a coin
flip after $\val{p14.flip}$ uses}, and to guarantee $\val{p14.uses}$ of them
together at $\val{p14.joint}$ per cent you would need each to hold at
$\val{p14.needed}$ per cent \dash{} a failure probability of
$\val{p14.neededdelta}$ per cent rather than $5$, nearly twenty times
tighter.
...
% ============================================================
\begin{summarybox}
\sumitem{1--2}{\result{\textbf{A theorem is hypotheses, a conclusion and the
...
\sumitem{3--4}{\result{\enquote{If and only if} is \textbf{two} theorems travelling
...
\sumitem{5--6}{\result{\textbf{The commonest misuse is to keep the conclusion and drop
...
\sumitem{8--9}{\result{\textbf{$P \Rightarrow Q$ is false in exactly one of the
  $\val{p14.rows}$ rows} \dash{} $P$ true and $Q$ false. It says nothing about
  cause and nothing about the case where $P$ fails}.}
\sumitem{9--10}{\result{\textbf{The converse is a different claim}, disagreeing in
...
\sumitem{10--11}{\result{\textbf{And $\lnot P$ tells you nothing about $Q$.}
...
\sumitem{11--12}{\result{\textbf{The contrapositive is the same claim}, in all
  $\val{p14.rows}$ rows, which is why proving it proves the original}.}
\sumitem{13--14}{\result{\textbf{A counterexample is one case with the hypothesis
...
\sumitem{15--16}{\result{\textbf{Quantifier order changes the claim.}
  There-is-a-$y$-for-all-$x$ is the stronger, and implies the other; the
  reverse fails, as a
  relation on $\val{p14.dom}$ elements shows and all
  $\val{p14.qchecked}$ relations on three confirm}.}
\sumitem{17--18}{\result{\textbf{Universal approximation is for-all-then-there-exists},
  so the network may depend on the function, the tolerance and the fixed
  activation. It is silent on
  $\val{p14.silent}$ things, including how large the network must be and
  whether training finds it}.}
\sumitem{19}{\result{\enquote{Networks can approximate any function} treats an
  existence claim as a construction. A high-degree polynomial does the same
  and nobody argues from it}.}
\sumitem{20--21}{\result{Three shapes to recognise: direct, contradiction, induction.
  \textbf{The negation of an implication is $P \wedge \lnot Q$}, which is why
  a proof by contradiction of one always starts there}.}
\sumitem{22--23}{\result{\textbf{Checking every case is a proof; checking many cases is
...
\sumitem{24--25}{\result{\textbf{Each shape fails in its own place}: a direct proof
...
\sumitem{26--27}{\result{\textbf{A convergence theorem assuming convexity says nothing
  ...
  sentence}.}
\sumitem{28--29}{\result{\textbf{A $\val{p14.conf}$ per cent bound used
  $\val{p14.uses}$ times guarantees all of them together at no better than
  $\val{p14.allhold}$ per cent}, falls to a coin flip after $\val{p14.flip}$
  uses, and would need $\val{p14.needed}$ per cent each to reach
  $\val{p14.joint}$ per cent across $\val{p14.uses}$}.}
\sumitem{30--31}{\result{The three misquotations are one of each part: a dropped
...
\item \enquote{There is a learning rate that works for every architecture}
  and \enquote{for every architecture there is a learning rate that works}.
