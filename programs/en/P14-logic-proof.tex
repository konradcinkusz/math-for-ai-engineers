...
\sumitem{15--16}{\result{\textbf{Quantifier order changes the claim.} There-is-a-$y$-
...
\sumitem{28--29}{\result{\textbf{A $\val{p14.conf}$ per cent bound used
  $\val{p14.uses}$ times guarantees all of them only $\val{p14.allhold}$ per
  cent of the time}, guarantees no better than a coin flip after
  $\val{p14.flip}$ uses, and would need $\val{p14.needed}$ per cent each to
  reach $\val{p14.joint}$ per cent across $\val{p14.uses}$}.}
