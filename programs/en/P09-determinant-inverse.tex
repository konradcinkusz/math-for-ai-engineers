...
about how close to flat the map is. $10^{-100}$ above, for a matrix no closer
to singular than it was before the scaling: multiplying every entry leaves the
condition number untouched, because that number is a ratio.
...
And one thing falls out of it that surprises people.
Program~\ref{prog:P06} made a good deal of $AB \ne BA$. Given the identity
above, say whether $\det(AB)$ and $\det(BA)$ can differ.
\dotline
\nextframe
\end{fr}

\begin{fr}
\ans{They cannot. Both are $\det A \det B$ and numbers commute \dash{} the
order matters to the map and not to the volume.}
...
The chain is worth asking about rather than the pair, because a chain is what
a network is. Program~\ref{prog:P08} answered this by counting directions and
got an answer no later layer could change; the product rule answers the same
question by multiplying numbers, and the two had better agree.
...
That check is what makes the next number a comparison rather than a race. Two
routes that disagreed would be two answers, and the cheaper of two answers is
not an interesting quantity; two routes that agree to the last fraction differ
in nothing but their cost.

At $n = \val{p09.cost.n}$, which is cheaper, and by roughly how much?
\dotline
\nextframe
\end{fr}
...
every divide increments a counter \dash{} so the ratio above is a
...
There is a case in which building all of them is worth it. Say what it is, and
then say what makes it rarer than the code suggests.
\dotline
\nextframe
\end{fr}

\begin{fr}
\ans{Genuinely many right-hand sides, arriving at different times \dash{} and a
factorisation of $A$ answers repeated right-hand sides too, and more cheaply.}
...
the picture, so a rotation cannot be scaling area \dash{} which pins
$\lvert\det\rvert$ to $1$ and leaves the sign open. Frame 5 said what the sign
records: whether the map turns the plane over. A rotation does not.
...
you can check it by hand. It is the matrix three frames back at
$\cos\theta = \tfrac{3}{5}$ and $\sin\theta = \tfrac{4}{5}$, so substituting
into that formula is the check:
...
The condition is also cheap to check, which is not always true of a compact
one. $R\T R = I$ is a matrix multiplication and a comparison, needing no
determinant, no elimination and no eigenvalue \dash{} so it is the rare
property that is both a strong guarantee and something you can test on a
matrix somebody hands you.

An orthogonal change of basis is often called free. There are two senses of
that word \dash{} cheap, and lossless. Say which one it earns, and why.
\dotline
\nextframe
\end{fr}

\begin{fr}
\ans{Not cheap \dash{} \emph{lossless}. No information about the vectors is
left behind in the old coordinate system.}
...
saying it needs a determinant. The position information is added by
\emph{re-describing} the query and the key rather than by altering them.

So say what the rotation costs, in this section's vocabulary.
\dotline
\nextframe
\end{fr}

\begin{fr}
\ans{Nothing. A rotation is an orthogonal change of basis \dash{} invertible,
determinant $\val{p09.rot.det}$, destroying nothing \dash{} so no direction of
the embedding is spent on carrying the position.}
Last question of the program, and it catches most people.

$\det = 1$ for a rotation, and a rotation preserves length. It is a short step
from there to reading the determinant as a length test, and the step is not
sound: everything in section 1 said the determinant measures \emph{area}, and
an area can be held fixed while the sides that make it change.

Here is a matrix with determinant $1$:
...
Two independent conditions, and this program has a counterexample against each:
the shear preserves area and not length, the reflection preserves length and
not orientation.
...
\sumitem{13--14}{\result{\textbf{A small determinant does not mean nearly singular.}
  Scaling a $100 \times 100$ matrix by $\num{0.1}$ multiplies its determinant by
  $10^{-100}$ \dash{} from $1$ to $10^{-100}$ in the worked case \dash{} and
  changes nothing about the map. \emph{Is anything destroyed}
...
\item A matrix has first row $(3, 1)$ and second row $(6, 2)$. Compute its
  ...
  \answerto{$3 \times 2 - 1 \times 6 = 0$. No inverse: the second row is twice
  the first, so both rows lie on one line through the origin, the plane is
  pressed onto a line, and two inputs share an output.}
...
\item Show that the product of two orthogonal matrices is orthogonal.
  \answerto{$(PQ)\T(PQ) = Q\T P\T P Q = Q\T I Q = Q\T Q = I$. So composing two
  orthogonal changes of basis is one orthogonal change of basis, and nothing is
  lost either time. Note that it need not be a rotation: determinants multiply,
  so two rotations give a rotation and a rotation with a reflection gives a
  reflection.}
