...
% ============================================================
\begin{summarybox}
\sumitem{1--3}{\textbf{A matrix scales every area by one factor}, because a
...
\sumitem{4--5}{\textbf{The area is signed}, and the sign is orientation: the
...
\sumitem{6--7}{$ad - bc$ in two dimensions, a signed volume in three, a signed
...
\sumitem{8--10}{\textbf{Zero means flat}, and flat means two inputs now share
...
\sumitem{11--12}{The worked case: rows $1,2,3$ then $4,5,6$ then $7,8,9$ has
  ...
  volume.}}
\sumitem{13--14}{\textbf{A small determinant does not mean nearly singular.}
...
\sumitem{15--16}{\textbf{$\det(AB) = \det(A)\det(B)$}, because a scale factor
  ...
  to the volume.}
\sumitem{17--18}{\textbf{One singular factor makes the whole chain singular},
...
\sumitem{19}{A product of hundreds of determinants underflows, so what is
...
\sumitem{24--26}{\textbf{Solving beats inverting} \dash{}
...
\sumitem{27}{And the stronger reason is accuracy rather than speed, which is
...
\sumitem{28--29}{\textbf{A change of basis is a matrix}, gated against
  ...
  times a vector.}
\sumitem{30--31}{It must be invertible, because the point never moved and the
...
\sumitem{32--33}{\textbf{A rotation has determinant $\val{p09.rot.det}$ and its
...
\sumitem{34}{Which is what Program~\ref{prog:F08}'s position encoding costs the
  ...
  carrying a position.}
\sumitem{35--36}{\textbf{Determinant $1$ and preserving length are two
