...
set is a perfectly reasonable set of vectors. It is a plane, it is
$\R^{3}$-shaped, and it does not contain the zero vector, which is the quickest
test there is: \textbf{every vector space contains the zero vector}, because
scaling any member by $0$ has to land inside.
\end{fr}
...
And \emph{usually}, because there is an exception you can probably already see:
if one of the two is a multiple of the other, every combination of them is a
multiple of either, and the span is a line.
...
\textbf{Three numbers, all different, and only the last one is the dimension:}
how many vectors are in the list, how long each vector is, and how many
independent directions they actually reach between them.

The first two are properties of how the data was written down. The third is a
...
evidence. \textbf{It cannot find one.} A search that succeeded would have
refuted a proof, and what the draws are really testing is the code.
\end{trapbox}
\end{fr}
...
The \emph{dimension} of a space is the size of any of its bases.
\end{fr}
...
% ============================================================
\begin{summarybox}
\sumitem{1--3}{\textbf{A vector space has two operations and one condition that
...
\sumitem{4--5}{The embeddings a model \emph{produces} are a subset of a space,
...
\sumitem{6--8}{A \textbf{subspace} is a subset that is a space in its own right.
...
\sumitem{9--11}{The \textbf{span} is everything the two operations reach from a
...
\sumitem{12--14}{\textbf{Three numbers, and only the last is the dimension}:
...
\sumitem{15}{A list is \textbf{dependent} when one member is a combination
...
\sumitem{16--18}{\textbf{Rank saturates.} Random vectors in
...
\sumitem{19--20}{\textbf{In $d$ dimensions at most $d$ vectors are
...
\sumitem{21--23}{A \textbf{basis} is independent and spanning \dash{} the exact
...
\sumitem{24--25}{\textbf{A basis is not unique.}
...
\sumitem{26--27}{So \enquote{embedding dimension $\val{p04.embed}$} promises a
...
\sumitem{28--29}{\textbf{The counting fact.} $\val{p04.vocab}$ tokens in
  ...
  and this one is not.}
\sumitem{30--31}{It does \emph{not} say the model cannot distinguish that many
  ...
  subject}.}
\sumitem{32--34}{The \textbf{linear representation} and \textbf{superposition}
