...
\item Two vectors in $\R^{3}$ are given, neither of them zero. What do they
  span? \teachesat{10--11}
  ...
  has been named. (With both zero the span is the zero vector alone, which is
  why the question says neither is.)}
...
Test one more. Is the set of vectors in $\R^{3}$ with at most one non-zero
component a subspace?
\nextframe
\end{fr}
...
what they reach.}{everything two operations can reach}
...
The reasoning behind the saturation is two lines and needs nothing this
program has not given. Take $\val{p04.sat.dim} + 1$ vectors of
$\val{p04.sat.dim}$ components each and look for a combination of them that is
zero: that is $\val{p04.sat.dim}$ equations, one per component, in
$\val{p04.sat.dim} + 1$ unknown coefficients. More unknowns than equations, so
a solution other than all-zeros always exists \dash{} which is exactly what
dependence says. So no set of more than $\val{p04.sat.dim}$ vectors in
$\val{p04.sat.dim}$ components can be independent, whatever the entries.
...
distance \dash{} none of which this program has been given. It needs only the
counting argument above, which is why it survives every choice of how to
...
perfectly good eight vectors; they reach two directions rather than
...
what the change does not touch.}{coordinates change, the vector does not}
...
on a different basis for a space that may be doing the same job. That is why a
claim about a single embedding component is a claim about a basis rather than
about a model, and why comparing two models component by component is comparing
two arbitrary choices of axes.

One qualification, because the word \enquote{neuron} usually names something
else. This argument holds for a purely linear object \dash{} an embedding
table, a residual stream \dash{} where any invertible change of basis leaves
the function unchanged. A unit sitting behind an elementwise activation is
different: $\relu$ applied coordinate by coordinate is not unchanged by a
rotation, so \emph{that} basis is singled out by the architecture, and a claim
about such a unit can be a claim about the model.
\end{aibox}
\end{fr}
...
In a space of $\val{p04.embed}$ dimensions a set of mutually independent
directions has at most $\val{p04.embed}$ members \dash{} that is the theorem.
There are infinitely many such sets, as the previous section said; what is
bounded is how many can be in one of them at a time.
...
\sumitem{16--18}{\result{\textbf{Independence saturates.} Random vectors in
...
\item Argue that the counting bound cannot by itself distinguish between the
  superposition account and an account in which the model represents
