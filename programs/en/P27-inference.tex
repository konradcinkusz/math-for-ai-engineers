...
\outcome{7--15}{build a confidence interval for a metric that has no formula,
  name the unit that has to be resampled, and say what a given width costs
  in items}
\outcome{16--23}{size and run a comparison between two models evaluated on
  the same prompts}
\outcome{24--31}{state what a p-value is, and what it is not}
\outcome{32--39}{say what forty comparisons on one leaderboard do to all of
  the above}
\outcome{40--42}{apply the whole of it to a number somebody has published}
\end{outcomes}
...
\item Two models were evaluated on the same prompts rather than on separate
  draws. Does that make the comparison harder or easier? \teachesat{17--18}
...
\item A comparison returns $p = \num{0.03}$. What is the probability that the
  two models are equally good? \teachesat{25--26}
...
The grid has to be finer than a tenth of a point, so $1/n$ has to be
under $\num{0.001}$.
...
needed to find it. The interval is the binomial's own percentiles, obtained by
summing Program~\ref{prog:P12}'s coefficient:
...
\mermaidfig{p27-what-to-resample}{The unit of resampling is a decision, and
  nothing checks it}{rows, users, and what the library sees}
...
You can now put an interval on anything you can compute, so there is exactly
one lever left. Your evaluation is $\val{p27.boot.n}$ items and the interval
above runs from $\val{p27.boot.lo}$ to $\val{p27.boot.hi}$, which is
$\val{p27.w.200}$ points wide; a leaderboard prints one decimal place.
Roughly how many items would it take to get that interval under one point?
\dotline
\nextframe
\end{fr}
...
\begin{fr}
\begin{ansblock}
About $\val{p27.w.onepoint}$ \dash{} the width has to fall by a factor of
$\val{p27.w.200}$, four times the items halves it, so the items go as the
square of that factor: $\val{p27.w.200}$ squared, times the
$\val{p27.boot.n}$ you have.
\end{ansblock}

Swept over three decades at the same accuracy, with the interval computed
exactly rather than resampled:

\begin{center}
\begin{tabular}{rrr}
\toprule
items in the set & interval width, points & the interval, in items \\
\midrule
$50$ & $\val{p27.w.50}$ & $\val{p27.w.span.50}$ \\
$\val{p27.boot.n}$ & $\val{p27.w.200}$ & $\val{p27.w.span.200}$ \\
$800$ & $\val{p27.w.800}$ & $\val{p27.w.span.800}$ \\
$\num{3200}$ & $\val{p27.w.3200}$ & $\val{p27.w.span.3200}$ \\
$\num{12800}$ & $\val{p27.w.12800}$ & $\val{p27.w.span.12800}$ \\
$\num{51200}$ & $\val{p27.w.51200}$ & $\val{p27.w.span.51200}$ \\
\bottomrule
\end{tabular}
\end{center}

The second row is the interval you already have, and the rest is that same
computation at other sizes. Four times the items halves the width at every
step, which is Program~\ref{prog:P25}'s rule measured rather than quoted
\dash{} and \textbf{the exactness is in the third column}, where the span
doubles as whole numbers. The points column is those spans divided by the set
size and then rounded, so dividing two of its entries gives $2$ to two
decimal places rather than $2$ exactly.

The one step that is not a halving is the first, and the smallest set is
where it breaks: $50$ items give $\val{p27.w.coarse}$ times the width of
$\val{p27.boot.n}$ rather than twice. A span is a whole number of items, and
on $50$ items one item is worth two points, so the interval is
$\val{p27.w.span.50}$ items wide and the next span down is a whole two points
narrower. That is section 1's grid at its coarsest, and here it rounds
outward: the bootstrap reports a shade more uncertainty than the formula
does.

The table brackets its own answer \dash{} $\val{p27.w.12800}$ points at
$\num{12800}$ items and $\val{p27.w.51200}$ at $\num{51200}$ \dash{} and the
crossing is at about $\val{p27.w.onepoint}$. \emph{About} is the honest form
rather than a hedge: the span moves one item at a time, so near the crossing
the width steps above and below one point as the set grows, and a size quoted
to the item would be a property of the grid rather than of the method.

All of that is one interval on one model. The question the program opened
with is about the difference between two, the number that answers that one is
section 3's rather than this one's, and it turns out that asking the
difference directly is far cheaper than putting an interval on each model
separately.
\end{fr}
...
$\val{p27.concord}$ items, so the comparison rests on
$\val{p27.to.b}$ items for B against $\val{p27.to.a}$ for A. Section 1
established that B is ahead by $\val{p27.net}$ item, and note that an odd
number of discordant items is what a lead of one requires: the count and the
lead have the same parity, because the lead is a difference of two numbers
adding to the count.
...
\begin{fr}
\ans{You cannot be surprised at all: the probability is
$\val{p27.p.exact}$}

\transcript{p27-odd-count}

\textbf{Exactly one}, and not as a rounding. $\val{p27.disc}$ coins cannot fall
evenly, so one side is always ahead and the smallest lead available is
$\val{p27.net}$ \dash{} which is the lead that was observed. A result at
least this lopsided is therefore certain, and the test cannot tell this
outcome apart from any other outcome whatever.

What can be said is which outcome it is: a lead of exactly $\val{p27.net}$,
one way or the other, is the single most likely result and happens
$\val{p27.p.one.pct}$ per cent of the time. \textbf{The published difference
between the two models is the least informative outcome the arithmetic
permits.}
...
reader can carry into a meeting, and it is thirteen times the gap that was
published.
...
\begin{fr}
\begin{aibox}
\textbf{Two lines your harness could print and does not.}
...
\index{pvalue@p-value}
\index{inference!what a p-value does not say}

\begin{fr}
The number from the last section has a name, and you have now computed one
before meeting the word, which is the right order \dash{} and computed it at
the one value where its meaning is hardest to mistake.
...
threshold, seen from the other end. The threshold is built so that
\textbf{one comparison in twenty between two identical models reports a
significant difference}, and no choice of test removes that.

What the procedure achieves here is $\val{p27.null.rate.pct}$ rather than
exactly $\val{p27.alpha}$, and the reason is worth having: the discordant
test is
...
$\num{1.96}$ to $\val{p27.z.bonf}$, and section 3's formula carries
...
\mermaidfig{p27-winner-is-noise}{What sorting a table of equals does to the
  top of it}{equal models, sorted by their errors}
...
\begin{fr}
A third axis, the same sum, and no leaderboard anywhere in it.

The evaluation runs and the change does not show a win. You run it again on
another seed. Then against the newer snapshot of the set. The fifth run shows
a win at $\val{p27.alpha}$ and that is the one that gets written up.

Nobody calls that $\val{p27.peek.m}$ comparisons and nobody writes the number
down anywhere. Count the attempts the way the leaderboard counted the models:
on a change that does nothing at all, what is the chance that one of
$\val{p27.peek.m}$ attempts shows a win?
\dotline
\nextframe
\end{fr}

\begin{fr}
\ans{$\val{p27.peek.pct}$ per cent}

$1 - \num{0.95}^{\val{p27.peek.m}}$, which is the leaderboard's own
expression with attempts where the models were. The arithmetic does not care
whether the $m$ things are models, configurations or Tuesdays.

\textbf{And this is the version of it nobody budgets for}, because the
attempts are separated in time rather than laid out in a column.
$\val{p27.models}$ rows in a table are visibly $\val{p27.models}$;
$\val{p27.peek.m}$ reruns across a fortnight are five afternoons, each of
which felt like one test, and the fortnight leaves no artefact saying
otherwise.

At $\val{p27.peek.coin}$ attempts the rate passes a half. A procedure that
reports whatever it was looking for as often as a coin does, with every
individual step of it correctly performed.

The repair is Bonferroni again, and putting it that way is what makes it a
consequence rather than a second rule to remember: holding the overall rate at
$\val{p27.alpha}$ across $m$ attempts needs
$\val{p27.alpha}/m$ at each one. \textbf{So $m$ has to be fixed before the
first attempt}, because a threshold cannot be divided by a number nobody has
decided yet. That is the whole of why \enquote{run it again} is not a
procedure \dash{} not that rerunning is dishonest, but that it leaves $m$
undefined until the result chooses it. Program~\ref{prog:P25}'s run of greens
is the same condition seen from the other side: the count that licenses the
claim is fixed before the runs, not once they are in.
\end{fr}
...
the one number you would have to ask for. A lead of one on an odd count is
the smallest there is, so the test is certain to see at least it.
...
\begin{summarybox}
\sumitem{1--3}{\result{\textbf{Read a score against its own denominator first.} A
  ...
  over a thousand items \dash{} $\val{p27.decimal.floor}$ \dash{} before the
  decimal can round}.}
\sumitem{3--5}{\result{\textbf{The published gap is $\val{p27.gap.items}$ item.}
...
\sumitem{6}{\result{Comparing two separate intervals by whether they overlap is not
  a test. Build the interval on the difference, which is the quantity the
  question is about}.}
\sumitem{7--9}{\result{\textbf{The bootstrap} resamples the sample with replacement
...
\sumitem{10--11}{\result{Its use is the case with no formula \dash{} a median, whose
...
\sumitem{12--14}{\result{\textbf{The unit of resampling is a decision and nothing
...
\sumitem{14--15}{\result{\textbf{The width is set by the item count and by nothing
  else}, and four times the items halves it \dash{} measured, not quoted. The
  interval spans $\val{p27.w.span.200}$ items on $\val{p27.boot.n}$ and
  $\val{p27.w.span.51200}$ on $\num{51200}$, and it falls under one point at
  about $\val{p27.w.onepoint}$ items}.}
\sumitem{16--18}{\result{\textbf{Pairing multiplies the items you need by
  ...
  $\Var(X-Y)$}.}
\sumitem{19--20}{\result{\textbf{Only the discordant items carry the difference.}
...
\sumitem{20--21}{\result{Under \enquote{equally good} those items are fair coin
  flips, so the test is Program~\ref{prog:P12}'s binomial coefficient over
  integers \dash{} exact, machine-independent, and needing no variance.
  \textbf{The worked gap gives $\val{p27.p.exact}$}: an odd count cannot fall
  evenly, so the observed lead of $\val{p27.net}$ is the smallest available
  and a result at least this lopsided is certain}.}
\sumitem{22}{\result{\textbf{On this evaluation a gap under
  $\val{p27.net.needed.pts}$ points is not a result}, found by search. The
  published gap was $\val{p27.gap.pts}$}.}
\sumitem{24--26}{\result{\textbf{A p-value is
  ...
  as the base rate}.}
\sumitem{27--28}{\result{Given a prior it can be turned round, and the answer is
...
\sumitem{29--30}{\result{\textbf{One comparison in twenty between identical models
  ...
  on it}.}
\sumitem{32--34}{\result{\textbf{$\val{p27.models}$ comparisons make that
...
\sumitem{35--36}{\result{\textbf{And sorting a table biases its top row.} The winner
...
\sumitem{38--39}{\result{\textbf{And the same sum counts attempts as readily
  as models.} Rerunning an evaluation until it shows a win turns
  $\val{p27.alpha}$ into $\val{p27.peek.pct}$ per cent over
  $\val{p27.peek.m}$ attempts, and $\val{p27.peek.coin}$ attempts make it a
  coin. \textbf{The number of attempts has to be fixed before the first one},
  because a threshold cannot be divided by a number the result chooses}.}
\sumitem{40--42}{\result{What is left when a comparison cannot resolve what you care
...
\item A model is reported at $\num{83.3}$ per cent on $60$ evaluation items.
  ...
  \answerto{Yes \dash{} but not for the reason frame 2 rejected
  $\num{71.4}$. $50/60$ is $\num{83.33}\ldots$ per cent, which prints as
  $\num{83.3}$, so the figure is a rounding of an achievable score; neither
  $\num{71.0}$ nor $\num{71.5}$ rounds to $\num{71.4}$, which is why that
  one was not. The grid here is $100/60 = \num{1.67}$ points, so the
  neighbours are $49/60 = \num{81.7}$ and $51/60 = \num{85.0}$ per cent
  \dash{} which is how coarse a $60$-item benchmark is.}
...
\item Your evaluation set has $800$ items and you want the interval half as
  wide. How many items does that take, and what happens to the interval's
  span, measured in items?
  \answerto{Four times as many, so $\num{3200}$ \dash{} and the span
  \emph{doubles}, from $\val{p27.w.span.800}$ items to
  $\val{p27.w.span.3200}$. Those are one fact rather than two: the width is
  the span divided by the set size, so quadrupling the set while doubling
  the span halves the width. A span that grows while the interval narrows is
  the half that reads wrongly.}
...
\item A benchmark reports $\num{62.48}$ per cent. What is the smallest number
  ...
  \answerto{The same criterion as frame 3, one decimal further out: the grid
  must be finer than a hundredth of a point, so $1/n < \num{0.0001}$ and
  $n > 10\,000$ \dash{} that is $\val{p27.decimal.floor.two}$ items. Almost
  no benchmark has that many, so two decimal places on a public leaderboard
  are decoration.}
...
\item Two models are compared on $n$ shared items and disagree on a fraction
  ...
  $\num{1.96}\sqrt{dn}$ items for a fair coin not to produce it \dash{}
  rounded up to the next value of the same parity as $dn$, and loose: at
  $dn = \val{p27.disc}$ it gives $11$ where the exact search gives
  $\val{p27.net.needed}$, because a discrete test steps over its threshold
  rather than landing on it. $\rho$ has
