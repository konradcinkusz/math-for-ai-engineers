...
\outcome{1--6}{read $\cos$ and $\sin$ off a point going round a circle, work in
  radians, and read an identity off that picture instead of remembering it}
...
\begin{note}
There is no sine rule here, no cosine rule, no addition formulas, no solving
of triangles and no list of formulas to learn \dash{} the book's scope
statement excludes all of it, and nothing later needs any of it. What it keeps
instead is the picture, and every identity this program does use is read off
the circle rather than remembered: this one, the two symmetries in the next
section, and the quarter-turn shift after them. You now have the circle, which
...
Now the symmetry, which Program~\ref{prog:F05} gave you the first word for.
Walking backwards round the circle by $\theta$ puts the point at the mirror
image, in
...
$\cos$ is \emph{even}, in exactly Program~\ref{prog:F05}'s sense. $\sin$ is
\emph{odd}, which is the same idea with a sign on it \dash{} the mirror image
comes back negated. Again the picture is the proof.
...
coordinates, and the reason is the one section 5 proves. Read the two together
as a \emph{pair}: that pair is frame 1's point on the circle, at an angle the
position sets. Moving $k$ places along the sequence turns the point through a
fixed angle, and turning a point is a linear map.

That is the motivation the original transformer's paper gives for choosing
these functions \dash{} the encoding at one position is a fixed linear
function of the encoding at any position a fixed distance away, so the model
can learn to attend by relative position rather than learning every absolute
position on its own. One function alone will not do it: a lone
$\sin\theta$ coordinate is half of a point, and the shift needs the other
half to be written down at all.
\end{aibox}
\end{fr}
...
tokens away, because it has gone round well over a hundred times in between.
A wave of
...
$\lVert a \rVert$ is the length of $a$, which is $\sqrt{a \cdot a}$ \dash{}
Pythagoras on the components, and this section's own circle equation with the
radius left free. Program~\ref{prog:F09} does both properly; all this section
needs is that the two lengths are positive numbers.
...
Reflecting both in a line through the origin would have done as well, and so
would negating both. The rotation is the one the next section needs, because
it is the one a position can be turned into.
\end{fr}
...
And you can see why without any algebra, from the previous section.
Rearranging the cosine-similarity formula gives
\[ a \cdot b = \lVert a \rVert\, \lVert b \rVert \cos\theta \]
and a rotation leaves both lengths
...
\emph{different} angle $\beta$. Their dot product still depends on $a$ and on
$b$. What can it depend on \emph{about the two angles}?
\nextframe
\end{fr}
...
is now a sentence rather than an incantation: it puts position into a dot
product so that the answer depends on the difference of the positions and not
on the positions themselves, and among the maps that turn the query and the
key by an angle the position alone decides, it is the one this identity
forces. It is not the only route to that property \dash{} a learned relative
term inside the score, or a bias added to it that depends only on the gap,
reach it another way \dash{} and this book compares none of them.
...
% ============================================================
\begin{summarybox}
\sumitem{1--3}{\result{\textbf{$\cos\theta$ and $\sin\theta$ are the two coordinates
...
\sumitem{4--5}{\result{\textbf{An angle is a distance walked, which is what a radian
...
\sumitem{5--6}{\result{$\cos^{2}\theta + \sin^{2}\theta = 1$ is
  \textbf{the equation of the circle with the names substituted in}, so it
  needs no proof and no memorising. Checked to $\val{f08.pyth.err}$. Every
  identity in this program is read off that circle the same way, and the book
  borrows none from outside it \dash{} no sine rule, no cosine rule, no
  addition formulas}.}
\sumitem{7--8}{\result{Both curves have period $2\pi$ and range $\intcc{-1}{1}$
  ...
  horizontal axis}.}
\sumitem{9--10}{\result{\textbf{$\cos\theta = \sin(\theta + \frac{\pi}{2})$}, checked
  ...
  apart}.}
\sumitem{11--13}{\result{In $A\sin(B\theta + C) + D$ the four letters are
...
\sumitem{14--15}{\result{\textbf{A wavelength is a number of positions, not a number
...
\sumitem{16--17}{\result{\textbf{Cosine similarity is the cosine of the angle between
...
\sumitem{18--19}{\result{$\val{f08.cos.same}$, $\val{f08.cos.right}$ and
...
\sumitem{20--21}{\result{\textbf{The mathematics is exact and the interpretation is
...
\sumitem{22--23}{\result{Cosine similarity depends on the two vectors only through the
  angle between them, so scaling both leaves it alone \dash{} and so does
  turning both by the same amount}.}
\sumitem{24--26}{\result{A rotation sends $(x, y)$ to
...
\sumitem{27--28}{\result{Rotating \emph{both} vectors by the same angle leaves the dot
  ...
  and $\beta$ leaves a dot product that depends on the two angles only
  through $\beta - \alpha$
  \dash{} checked to $\val{f08.rot.rel.err}$}.}
\sumitem{29--30}{\result{\textbf{That identity is rotary position embedding.} Turn the
...
\item Why can a rotation not be used to make one vector longer than another?
  ...
  out.}
\item Name the identity this program takes from the circle, and say where it
  comes from.
