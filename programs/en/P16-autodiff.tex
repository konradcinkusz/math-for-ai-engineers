...
multiplying $\val{p16.depth}$ of them together: five matrix multiplications, so
$5 \times \val{p16.width}^{3}$ multiplications in all.
\end{ansblock}
...
Take a chain $J_3 J_2 J_1$. In what order would you multiply to get
$J_3 J_2 J_1 v$ without ever building a matrix \dash{} and in what order for
...
\begin{fr}
\begin{ansblock}
$J_3(J_2(J_1 v))$, from the right; and $((u^{\mathsf{T}}J_3)J_2)J_1$, from the
left.
\end{ansblock}

Each keeps every intermediate a vector: a matrix times a vector is a vector,
and a row vector times a matrix is a row vector, so nothing bigger than a
layer's worth of numbers ever exists. \textbf{Right-to-left on $Jv$ is forward
mode. Left-to-right on $u^{\mathsf{T}}J$ is reverse mode.}

There is a third way to bracket and nobody uses it. Write $(J_3 J_2) J_1 v$
and the first step multiplies two \emph{matrices} together, which builds
exactly the object this section says never gets built. It gives the same
answer, because matrix multiplication is associative, and it is the expensive
one \dash{} so the two modes are not the two brackets of $Jv$. They are one
bracket of $Jv$ and one of $u^{\mathsf{T}}J$.

\textbf{That the bracketing decides the cost is this program's whole subject,
and Program~\ref{prog:P06} measured it one program before it had a name}: one
bracketing of a triple product cost $\val{p06.cost.left}$ multiplications and
the other $\val{p06.cost.right}$, for an identical result. Its shapes are not
these shapes, so that factor is not the price of choosing the wrong mode
\dash{} the price for this chain is in section~\ref{sec:P16-modes}, and it is
the input count. The two figures here are Program~\ref{prog:P06}'s own and the
script checks them against it, so the two programs cannot disagree about one
computation.

\mermaidfig{p16-two-brackets}{The same product, from either end.}{%
  pushing from one end, pulling from the other}
\end{fr}
...
Multiplying two of the Jacobians together first \dash{} $(J_3J_2)$, from
either end or from the middle \dash{} would form a matrix, and that is the
thing neither mode ever does. \textbf{Nobody forms a Jacobian} is not a
...
Both routines count their own multiplications as they go. Predict the ratio of
the forward count to the reverse count before reading on.
\dotline
\nextframe
\end{fr}
...
There is a second half people drop, and section~\ref{sec:P16-memory} is about
it: reverse mode
...
\mermaidfig{p16-keep-or-recompute}{What the backward pass has to keep, and
what forward mode does not.}{what the backward pass has to keep}
\end{fr}
...
does not, and the price is stated exactly: one extra forward pass. That is a
third more work rather than a doubling, and the arithmetic behind it is worth
having: a backward pass costs about two forward passes, so a step is roughly
three forward-equivalents and one more forward is a third on top.
...
Notice first what the difference needs: two values of the function, and
nothing else. It does not read the backward pass, does not know which rule was
applied at which node, and would run unchanged against a model whose gradient
code you have never seen. That independence is the whole of its value here
\dash{} a check sharing machinery with the thing it checks would agree with it
for the wrong reason.

And set it against what this section has just counted. Reverse mode's bill was
the intermediate values it has to keep, and the whole of this section has been
about paying it; a finite difference keeps nothing at all, and needs no tape to
keep it on. Whatever the objection to it turns out to be, it is not memory.
...
\textbf{So a gradient check does not agree to the last bit}, and demanding
that it should would be demanding the test be better than it can be.

How close it gets is a property of the difference rather than of the code, and
it moves with the step, the precision and which difference you take.
Program~\ref{prog:F11} measured a one-sided difference bottoming out around
$10^{-8}$; a central difference does better still, because its truncation
error shrinks as $h^{2}$ rather than as $h$.

So there is no one digit count that means \emph{correct}. A handful of digits
is what a one-sided check in single precision gives over a whole network; this
program's own differentiator, checked against a central difference in double
precision at $\val{p16.check.n}$ inputs, agrees to better than
$\val{p16.check.bound}$ \dash{} orders tighter. Both are the check passing.
What would be a mistake is to fix on one tolerance and call everything outside
it a bug.
\end{fr}
...
The three are not three versions of one mistake, and they get harder in a
particular way. This first one is local: one operation, one input value, and
the whole of the difficulty fits in a single line of code. The other two do
not fit in any single line, because what has gone wrong is a relationship
between two places in the program that were written at different times and
that look correct read separately. They are in that order deliberately.
...
\begin{fr}
The third. Reverse mode records each operation as it runs \dash{} the
\textbf{tape} \dash{} keeping for each one either the operand it will need on
the way back or the local derivative it computed from it.

Now suppose a value the tape still refers to is overwritten in place after
being used. Say what the backward pass computes with, before reading on.
\dotline
\nextframe
\end{fr}
...
In the listing below, the product of $\val{p16.inplace.a}$ and
...
\sumitem{7--8}{\result{\textbf{The gradient of a scalar loss is one row and one
  column per parameter} \dash{} $\val{p16.params}$ numbers here, exactly the
  size of the model, which is what makes the \emph{next} Jacobian's size the
  surprise}.}
...
\sumitem{11--14}{\result{\textbf{Nobody forms a Jacobian.} What is computed is a
  ...
  matrices}.}
\sumitem{14--15}{\result{\textbf{Forward mode is $J_3(J_2(J_1v))$ and reverse
  mode is $((u\T J_3)J_2)J_1$}: one bracket of $Jv$ and one of $u\T J$, each
  keeping every intermediate a vector. Multiplying two Jacobians together
  first is the third bracket, it forms the matrix nobody forms, and
  Program~\ref{prog:P06} measured $\val{p06.cost.left}$ against
  $\val{p06.cost.right}$ for what bracketing costs}.}
...
\item Your gradient check agrees to six digits for every parameter except one,
  ...
  section~\ref{sec:P16-silent}, and how would you tell them apart?
...
\item Show that forward and reverse mode compute the same thing in the same
  number of operations when there is one input and one output \dash{} and say
  what you had to assume about $f$.
  \answerto{Nothing beyond differentiability. With one input and one output
  every Jacobian in the chain is a single number, the chain is a product of
  scalars, and both bracketings are the same product in a different order
  \dash{} the same count either way. Linearity is not used anywhere, which is
  worth noticing: the shape is the whole argument, and the two modes separate
  only when a dimension is bigger than one.}
\item A model has $P$ parameters and the loss is a vector of $C$ per-class
  ...
  in section~\ref{sec:P16-modes} work.}
