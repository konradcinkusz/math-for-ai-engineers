...
\item Two vectors both have length $1$. If one is nearer to a query than the
  ...
  \answerto{Yes, always. On the unit sphere the \emph{squared} distance is
...
as long, so a \textbf{positive} scalar changes the size and not the
direction.
...
different numbers of components, neither addition nor subtraction is
defined, because there is nothing to pair the leftover components with.
Scaling is untouched: it takes one vector and a number.
...
equality or ordering comparison against those components is \code{False}
\dash{} including the one that asks whether it is equal to itself \dash{}
while \code{!=} comes back \code{True}. A \code{nan} compares unequal to
...
\begin{tabular}{lcc}
\toprule
The two vectors point & $\cos\theta$ & $a \cdot b$ \\
\midrule
...
\begin{fr}
\ans{A, necessarily}
The minus sign is the whole of it: as the cosine goes up the distance goes
down, and as the cosine goes down the distance goes up, with no exceptions and
no regions where the behaviour changes. \textbf{On the unit sphere, distance
and cosine similarity are one question with two answers.} Not usually, not
approximately: they rank every pair of
...
when the candidates all share one length, and the previous frame is exactly
why.} Normalising is the usual way to arrange that and it is not the only one:
any common length will do, because the cosine is then the only thing left that
varies. Take the shared length away and
$\lVert a - b \rVert^{2} = 2 - 2\cos\theta$ becomes
...
\sumitem{5--6}{\result{Adding a plain number to a vector is \textbf{not defined} in
  ...
  numbers of components support neither addition nor subtraction, which is
  the entire content of a shape error}.}
...
\item The same store holds unnormalised embeddings and the same switch is
  made. What changes?
  \answerto{The ranking can change, and nothing in the output says so. Distance
  has the lengths in it as well as the angle, so a vector can be pushed down by
  its length alone \dash{} too short as much as too long \dash{} while its
  cosine does not move. Frame 25 is the case: the candidate that loses on
  distance is the \emph{short} one, which is also the more similar.}
...
\item A colleague reports that normalising the embeddings \enquote{fixed} a
  ...
  was direction, so documents whose embeddings were the wrong length \dash{}
  short or long \dash{} were being pushed down the list for a reason that had
  nothing to do with what they said. It would be confirmed by finding that the
  documents that moved are the ones whose embeddings had unusual lengths.}
