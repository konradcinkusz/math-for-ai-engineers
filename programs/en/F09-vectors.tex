...
% ============================================================
\begin{summarybox}
\sumitem{1--4}{\textbf{A vector is a list of numbers.} That is the whole
...
\sumitem{5--6}{Adding a plain number to a vector is \textbf{not defined} in
...
\sumitem{7--9}{$\lVert a \rVert$ is the square root of the sum of the squares,
...
\sumitem{10--13}{\textbf{Normalising divides every component by the length}
  ...
  mathematics.}
\sumitem{14--16}{$a \cdot b = \sum_i a_i b_i$ takes two vectors and returns
...
\sumitem{19--20}{\textbf{Lengths do not add.}
...
\sumitem{21--22}{\textbf{$\lVert a - b \rVert^{2} = \lVert a \rVert^{2} +
  ...
  $\val{f09.unit.err}$.}
\sumitem{23}{So \textbf{on the unit sphere distance and cosine similarity rank
  ...
  answer.}
\sumitem{24--25}{\textbf{Off the sphere they can name different winners.} On
...
\sumitem{26--27}{Normalising the same two candidates puts $A$ nearer
...
\sumitem{28--30}{Every operation here is one pass over the components, so
  ...
  $\val{f09.undrawn}$ cannot.}
\sumitem{31--32}{Identities transfer exactly. Claims about \emph{where things
