...
\item A rank-$1$ approximation is subtracted from $A$. What is left, in terms
  ...
  \answerto{Measured as the sum of squares, $\sigma_2^{2}$, if the piece removed
  was the largest one \dash{} each rank-one piece contributes its own singular
  value squared and nothing else does. Its square root, $\sigma_2$, is the same
  answer in the Frobenius norm.}
...
singular values. \textbf{$\lvert\det A\rvert$ is the product of the stretches}
\dash{} which is what Program~\ref{prog:P09} meant by volume, said in this
program's vocabulary. The modulus is not a nicety: the two rotations contribute
a sign and nothing else, so the sign of $\det A$ says whether the map turns
space over and carries no information about size.
\end{fr}
...
Program~\ref{prog:P10}'s theorem applies to symmetric matrices only, and this
one is about matrices of every shape. Say why that is not a gap.
\dotline
\nextframe
\end{fr}

\begin{fr}
\ans{Because $A\T A$ is symmetric whatever $A$ was. Singular values exist for
every matrix because they are borrowed from a symmetric matrix you can always
build.}
...
The two constructions are worth holding apart before you answer. The singular
values came from two \emph{different} rotations, one on each side, which is
what a matrix of general shape needs; an eigenvalue asks for a direction the
matrix leaves alone, which is a question about one side only. Nothing so far
says the two answers have to be related.
...
$\val{p11.sig.hi}$ is a difference of under $5\%$, and a plot of one labelled
as the other would look entirely reasonable.
\end{trapbox}
...
They nonetheless imply the same list of singular values, and the reason is one
line: if $R\T R v = \lambda v$ with $\lambda \ne 0$, then
$RR\T(Rv) = \lambda(Rv)$, so every non-zero eigenvalue of one is a non-zero
eigenvalue of the other. \textbf{The larger Gram carries extra zeros}, one for
each dimension the matrix cannot reach.

Their traces agree at $\val{p11.rect.energy}$, which the script checks. That is
the shared spectrum showing up as a total rather than a reason for it \dash{}
equal traces on their own would prove nothing, since $\operatorname{diag}(1, 3)$
and $\operatorname{diag}(2, 2)$ have the same trace and different singular
values.
\end{fr}
...
The construct has a name and a trap in it. $u_i v_i\T$ is a column times a
row, which is the \textbf{outer product} \dash{} the thing
Program~\ref{prog:P06} said a low-rank update must never form. Move the
transpose to the other vector and $a\T b$ is the inner product, which is a
\emph{number}. One mark in a different place, and a matrix on one side against
a scalar on the other. Here only the first is even available: $u_i$ has as many
components as $A$ has rows and $v_i$ as many as it has columns, so for a
rectangular $A$ the two vectors cannot be paired the other way round at all.

The pieces do not interact: the $u$'s are perpendicular and so are the $v$'s.

Which is what makes the next question answerable at all. When pieces overlap
there is no telling what any one of them contributes on its own \dash{} the
whole is not the sum of the parts and asking for a share is asking for
something that does not exist. Perpendicular pieces are exactly the case where
it does.

So read $A$ as one long vector and take its squared length, the way
Program~\ref{prog:P05} would. What does that total decompose into?
\dotline
\nextframe
\end{fr}
...
On the constructed matrix of section 1, truncating to rank $1$ leaves
$\val{p11.ey.err2}$ measured that way \dash{} which is $\sigma_2^{2}$, and the
script checks it over fractions.

\textbf{Two measures, one square apart, and the change of unit is worth
stopping at.} The quantity the last two frames have been adding is a sum of
squares, so what truncation leaves is $\sigma_2^{2} = \val{p11.ey.err2}$. Its
square root, $\val{p11.ey.err}$, is $\sigma_2$ itself \dash{} the length of the
error matrix read as one long vector, which has a name, the \textbf{Frobenius
norm}, and which is the measure Eckart--Young below is stated in.

The error is not \emph{about} $\sigma_2$ or bounded by it. In that norm it
\emph{is} $\sigma_2$.
...
Note that the numbers themselves fall by a factor of a hundred across the list
and their squares by a factor of ten thousand, so the two ways of adding them
up give very different answers. Take the squared total \dash{} the
$\sigma_i^{2}$ of four frames back, which is the one that decomposes.

What fraction of it do you still have?
\dotline
\nextframe
\end{fr}
...
sensor, or from the last rounding Program~\ref{prog:P01} described.
...
That case had a condition number of a million and cost six of sixteen digits.
Generalise it: how many digits does a condition number of $\kappa$ cost?
\dotline
\nextframe
\end{fr}

\begin{fr}
\ans{\textbf{About $\logb{10}\kappa$ of them.} Double precision starts with
about sixteen, so a condition number of $10^{16}$ leaves nothing at all, and
one of $\val{p11.spec.kappa}$ costs two of the sixteen and is unremarkable.}
...
Before the table: predict roughly how many decimal digits separate the two
answers.
\dotline
\nextframe
\end{fr}
...
\begin{fr}
\ans{Squaring $\kappa$ doubles $\logb{10}\kappa$, so the normal equations
should give away about as many digits again as the problem already costs.}
...
did not know the answer in advance. They are wrong from the sixth
...
Two claims, both with numbers. Say which of them decides the advice, and why
...
\begin{fr}
\ans{The second. A factor of $\val{p09.cost.ratio}$ in speed is an engineering
trade-off that a faster machine can absorb; digits are not recoverable at any
speed.}
...
by $U\T$, divide by each \emph{non-zero} singular value and send the zero ones
to zero, turn by $V$. Written as a matrix it is called the
\textbf{pseudoinverse}, $A^{+}$.

That qualification is the whole of why it exists. A zero singular value is a
direction $A$ flattens, and no rule can undo flattening; declining to answer in
that direction is what lets $A^{+}$ be defined for a matrix that has no
inverse.
...
It is worth being plain about what it is for. The pseudoinverse is not a way
of inverting a matrix that has no inverse \dash{} nothing is \dash{} it is a
name for the answer Program~\ref{prog:P08} already derived, written so that
one symbol stands for it. Naming a thing does not make it possible; it makes
it quotable.
...
% ============================================================
\begin{summarybox}
\sumitem{1--2}{\result{\textbf{An eigenvector equation cannot be written for a
...
\sumitem{3--4}{\result{The repair asks for \textbf{two} sets of perpendicular
  ...
  no conditions}.}
\sumitem{5--6}{\result{Checked exactly on a matrix built from \emph{two} different
...
\sumitem{7--8}{\result{$A\T A = V\Sigma^{2}V\T$, so \textbf{the singular values are the
...
\sumitem{9--10}{\result{\textbf{Singular values are not eigenvalues}, and the example
  ...
  difference of under $5\%$ that no plot would show. They differ in kind too
  \dash{}
  singular values exist for every shape, are always real and non-negative, and
  never run short of directions}.}
\sumitem{11}{\result{A rectangular matrix has \textbf{two} Gram matrices of different
  sizes, whose traces agree at $\val{p11.rect.energy}$ \dash{} which is the
  shared non-zero spectrum showing up as a total rather than a reason for it.
  The larger carries extra zeros, one for each dimension the matrix cannot
  reach}.}
\sumitem{12--13}{\result{$A$ is a \textbf{sum of rank-one pieces}, one per singular
  ...
  place of arrows}.}
\sumitem{14--15}{\result{Truncating to rank $r$ therefore leaves \textbf{exactly} the
  pieces discarded: $\val{p11.ey.err2}$ for the worked matrix, which is
  $\sigma_2^{2}$, measured as the sum of squares the frame before defined. Its
  square root, $\val{p11.ey.err}$, is $\sigma_2$ \dash{} the Frobenius norm of
  the error, and the norm Eckart--Young is stated in. Not about $\sigma_2$, not
  bounded by it}.}
\sumitem{16--17}{\result{\textbf{Eckart--Young}: of all matrices of rank $r$ the
  ...
  carries away.}
\sumitem{18--19}{\result{Two of six singular values carrying $\val{p11.spec.pct}\%$ is
...
\sumitem{20--21}{\result{Error arrives with the input and is stretched like everything
...
\sumitem{22--23}{\result{\textbf{You lose about $\logb{10}\kappa$ decimal digits}, out
...
\sumitem{24}{\result{For a symmetric positive definite matrix $\kappa$ is the
...
\sumitem{25--26}{\result{$A\T A$ has the \emph{squared} singular values, so
  ...
  way}.}
\sumitem{27--29}{\result{Measured on a degree-$\val{p11.ls.deg}$ fit through
...
\sumitem{30}{\result{So \enquote{do not form the normal equations} is two claims with
...
\sumitem{31--32}{\result{The \textbf{pseudoinverse} $A^{+}$ is turn, divide by each
  \emph{non-zero} singular value and send the zero ones to zero, turn \dash{}
  agreeing with $A^{-1}$ where that exists and giving the least-squares answer
  where it does not. Dividing by the smallest
  $\sigma$ is where the condition number becomes a division you can point at,
  and truncating first is the trade that bounds it}.}
\sumitem{33}{\result{\textbf{One factorisation answers six programs' questions}: rank
...
\item The truncation of $A$ to rank $2$ leaves an error of $1$. What does that
  ...
  rank-$2$ matrix?
  \answerto{In the Frobenius norm the error is
  $\sqrt{\sigma_3^{2} + \sigma_4^{2} + \cdots}$, so $\sigma_3 \le 1$, with
  equality exactly when $\sigma_4$ onwards vanish \dash{} which is the case if
  $A$ has three singular values. And by Eckart--Young no other rank-$2$ matrix
  is closer, so $1$ is not merely this method's error but the best available.}
...
\item Why does a matrix with a zero singular value have no inverse, in this
  ...
  \answerto{Because $\Sigma$ flattens that direction: undoing the map there
  would mean dividing by zero, which is why no inverse exists. The map is not
  one-to-one, which is Program~\ref{prog:P08}'s null space and
  Program~\ref{prog:P09}'s zero determinant. Note that $A^{+}$ does \emph{not}
  divide by zero \dash{} it sends that direction to zero and declines to
  answer, which is how it survives where $A^{-1}$ cannot.}
\end{testexercises}
...
\item Show that $\kappa(A) \ge 1$ for every matrix, and say which matrices
  ...
  multiple of the identity and $A$ is an orthogonal matrix scaled by a
  constant \dash{} a rotation or a reflection, and for a rectangular $A$ any
  scaled isometry \dash{} which stretches every direction alike and so cannot
  distort a relative error at all.}
