...
That list is the \textbf{sample space}\index{sample space}. Two properties make
it one: the
...
An alarm is a rare event, which is what you would want.

\begin{note}
Notice that neither of the two counts you added was given to you. The
detector's specification describes what it does to a faulty request and what
it does to a clean one, and those counts are what happens when you also say
how many of each there were. A specification and a population are two
different things, and only the pair produces a number.
\end{note}
\end{fr}
...
Section 2's trap box named those as different numbers; this is how
different.
...
alone, so the base rate cannot move it; precision conditions on what the model
flagged, so the base rate is inside it. Two adjacent lines of the same report,
behaving completely differently, with nothing on the card to say so.
\end{aibox}
...
coincidence. Real alarms are $ps$ and false ones $(1-p)f$, so they are equal
when $p = f/(s+f)$ \dash{} and for a detector whose miss rate equals its
false-alarm rate, as this one's does, that is $f$ itself. It is exact over
fractions and the script asserts it.
...
truth \dash{} whether the service is actually broken, which it is half the
time \dash{} and each is right three times in four. The two monitors do not
talk to each other at all. Write $X$ and $Y$ for the events that the first and
the second monitor report it broken.
...
$\val{p23.two.pct}$: \textbf{two independent detectors are enough to cross a
half}, and a third leaves only
...
\emph{given the truth} \dash{} which is the second row of this section's own
two-row table, and which nothing in either alarm tells you.
...
\begin{summarybox}
\sumitem{1--3}{\result{A \textbf{sample space} is the list of outcomes, exactly one of
...
\sumitem{4--5}{\result{$\Prob(\bar{A}) = 1 - \Prob(A)$, from two of the three rules and
  no picture.} Every result here is got the same way: split into disjoint
  pieces, add, use that the whole is $1$.}
\sumitem{6--7}{\result{Additivity needs \emph{disjoint} pieces:
...
\sumitem{8--9}{\result{A probability is a \textbf{measure}, and rule two bounds the
  ...
  probability may not}.}
\sumitem{10}{\result{\textbf{Conditioning is changing the denominator}:
  $\Prob(A \mid B) = \Prob(A \cap B)/\Prob(B)$. Two of the four outcomes are
  gone entirely, and what is left is renormalised}.}
...
\sumitem{14--15}{\result{$\Prob(A \cap B) = \Prob(A \mid B)\Prob(B)$ is the definition
  ...
  numbers}.}
\sumitem{16--17}{\result{\textbf{Bayes' theorem is one quantity written two ways and
...
\sumitem{20--21}{\result{$\Prob(B) = \Prob(B \mid A)\Prob(A) +
  ...
  anybody mentioned it}.}
\sumitem{22--23}{\result{A detector $\val{p23.sens.pct}$ per cent accurate on a fault
  ...
  every real one}.}
\sumitem{23--24}{\result{$\val{p23.sens.pct}$ per cent is
  $\Prob(\text{alarm} \mid \text{fault})$ and the question was
  $\Prob(\text{fault} \mid \text{alarm})$. A detector's quoted numbers are
  properties of the detector \emph{and} the population, and the base rate
  moves only one of the two}.}
\sumitem{25--26}{\result{The \textbf{prosecutor's fallacy} is swapping those two:
...
\sumitem{26--28}{\result{Same detector, four base rates:
  $\val{p23.ppv.10.pct}$, $\val{p23.ppv.100.pct}$, $\val{p23.ppv.pct}$ and
  $\val{p23.ppv.10000.pct}$ per cent. \textbf{Compare the base rate with the
  error rate}: the alarms are half real at $p = f/(s+f)$, which for this
  detector is the error rate itself}, and the transcript prints it as a
  fraction so that it says so.}
\sumitem{29--30}{\result{A detector that never fires is $\val{p23.never.pct}$ per cent
...
\sumitem{31--32}{\result{$A$ and $B$ are independent when
  $\Prob(A \cap B) = \Prob(A)\Prob(B)$, which divided through says being told
  $B$ does not move your answer about $A$}.}
\sumitem{33--35}{\result{Two fair coins are independent and, given that exactly one is
...
\sumitem{36--37}{\result{Pairwise independence is not independence of the collection:
  those same three events are independent in every pair and have
  $\Prob(A \cap B \cap C) = 0$ against a product of $\frac{1}{8}$}.}
\sumitem{38--39}{\result{Two noisy readings of one cause are \emph{dependent}
...
\sumitem{40--41}{\result{Both directions happen, so neither word implies the other. The
...
\sumitem{42--43}{\result{One alarm at ratio $\val{p23.lr}$ gives
  ...
  $\val{p23.three.false.pct}$ per cent of triples false}.}
\sumitem{44--45}{\result{A duplicate detector has likelihood ratio $1$ and moves
...
\item Three events are pairwise independent and satisfy
  ...
  it is the only one. On more outcomes there are others, and the natural one
  is section 5's construction with three faces instead of two: let $X$ and $Y$
  be independent and uniform on $\{0, 1, 2\}$, and take $A = \{X = 0\}$,
  $B = \{Y = 0\}$ and $C = \{X + Y \equiv 1 \bmod 3\}$. Each has probability
  $\frac{1}{3}$, each pair meets in exactly one of the nine outcomes, and no
  outcome is in all three. A die does not work, incidentally: on six equally
  likely outcomes there is no such triple at all.}
