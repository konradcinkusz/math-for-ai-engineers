...
% ============================================================
\begin{summarybox}
\sumitem{1--2}{\textbf{A function is convex when the chord never dips below
  the curve}, for every pair of points and every position between them. That
  is the definition, not a picture of it.}
\sumitem{3--4}{\textbf{The inequality can be checked where the picture cannot},
...
\sumitem{7--8}{\textbf{The second test is Program~\ref{prog:P17}'s object}:
...
\sumitem{9--10}{\textbf{Every local minimum of a convex function is global},
  by the chord in two lines. That is what the promise is.}
\sumitem{11--12}{$\val{p19.basins.convex}$ basin against
...
\sumitem{13--14}{\textbf{It is a promise about the problem, not the
...
\sumitem{15--16}{\textbf{Jensen's inequality} is the chord read as an average:
  $f$ of a weighted mean is at most the weighted mean of $f$.}
\sumitem{17--18}{\textbf{Applying the convex function first gives the larger
  number}, and the two are equal exactly when every input is the same. For a
  concave function every inequality turns round.}
\sumitem{19--20}{\textbf{An average of perplexities is always the larger of
...
\sumitem{21}{\textbf{The belief survives because it is almost true}: the two
  agree exactly when every token costs the same, which is what a test fixture
  looks like.}
\sumitem{22--24}{\textbf{The size of the error is set by the spread of the
  ...
  real corpus.}
\sumitem{25--26}{Keep it apart from the macro/micro average
