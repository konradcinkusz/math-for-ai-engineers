...
\item A leaderboard averages the per-document perplexities of a model. What
  ...
  \answerto{A different quantity from the perplexity, and one that is never
  smaller. The perplexity is the exponential of the mean loss; the mean of the
  exponentials sits above it by a factor that grows with how varied the
  documents are, and equals it only when they are all the same.}
...
\mermaidfig{p19-one-basin}{Where the walk stopped, and what that is worth.}{%
  one basin, and the question it settles}
\end{fr}
...
\begin{fr}
\begin{ansblock}
The average of the perplexities \dash{} strictly, for those four, because
$e^{x}$ is convex and the four losses differ.
\end{ansblock}

So the two numbers are \textbf{ordered}, not merely different, and the wrong
one is never the smaller. It is strictly larger whenever the per-document
losses differ, and equal to the right one only when every document costs the
same. A harness with this bug therefore never flatters a model, and reports it
as worse than it is on every corpus that is not uniform.
...
\textbf{That one line is the evidence lower
bound.}\index{Jensen's inequality!and the ELBO} Written with an
...
% ============================================================
\begin{summarybox}
\sumitem{1--2}{\result{\textbf{A function is convex when the chord never dips
below
  the curve}, for every pair of points and every position between them. That
  is the definition, not a picture of it}.}
\sumitem{3--4}{\result{\textbf{The inequality can be checked where the picture
cannot},
...
\sumitem{7--8}{\result{\textbf{The second test is Program~\ref{prog:P17}'s
object}:
...
\sumitem{9--10}{\result{\textbf{Every local minimum of a convex function is
global},
...
\sumitem{15--16}{\result{\textbf{Jensen's inequality} is the chord read as an
average:
  $f$ of a weighted mean is at most the weighted mean of $f$}.}
\sumitem{17--18}{\result{\textbf{Applying the convex function first gives the
larger
  number}, and the two are equal exactly when every input is the same. For a
  concave function every inequality turns round}.}
\sumitem{19--20}{\result{\textbf{An average of perplexities is never the
smaller of
  the two}, because $e^{x}$ is convex \dash{} strictly larger whenever the
  per-document losses differ, equal when they do not, and
  Program~\ref{prog:F04}'s pair is gated against that ordering}.}
\sumitem{21}{\result{\textbf{The belief survives because it is almost true}:
the two
  agree exactly when every token costs the same, which is what a test fixture
  looks like}.}
\sumitem{22--24}{\result{\textbf{The size of the error is set by the spread of
the
...
\sumitem{27--28}{\result{\textbf{$\ln$ is concave, so the inequality turns
round}, and
...
\sumitem{31--32}{\result{\textbf{A network's loss is non-convex before any
activation
  or any depth}, because the weights of different layers multiply and a
  product of two variables is a saddle}.}
\sumitem{33--38}{\result{\textbf{What is lost is one claim and nothing else.}
The
