...
\label{prog:P20}
\index{optimiser}
\index{gradient descent!as an optimiser}
...
the script checks it at $\val{p20.sweep.kappas}$ condition numbers from
$\val{p20.sweep.k.lo}$ to
...
\mermaidfig{p20-each-fixes-one}{Each addition repairs the one before it.}{%
  a ceiling, a sideways walk, an average}
\end{fr}
...
A hyperparameter is only meaningful together with the line it sits in. Two
libraries can both offer \code{momentum=0.9} and mean different
...
For the small ones it falls short, and not by the same amount in both:
$\val{p20.adam.shortfall}$ per cent at the smaller of the two gradients and
$\val{p20.adam.shortfall.lo}$ at the larger.
...
\begin{fr}
The shortfall is not noise and it is not an approximation, and it has one
cause. No implementation can divide by $\sqrt{v}$ while $v$ is still near
zero, so every one of them carries a small $\varepsilon$ to stop that. The
departure from $\eta$ is entirely the epsilon's, and it is largest where the
gradient is \emph{smallest} \dash{} the opposite end from the one people
worry about.

Which raises the question the epsilon is usually written without any thought
about. Where should it go: inside the square root, added to $v$, or outside
it, added to $\sqrt{v}$?
\dotline
\nextframe
\end{fr}
...
That is the units argument again. With the epsilon outside, the first step is
exactly
\[ \frac{\eta}{1 + \varepsilon/\lvert g\rvert} \]
which the script checks in every coordinate at every scale, and it has a
measurable consequence. With $\varepsilon = \val{p20.eps}$ and a gradient of
$\val{p20.eps.g}$:
...
Measured, and run to equilibrium under both optimisers, because the comparison
that matters is between the two rules rather than between the two coordinates.
Two coordinates differing only in curvature, by a factor of
...
& steep & flat \\
\midrule
Adam, penalty in the gradient & $\val{p20.wd.l2.steep}$ & $\val{p20.wd.l2.flat}$ \\
Adam, penalty on the weight & $\val{p20.wd.decoupled}$ & $\val{p20.wd.decoupled.flat}$ \\
plain descent, penalty in the gradient & $\val{p20.wd.sgd.steep}$ & $\val{p20.wd.sgd.flat}$ \\
plain descent, penalty on the weight & $\val{p20.wd.sgd.steep}$ & $\val{p20.wd.sgd.flat}$ \\
\bottomrule
\end{tabular}
\end{center}

Three of those four rows are the same pair of numbers, and the one that is not
is the whole of the distinction. Plain descent's two rows agree to better than
$\val{p20.wd.sgd.gap}$, because for it the two forms are one update written
twice. Adam's do not.

And the pair three of them share is not Adam's doing either: at a fixed point
the whole gradient is zero, so any optimiser that adds $\lambda w$ before
stepping settles where the \emph{penalised} objective has its minimum, which
is $at/(a+\lambda)$ and is asserted against the closed form. That is where the
$\val{p20.wd.l2.spread}$ between the coordinates comes from \dash{} one
$\lambda$ meeting two curvatures, so the penalty nearly annihilates the
coordinate with the small gradient \dash{} and it is a property of the penalty
rather than of the method carrying it.

What belongs to Adam alone is the second row. Decoupled
decay\index{optimiser!decoupled decay}%
\index{AdamW|see{optimiser, decoupled decay}} never passes
through $\sqrt{v}$, so it pulls both coordinates by the same fraction of
themselves against a step whose size the normaliser has already fixed; at this
$\lambda$ that leaves both at their data minimum, to better than
$\val{p20.wd.gap}$. The penalty knows nothing about either coordinate, which
is the property it was decoupled to have.
\end{fr}

\begin{fr}
\begin{trapbox}
\textbf{\enquote{Adam with \code{weight\_decay} is $L_2$ regularisation.}} It
is not, and the table above is why: the two forms are one rule for plain
descent and two different rules for an adaptive method. Adding the penalty to
the gradient makes its effective strength a function of each parameter's own
gradient history, so \enquote{the regularisation strength} is not one number
and does not stay still.
...
\begin{summarybox}
\sumitem{1--2}{\result{Every optimiser here is $w \leftarrow w - \eta d$ with a
  different $d$. \textbf{The list is one line and a sequence of repairs},
  not six algorithms}.}
\sumitem{3--4}{\result{The step size is capped by the \emph{steepest} direction,
...
\sumitem{5--6}{\result{The wasted effort is a sideways component that reverses
every
...
\sumitem{7}{\result{On that bowl plain descent needs $\val{p20.steps.sgd}$
steps, and
  its predicted count from $(\kappa-1)/(\kappa+1)$ is \textbf{exact at every
  condition number tried}}.}
\sumitem{8--9}{\result{Momentum is Program~\ref{prog:F04}'s average with a
gradient
  in it: it cancels what alternates and keeps what does not, which is exactly
  the shape of the zig-zag}.}
\sumitem{10--11}{\result{It replaces $\kappa$ by $\sqrt{\kappa}$ \textbf{in the
  rate} \dash{} $\val{p20.rate.sgd}$ becomes $\val{p20.rate.mom}$}.}
\sumitem{12--14}{\result{The measured count is $\val{p20.steps.mom}$ against a
  ...
  count is not.}}}
\sumitem{15--16}{\result{The advantage grows with $\kappa$ and never reaches
...
\sumitem{17--19}{\result{$v \leftarrow \beta v + g$ and
  ...
  rescaling diverges}.}
\sumitem{20}{\result{What travels with a hyperparameter is the expression it
appears
  in, never the number on its own}.}
\sumitem{21--24}{\result{Dividing by $\sqrt{v}$ removes the units, so the step
is
...
\sumitem{25}{\result{Measured over twelve decades the first step is
...
\sumitem{26--27}{\result{The epsilon goes \textbf{outside} the root, where it is
  ...
  $\val{p20.eps.short.out}$}.}
\sumitem{28--29}{\result{Under Program~\ref{prog:P17}'s reparameterisation plain
  ...
  were written.}}}
\sumitem{30--31}{\result{Bias correction is Program~\ref{prog:F04}'s
  $1 - \beta^{\,t}$, one per coefficient, and at $\beta_2 = \num{0.999}$ it is
  working for hundreds of steps rather than for three}.}
\sumitem{32--33}{\result{A warmup protects a denominator estimated from almost
no
  data: the scale estimate swings by $\val{p20.warm.early}$ over ten steps and
  $\val{p20.warm.late}$ over three hundred}.}
\sumitem{34--35}{\result{A penalty can be added to the gradient or subtracted
from
  the weight, and \textbf{for plain descent the two are the same line}}.}
\sumitem{36--37}{\result{For an adaptive method they are not, and the four-row
  table is the evidence: three rows agree because any optimiser that adds
  $\lambda w$ before stepping settles at the penalised objective's own
  minimiser $at/(a+\lambda)$, and Adam's decoupled row is the one that does
  not}.}
\sumitem{38--39}{\result{\code{gamma} \emph{multiplies}:
...
\sumitem{40--42}{\result{A cosine and a line spend the same budget \dash{}
