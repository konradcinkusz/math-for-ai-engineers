...
\item You know a function's value, slope and curvature at a point. What is the
  best you can say about it nearby? \teachesat{1--4}
...
\item Program~\ref{prog:P15} found each direction multiplied by
  $1 - \eta\lambda$ every step. What ceiling does that put on $\eta$, and is
  the ceiling itself usable? \teachesat{16--17}
  \answerto{$\eta$ must be strictly under $2/\lambda$, because the walk stays
  bounded exactly when $\lvert 1 - \eta\lambda \rvert < 1$ \dash{} so there is
  no largest usable value, only a bound the set does not contain. At
  $2/\lambda$ exactly the factor is $-1$ and the coordinate never shrinks,
  which Test exercise 1 works out. With several directions the smallest of
  those bounds wins, so the ceiling is $2/\lambda_{\max}$.}
\item Which direction of a valley decides the step size, and which one has to
  ...
  condition number.}
\item Newton's method uses the curvature to choose a step for each direction
  separately. Why is it not used for training?
  \teachesat{26--27}
...
not survive, and the sign count is why: a minimum needs every sign to agree
and a saddle needs one dissenter, so a stationary point in high dimension is
...
it.}{the factor that decides whether it grows}
\end{fr}
...
\begin{fr}
\begin{ansblock}
Yes, with a margin. Its factor is
$1 - \val{p17.p15.eta} \times \val{p17.lam.hi} =
\val{p17.p15.factor}$: negative, so the steep coordinate changes side every
step, and smaller than one in size, so it shrinks.
\end{ansblock}

\textbf{The sign flip is not a warning.} It begins as soon as $\eta$ passes
$1/\lambda$, which here is half the boundary, so alternation is the normal
behaviour over the whole upper half of the stable range. A loss that
oscillates is not thereby one hair from divergence; it may be at half the
ceiling and perfectly safe, and telling the two apart needs the factor rather
...
Its consequence is a rate. The best single step size for a quadratic is
$2/(\lambda_{\max} + \lambda_{\min})$ \dash{} Further problem 2 asks why, and
the short answer is that it is where the two extreme directions are equally
badly served. Even at that step, $\val{p17.best.eta}$ here, the slow direction
...
solve $H\,\vect{s} = -\nabla f$ and step by $\vect{s}$. On a quadratic,
...
Put that beside Program~\ref{prog:F01}'s ladder of magnitudes and it clears
every rung but the top one. It is larger than the tokens in a training corpus
and larger than the arithmetic a device does in a second; the only thing above
it there is the whole arithmetic of training the model once, about a hundred
times larger. And that comparison is not like for like: those are quantities
you \emph{spend}, over months and across a cluster, where this is bytes that
have to be held at once. No storage that exists is within many orders of it.

The number of parameters is a number you can hold; the number of \emph{pairs}
of parameters is not, and squaring is what does it.
...
\mermaidfig{p17-sharp-is-not-a-fact}{The same function, two curvatures.}{%
  the same function, a different curvature}
\end{fr}
...
\begin{fr}
The honest position is worth stating in full, because it is easy to
over-correct into saying the whole idea is empty.
...
And notice that the awkward reparametrisation is not one case to be
legislated against. The rescaling above used one coordinate and one constant,
and nothing in the argument depended on either: every positive constant gives
another, and every coordinate gives a family of them. So the answer cannot be
a rule that excludes one inconvenient change of variables, because there is a
continuum of them and they are all equally legitimate.
...
\begin{fr}
\begin{ansblock}
Because $\lambda_{\max}$ rescales too. Under a \emph{uniform} rescaling it does
not matter, because $\eta$ rescales with it in the opposite direction and the
\emph{step actually taken} is unchanged.
\end{ansblock}

\textbf{Uniform is doing real work in that sentence.} Rescale one parameter
among many, which is the case section 5 built, and one shared $\eta$ cannot
follow it: $\lambda_{\max}$ moves and every other direction's step moves with
it. That is Further problem 4's case against this one, and the difference
between them is whether the change of coordinates is a change of \emph{every}
coordinate.
...
% ============================================================
\begin{summarybox}
\sumitem{1--2}{\result{\textbf{The second-order Taylor model is value, slope and half
  the curvature times the square}, and the $\tfrac{1}{2}$ is what makes the
  model's second derivative match the function's}.}
\sumitem{3--4}{\result{\textbf{\enquote{Best} means an order, not a size}: the
  ...
  quoting two errors}.}
\sumitem{5--6}{\result{\textbf{The model is local.} It makes the local description
  better, not wider, and says nothing about where the minimum is}.}
\sumitem{7--8}{\result{At a stationary point the linear term vanishes, so the sign of
  the curvature classifies \dash{} completely in one dimension, and not in
  more}.}
\sumitem{9--10}{\result{\textbf{The Hessian is the Jacobian of the gradient}, so
  Program~\ref{prog:P16}'s rule makes it square with one row and column per
  input}.}
\sumitem{11--12}{\result{\textbf{It is symmetric}, so Program~\ref{prog:P10} applies
  unchanged: real eigenvalues, perpendicular directions, each with its own
  curvature}.}
\sumitem{13--14}{\result{The eigenvalue signs give bowl, inverted bowl, saddle and
  ridge \dash{} P10's classification, now for any smooth function at a
  stationary point}.}
\sumitem{15}{\result{\textbf{A stationary point in high dimension is overwhelmingly
...
\sumitem{16--17}{\result{\textbf{The walk stays bounded exactly when
  $\lvert 1 - \eta\lambda\rvert < 1$, that is $\eta < 2/\lambda$}, which is a
  fact about a geometric sequence and about nothing else}.}
\sumitem{18--19}{\result{\textbf{The steepest direction sets the step for the whole
...
\sumitem{20--21}{\result{Program~\ref{prog:P15}'s walk at $\val{p17.p15.eta}$ is under
  ...
  growth is geometric}.}
\sumitem{22--23}{\result{\textbf{The condition number $\val{p17.kappa}$ is the same
...
\sumitem{24--25}{\result{\textbf{Newton's method lands on a quadratic's minimum in
  $\val{p17.newton.steps}$ step}, by giving each direction its own step size}.}
\sumitem{26--28}{\result{\textbf{And its Hessian has $\val{p17.hess.entries}$ entries
  ...
  cheap}.}
\sumitem{29--30}{\result{\textbf{Newton finds stationary points, not minima}, so at a
...
\sumitem{31--33}{\result{\textbf{Rescaling a parameter by $c$ changes the
  curvature by $c^{2}$ \dash{} $\val{p17.rescale.factor}$ at the $c$ the
  frames use \dash{} and leaves every output identical.} So one model
  has two sharpnesses and a bare sharpness number is a fact about the
  coordinates}.}
\sumitem{34}{\result{\textbf{No quantity built from the raw eigenvalues survives it}
...
\sumitem{35--36}{\result{What would survive is a measure that transforms as the
...
\sumitem{37--38}{\result{\textbf{The step-size bound has the same problem and,
  under a \emph{uniform} rescaling, does not suffer from it}, because
  $\eta$ rescales with $\lambda$ and the step taken is unchanged; rescale one
  parameter among many and a single shared $\eta$ cannot follow. The question
  about a curvature is never \emph{is it big} but \emph{compared with
  what}}.}
\end{summarybox}
...
\item A loss has Hessian eigenvalues $50$, $5$ and $\num{0.5}$ at the current
  point. Give the ceiling on the step, the condition number, and which
  direction moves most slowly as a result.
