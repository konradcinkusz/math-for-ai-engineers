...
survived five programs after one had been run.
...
E3 & The angle between random unit vectors as the dimension climbs, and the
  concentration towards a right angle & Program~\ref{prog:P05} \\
...
E8 & Forward against reverse divergence fitted to one bimodal target;
  mode-covering against mode-seeking & Program~\ref{prog:P30} \\
E9 & Score spread and attention entropy with and without the square-root
  divisor, across head sizes & Programs~\ref{prog:P25}
  and~\ref{prog:P32} \\
E10 & A power-law fit on the shape the literature uses, with the fit's own
  extrapolation error reported & Program~\ref{prog:P33} \\
\bottomrule
\end{tabularx}
\end{center}

\medskip
\textbf{Half run.} One specification has one clause met and the other not,
and the distinction is worth keeping rather than rounding to either side.
Whether a specification is met is a judgement about a program, not something
a table can settle.
...
\midrule
E1 & The score at which a naive $\softmax$ overflows each format, and the
  error of the naive computation against the stabilised one as the scores
  grow & Program~\ref{prog:P02} \\
\bottomrule
\end{tabularx}
\end{center}

Program~\ref{prog:P02} gives the score at which each of the four formats
overflows, which is the first clause exactly. It does not sweep the magnitude
and set the naive computation against the stabilised one, which is the second.
...
\begin{tabularx}{\linewidth}{@{}lXl@{}}
\toprule
\textbf{\#} & \textbf{What it would settle} & \textbf{Where it would go} \\
\midrule
...
E5 & Forward against reverse mode: time and peak memory against depth, and
  the measured cost of checkpointing & Program~\ref{prog:P16} \\
E7 & Interval width against evaluation-set size on a public benchmark; the
  size needed to resolve one point & Program~\ref{prog:P27} \\
\bottomrule
\end{tabularx}
\end{center}

E5 is the one worth a sentence, because a program did measure something
next to it. Program~\ref{prog:P16} counted operations exactly and derived
the checkpointing peak from the arithmetic, deliberately and with its
reasons given there. A wall clock and a peak-memory figure are a different
instrument, and they are what this specification asks for.
...
Boxes headed \enquote{\lblVerify} & $\val{appf.verifybox}$ \\
Frames that put a question to you & $\val{appf.elicit}$ \\
Polish renderings named in Appendix~\ref{app:D}, in
  $\val{appf.terms.rows}$ rows & $\val{appf.terms.rend}$ \\
...
\medskip
Three of those describe one edition where the build's own ledger describes
both, and in each the difference is a factor of two rather than a
disagreement: there are $\val{appf.transcripts.both}$ transcripts across the
two editions and $\val{appf.diagrams.both}$ diagrams, and the Polish
renderings likewise number twice $\val{appf.terms.rend}$, because each
edition carries its own. Quote whichever you mean and say which it is.

\medskip
The two frame counts do not describe a shortfall. The planned figure is the
sum of estimates made program by program, and each was made before its
neighbours existed: a program whose neighbour has already taught something
arrives with less left to do, so the estimates ran high. Every program in
...
rather than the Quiz: the Quiz's items serve on entry and on exit alike, so a
score difference across it measures memory of the questions.
\end{warning}

\medskip
Other things are owed, and each is worth naming on its own because each is a
different kind of debt. They are not counted here, for the reason the
experiment table gives: a list can be read and a tally decays. This one has
been three and has been two, and neither number was wrong when it was written.

\medskip
\textbf{The measurements that need a trained model.} These claims are stated
as mechanism and labelled as judgement rather than measured, because settling
them needs weights this book does not ship and does not download: whether a
real embedding matrix's spectrum falls the way low-rank methods assume;
whether the rank of a deep attention stack falls without any narrow layer to
force it; whether the basin a training walk lands in matters at the scale
people train at; whether the independence the square-root divisor assumes
survives training; and whether an adaptive optimiser reaches a better answer
than momentum on a real surface. Each is marked where it appears, and no
count of them is given here for the reason the experiment table gives.
...
\medskip
\textbf{And this book has no ISBN, so it cannot be catalogued.} The licences
let anybody read it and copy it. A distributor, a bookshop and a library each
need an identifier it has not got, and two languages at two paper formats
would need up to four of them. It is the only entry here that is not about
what the book says: the others are debts against the contents and this one is
a debt against whether anybody can obtain the book at all. The number is
issued rather than derived, which is why the copyright page names its absence
rather than leaving a reader to infer it from a blank.

\medskip
\textbf{The Polish edition has not settled every term it uses.} A batch, an
embedding and a learning rate are each written more than one way, and
Appendix~\ref{app:D} lists what it knows of rather than picking quietly, so a
reader who meets the second word knows it is the first. The cost is real and
it is navigational: a pointer can land on a different noun from the one it
cites. The instances where a pointer could be repaired on its own have been;
what is left needs the word settling, which is an editorial decision about a
whole edition rather than a defect to sweep.

\medskip
Those are the debts this book knows about, and whether they are all of them
is not something this page can establish: what is on the page can be counted,
and an absence nobody has yet noticed cannot. So this appendix is itself a
claim about the book, of exactly the kind the preceding pages spend their
length teaching you to distrust, and the only reason to give it more weight
than the others is that its numbers are counted by a machine and its
judgements are labelled as judgements.
