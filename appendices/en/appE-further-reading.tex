...
Formal specification: transition systems, inductive invariants, temporal logic
  & Program~\ref{prog:P14} teaches you to \emph{read} a theorem and
  deliberately not to write one, and an inductive invariant has to be written;
  Programs~\ref{prog:F10} and~\ref{prog:P13} give the set and the relation,
  and nothing here goes on to a state machine & Lamport; Jackson \\
\bottomrule
\end{tabularx}
\end{center}
...
what an invariant is for. Wayne is the practical route in and Lamport the
reference behind it; Jackson does the same ideas over relations, and Leino the
version that lives inside a program's own source.
