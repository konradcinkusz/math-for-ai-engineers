...
\medskip
And one exclusion that belongs to none of those three, which is worth saying
plainly rather than filing under the nearest heading. \textbf{Formal
specification} \dash{} transition systems, inductive invariants, temporal
logic, refinement \dash{} is not a rigorous foundation underneath what this
book teaches, because it is underneath nothing here. It is not a specialism
beginning where a working depth ends, because there is no working depth here
to begin from. And it is not a technique a library now performs, because the
thing it produces is a sentence rather than a computation.

What the book gives is the ground it stands on and then stops.
Program~\ref{prog:F10} gives the set and Program~\ref{prog:P13} the relation
built on it; Program~\ref{prog:P14} teaches you to \emph{read} a theorem and
deliberately not to write one \dash{} and an inductive invariant has to be
written. For the notation itself: Wayne, then Lamport; Jackson for the same
ideas over relations, and Leino for the version that lives inside a program's
own source.

\medskip
\textbf{And the exclusions this book is least sure about}, stated because
...
\emph{Formal specification} is excluded, and what makes that worth doubting is
the shape of the reader's job rather than the shape of the mathematics. The
argument for leaving it out was that an engineer's mathematics is the
mathematics of the model. The argument against it is that when the code
arrives written by something else, what is left is to say what correct means
and then to check it \dash{} and the first of those is a notation, which this
book does not teach. Program~\ref{prog:P25} marks where the statistical half
stops: a run of greens bounds a rate over the inputs it drew from and says
nothing about any one of them, and saying something about any one of them is
...
A concentration inequality, and why boundedness is enough &
