...
%
% Section D.1's table is NOT gated and is hand-authored on purpose. Its first
% two columns are literal text, because writing the contract macro there would
...
\noindent
Two languages, one book, and one place where the choices are written down. This
...
The tables are headed in English throughout, and that is a decision rather than
an oversight: the English term is the one a reader searches for, reads in a
paper and meets in an API, so it is the head that stays put. A Polish reader
looking a term up therefore reads down the middle column rather than the first.
...
The first two columns are set as literal text on purpose. Writing the contract
macro here would print the current edition's form in both columns and say
nothing.
...
to be surprised by: the variance is taken up in section~\ref{sec:D-boxes}, and
the interval in section~\ref{sec:D-unsettled}.

That table is the symbols. Punctuation diverges too, on the same mechanism, and
is named here rather than tabulated: \code{\textbackslash dash} sets an em dash
in this edition and a półpauza in the Polish one, and
\code{\textbackslash enquote} sets the quotation marks of whichever edition is
being built. Both are macros in the same two language files, and for the same
reason \dash{} a typographic convention is not something a translator can be
asked to carry in their head for a whole book.
...
and inflects it Polish \dash{} \emph{tokenizator}, \emph{benchmarku},
\emph{hiperparametr} \dash{} which is what Polish engineers say, and a calque
...
\plterm{gradient descent}{spadek gradientu}{Established. The book writes
  ...
  name in two grammatical forms rather than two words \dash{} which is why it
  is here rather than in section~\ref{sec:D-unsettled}.}
...
The terms below are rendered with more than one \emph{word} in the Polish
edition. They are listed rather than quietly picked, because a reader who meets
the second word needs to know it is the first one.
...
\textbf{English} & \textbf{Polish edition} & \textbf{note} \\
\midrule
\plterm{batch}{partia, wsad, batch}{One object; each rendering has a job.
  \emph{Partia} is the base, and the only one that compounds \dash{}
  \emph{mikropartia}; \emph{wsad} carries the adjective \emph{wsadowy}, which
  \emph{partia} cannot form; \emph{batch} is the borrowing. No count is given
  here: one was, and a fourth had appeared under it.}
...
\plterm{learning rate}{współczynnik uczenia, krok uczenia}{Both are in real
  use. \emph{Krok} reads as \emph{step}, which is what the coefficient
  produces rather than what it multiplies: it multiplies the gradient, and the
  step is the result.}
\bottomrule
\end{tabularx}
\end{center}
...
each other, and it is the argument for settling \emph{embedding} first.
\end{note}

\medskip
One row sits below rather than above for the opposite reason: it looks
unsettled and is not. \emph{Próba} and \emph{próbka} divide the work between
them rather than competing for it, and the division is worth knowing.
...
\medskip
Two more entries are worth naming for the same reason, and neither is a term.
