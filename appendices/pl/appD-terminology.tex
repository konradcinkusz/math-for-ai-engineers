...
%
% Tabela w sekcji D.1 NIE jest bramkowana i jest pisana ręcznie celowo. Dwie
% jej pierwsze kolumny to dosłowny tekst, bo wpisanie tam makra kontraktu
% wypisałoby formę bieżącego wydania w OBU kolumnach i nie powiedziałoby
% niczego -- a poza tym C10 zakazuje \tan, \cot, \arctan, \gcd i
% \operatorname{lcm} wprost i słusznie, bo proza nigdy nie może zaszywać formy,
% którą drugie wydanie składa inaczej. Parytet C3 pilnuje, by oba pliki
% językowe definiowały ten sam ZBIÓR makr; nic nie sprawdza, czy ta tabela
% opisuje je poprawnie. Czytaj ją więc
...
\noindent
Dwa języki, jedna książka i jedno miejsce, w którym spisano wybory. Ten dodatek
...
Tabele mają wszędzie hasła angielskie i jest to decyzja, a nie przeoczenie: to
terminu angielskiego czytelnik będzie szukał, to jego przeczyta w artykule i
zobaczy w API, więc to on jest hasłem, które się nie rusza. Polski czytelnik
szukający terminu czyta więc kolumnę środkową, a nie pierwszą.
...
Dwie pierwsze kolumny są złożone jako dosłowny tekst i jest to celowe.
Wpisanie tu makra kontraktu wypisałoby formę bieżącego wydania w obu kolumnach
i nie powiedziałoby niczego.
...
czytelnika: wariancja wraca w sekcji~\ref{sec:D-boxes}, a przedział
w sekcji~\ref{sec:D-unsettled}.

Ta tabela to symbole. Interpunkcja rozchodzi się tak samo, tym samym
mechanizmem, i jest tu nazwana, a nie stabelaryzowana:
\code{\textbackslash dash} składa w wydaniu angielskim pauzę, a w tym
półpauzę, a \code{\textbackslash enquote} składa cudzysłowy tego wydania,
które akurat jest budowane. Oba są makrami w tych samych dwóch plikach
językowych i z tego samego powodu \dash{} konwencji typograficznej nie da się
wymagać od tłumacza przez całą książkę.
...
angielskie i odmienia je po polsku \dash{} \emph{tokenizator},
\emph{benchmarku}, \emph{hiperparametr} \dash{} bo tak mówią polscy
inżynierowie,
...
\plterm{gradient descent}{spadek gradientu}{Ustalone. Książka pisze i
  ...
  w dwóch formach gramatycznych, a nie dwoma słowami \dash{} i dlatego jest
  tutaj, a nie w sekcji~\ref{sec:D-unsettled}.}
...
Terminy poniżej są w wydaniu polskim oddawane więcej niż jednym
\emph{słowem}. Są wypisane, a nie po cichu rozstrzygnięte, bo czytelnik, który
napotyka drugie słowo, musi wiedzieć, że to pierwsze.
...
\textbf{Angielski} & \textbf{Wydanie polskie} & \textbf{notatka} \\
\midrule
\plterm{batch}{partia, wsad, batch}{Jeden obiekt; każdy odpowiednik ma swoje
  zadanie. \emph{Partia} jest podstawą i jako jedyna tworzy złożenie \dash{}
  \emph{mikropartia}; \emph{wsad} niesie przymiotnik \emph{wsadowy}, którego
  \emph{partia} nie tworzy; \emph{batch} to zapożyczenie. Liczby tu nie
  podajemy: podano ją raz, a pod spodem pojawił się czwarty.}
...
\plterm{learning rate}{współczynnik uczenia, krok uczenia}{Obie formy są w
  realnym użyciu. \emph{Krok} to \emph{step}, czyli to, co ten współczynnik
  daje, a nie to, co mnoży: mnoży gradient, a krok jest wynikiem.}
\bottomrule
\end{tabularx}
\end{center}
...
\medskip
Jeden wiersz stoi poniżej, a nie powyżej, z odwrotnego powodu: wygląda na
nieustalony, a nie jest. \emph{Próba} i \emph{próbka} dzielą między siebie
pracę, zamiast o nią konkurować, a ten podział warto znać.
...
\medskip
Dwa dalsze hasła warto nazwać z tego samego powodu i żadne nie jest terminem.
$\intcc{a}{b}$ zachowuje nawiasy kwadratowe zamiast ostrych, których uczy
