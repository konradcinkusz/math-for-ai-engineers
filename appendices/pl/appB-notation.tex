...
%%
%% TABELA SYMBOLI ZOSTAŁA ODRZUCONA. Ten dodatek niósł kiedyś zaślepkę
%% obiecującą "każdy symbol, którego książka używa, w kolejności, w jakiej
%% czytelnik go spotyka, wraz z ramką, która go wprowadza". Zaślepka zniknęła
%% w #113, a uzasadnienie stoi w otwarciu dodatku, gdzie czytelnik je widzi.
%% To jest notatka dla następnego autora, żeby pytanie było zamknięte, a nie
%% otwierane na nowo z pustej sekcji.
%%
%% Sprawdzone, a nie przyjęte na wiarę, bo to jest twierdzenie o dwóch innych
%% programach: F02 wprowadza indeks dolny wprost
%% (programs/en/F02-language-algebra.tex, "A subscript is nearly always an
%% index"), a cały F04 buduje sigmę od zera. Robotę, którą miałaby wykonać
%% tabela symboli, wykonują więc programy wprowadzające te symbole, a tabela
%% byłaby drugim miejscem, w którym każdy z nich musiałby się zgadzać.
%%
%% Ile by to kosztowało, gdyby ktoś tę decyzję odwrócił. Makro to łatwa
%% połowa \dash{} oznacz każdy symbol przy pierwszym użyciu i odtwórz z tyłu,
%% dokładnie tak, jak Dodatek C robi to z \result{}. Trudna połowa jest taka,
%% że "w kolejności, w jakiej czytelnik go spotyka" jest własnością TEGO,
%% GDZIE znaczniki stoją, więc jest to przejście czytelnicze przez
%% czterdzieści siedem programów w dwóch wydaniach, a nie skryptowe, a
%% znacznik w złym programie po cichu przestawia tabelę.
...
materiały dydaktyczne do dziś piszą $D^{2}(X)$ na wariancję i $M(X)$ na wartość
oczekiwaną, obok $\Var$ i $\Ex$ \dash{} to jest bieżący uzus, a nie ciekawostka
historyczna, i Program~P24 podaje obie równoważności tam, gdzie pojęcia się
pojawiają. Ta książka pisze $\Var$ i $\Ex$, bo tak piszą biblioteki i artykuły,
które czytelnik otworzy, a to ten sam argument, który poniżej stawia test
klawiatury o nazwie.
\end{notationbox}
