...
| `11-ab-ovo-session-prompt.md` | The brief for the Claude Code session that starts `ab-ovo`, the learning application, in its own empty repository: what to attach, the decisions already taken, the phases with their definitions of done, and the ground rules — on the pattern of the standards' own `MASTER-PROMPT.md`, with the greenfield procedure delegated to `/init-generic-template` rather than retyped. |
