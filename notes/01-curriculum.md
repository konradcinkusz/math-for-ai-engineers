...
| Complex analysis, and the Fourier transform, **which the book does not mention at all** | F8 gives the unit circle; convolution theorems and spectral methods are a book of their own. This row used to read "beyond a mention", which asserts a mention: `grep -rni fourier programs/` returns nothing. | Bracewell; Osgood's lecture notes. |
...
| Formal specification: transition systems, inductive invariants, temporal logic, refinement | P14 teaches you to *read* a theorem and deliberately not to write one, and an inductive invariant has to be written. F10 gives the set and P13 the relation it is built on; nothing goes on to a state machine. P25 now says where the statistical half stops: a run of greens bounds a rate over the inputs it drew from and says nothing about any one of them. | Lamport, *Specifying Systems*; Wayne, *Practical TLA+*; Jackson, *Software Abstractions*; Leino, *Program Proofs*. |
| Category theory, differential geometry, manifolds | Occasionally invoked in interpretability writing; not load-bearing for the work. **No source here was surveyed for the category theory**, which Lee does not cover. | Lee, *Introduction to Smooth Manifolds*, for the geometry. |
