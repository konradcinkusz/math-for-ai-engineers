...
\newcommand{\lblQuizIntro}{%
  Take this before you read the program. If you score well now, work only the
  frames the questions send you to. The frame numbers beside each question are
  the route back, and they are the whole point of scoring it.}
...
\newcommand{\lblCanYouFooter}{%
  Then work the \lblTestExercises{}. They score what the program taught, item
  by item, and they are the instrument this book measures itself against. The
  \lblQuiz{} is not: it was the diagnostic on the way in, and answered a
  second time it is the same questions asked of a reader who now remembers
  the answers.}
...
% convention in their head forty-seven programs deep.
\newcommand{\dash}{---}
...
\newcommand{\lblTranscriptMissing}{TRANSCRIPT NOT COMPUTED --- run \texttt{make numbers}}
...
\newcommand{\lblNoResults}{No results were marked. This is a partial build.}
\newcommand{\lblResultsIntro}{%
  Every result the book establishes, in program order, each with the frames
  that establish it. Nothing here is transcribed: each line is marked in the
  Summary of the program that proves it, so the program and the frame range
  are the ones this book was on when it read the mark.}
...
% What a see-reference prints in the index. These are user-visible strings and
% differ between the editions, so they live here; preamble.tex builds \see and
% \seealso out of them rather than out of babel's \seename, which is not there
% on an installation where the preamble's babel probe found nothing.
\newcommand{\lblIndexSee}{see}
\newcommand{\lblIndexSeeAlso}{see also}

% ---------- PDF metadata ----------
% The subject, the keyword list and the document language are user-visible
% strings that differ between the editions, so they live here rather than in
% the four main files, where the PDF *title* still sits because the main files
% are the wiring and C15 guards their shape.
%
% preamble.tex sets its link colours in a \hypersetup that runs BEFORE it
% reads this file, so these three cannot go in that block -- they are applied
% in a second \hypersetup immediately after the \input, which is the first
% point at which they exist. Do not merge the two blocks.
\newcommand{\lblPdfSubject}{A programmed course in the mathematics an AI engineer needs, from arithmetic to one transformer block}
\newcommand{\lblPdfKeywords}{mathematics, machine learning, linear algebra, calculus, probability, information theory, programmed learning}
\newcommand{\lblPdfLang}{en-GB}

% ---------- The KDP volume edition ----------
% Used only by kdp/preamble-kdp.tex and the generated series map. They live
% here rather than in that preamble because they are user-visible strings that
% differ between the editions, which is what this file is for -- and because
% C3 compares the two catalogues, so a string added to one edition and not the
% other is an undefined control sequence in exactly one build.
%
% No count of volumes appears in any of these. The split lives in
% tools/volumes.json and the series map prints it; a sentence saying how many
% volumes there are would be a second copy of that fact, and this book has
% spent enough passes correcting tallies nothing can check.
\newcommand{\lblVolume}{Volume}
\newcommand{\lblSeriesMap}{The series}
\newcommand{\lblYouAreHere}{you are here}
\newcommand{\lblTitle}{Title}
\newcommand{\lblParts}{Parts}
\newcommand{\lblSeriesMapIntro}{This book is published as a series of volumes.
  Each one is self-contained: it carries the answers to its own programs, and
  the notation, formula reference and terminology for the material it holds.}
\newcommand{\lblSeriesMapNote}{A reference to a program in another volume is
  printed with that volume's number after it, like \lblProgram~12
  (\lblVolume~III). Further reading and the manifest are in the final volume.}
% The AI-assistance line on the volume copyright page. KDP asks separately, at
% upload, for a machine-readable declaration; that one is a decision about
% authorship and lives in tools/volumes.json, unset until a human sets it.
% This is only the sentence a reader sees.
\newcommand{\lblAiAssist}{This book was prepared with the assistance of AI
  tooling. Every number it prints is produced by a script in \texttt{code/}
